...
/**
 * Deserializes a binary PDU into a PoseArray object.
 * @param {ArrayBuffer} binary_data
 * @returns { PoseArray }
 */
export function pduToJs_PoseArray(binary_data) {
    const js_obj = new PoseArray();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_PoseArray(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_PoseArray(meta, binary_data, js_obj, base_off) {
    ...
    { // varray
        ...
        for (let i = 0; i < array_size; i++) {
            ...
            binary_read_recursive_Pose(meta, binary_data, tmp_obj, item_offset);
...
/**
 * Serializes a PoseArray object into a binary PDU.
 * @param { PoseArray } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_PoseArray(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_PoseArray(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_PoseArray(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: header, type: std_msgs/Header (struct)
    ...
    { // varray
        ...
        for (let item_index = 0; item_index < array_size; item_index++) {
            ...
            binary_write_recursive_Pose(item_offset, bw_container, bw_container.heap_allocator, js_obj.poses[item_index]);
        }
