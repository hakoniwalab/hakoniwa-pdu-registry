defmodule HakoPdu.HakoMsgs.ShareObjectOwnerConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_share_object_owner(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.ShareObjectOwner.new()
    binary_read_recursive_share_object_owner(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_share_object_owner(obj) do
    {base_data, heap_data} = binary_write_recursive_share_object_owner(0, "", "", obj)
  ...
  def binary_read_recursive_share_object_owner(meta, binary_data, obj, base_off) do
    ...
    owner_id = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: last_update, type: uint64 (primitive), offset: 136, size: 8
    last_update = read_primitive("uint64", binary_part(binary_data, base_off + 136, 8))
  ...
  def binary_write_recursive_share_object_owner(parent_off, heap_data, allocator, obj) do
    # member: object_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.object_name, 128))

    # member: owner_id, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.owner_id, 4))

    # member: last_update, type: uint64 (primitive), offset: 136, size: 8
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("uint64", obj.last_update, 8))
