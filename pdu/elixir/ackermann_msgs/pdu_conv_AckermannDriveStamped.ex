defmodule HakoPdu.AckermannMsgs.AckermannDriveStampedConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ackermann_drive_stamped(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.AckermannMsgs.AckermannDriveStamped.new()
    binary_read_recursive_ackermann_drive_stamped(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ackermann_drive_stamped(obj) do
    {base_data, heap_data} = binary_write_recursive_ackermann_drive_stamped(0, "", "", obj)
  ...
  def binary_read_recursive_ackermann_drive_stamped(meta, binary_data, obj, base_off) do
    ...
    drive =
      HakoPdu.AckermannMsgs.AckermannDriveConverter.binary_read_recursive_ackermann_drive(
        meta,
        binary_data,
        HakoPdu.AckermannMsgs.AckermannDrive.new(),
        base_off + 136
      )
  ...
  def binary_write_recursive_ackermann_drive_stamped(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: drive, type: AckermannDrive (struct), offset: 136, size: 20
    {allocator, heap_data} =
      HakoPdu.AckermannMsgs.AckermannDriveConverter.binary_write_recursive_ackermann_drive(
        parent_off + 136,
        heap_data,
        allocator,
        obj.drive
