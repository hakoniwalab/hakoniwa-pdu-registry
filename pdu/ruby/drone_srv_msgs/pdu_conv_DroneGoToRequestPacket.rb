require_relative 'pdu_type_DroneGoToRequestPacket'
...
require_relative 'pdu_conv_DroneGoToRequest'


module HakoPdu::DroneSrvMsgs::DroneGoToRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_go_to_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneGoToRequestPacket.new
    binary_read_recursive_drone_go_to_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_go_to_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_go_to_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_go_to_request_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: DroneGoToRequest (struct), offset: 272, size: 168
    obj.body = HakoPdu::DroneSrvMsgs::DroneGoToRequest.new
    HakoPdu::DroneSrvMsgs::DroneGoToRequestConverter.binary_read_recursive_drone_go_to_request(meta, binary_data, obj.body, base_off + 272)

    obj
  end

  def self.binary_write_recursive_drone_go_to_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DroneGoToRequest (struct), offset: 272, size: 168
    HakoPdu::DroneSrvMsgs::DroneGoToRequestConverter.binary_write_recursive_drone_go_to_request(parent_off + 272, heap_data, allocator, obj.body)
