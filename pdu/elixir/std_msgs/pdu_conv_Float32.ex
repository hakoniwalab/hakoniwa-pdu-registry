defmodule HakoPdu.StdMsgs.Float32Converter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_float32(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.Float32.new()
    binary_read_recursive_float32(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_float32(obj) do
    {base_data, heap_data} = binary_write_recursive_float32(0, "", "", obj)
  ...
  def binary_read_recursive_float32(meta, binary_data, obj, base_off) do
    # member: data, type: float32 (primitive), offset: 0, size: 4
    data = read_primitive("float32", binary_part(binary_data, base_off + 0, 4))
  ...
  def binary_write_recursive_float32(parent_off, heap_data, allocator, obj) do
    # member: data, type: float32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float32", obj.data, 4))
