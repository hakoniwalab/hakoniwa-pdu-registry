...
require_relative 'pdu_conv_Point32'


module HakoPdu::GeometryMsgs::PolygonConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_polygon(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Polygon.new
    binary_read_recursive_polygon(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_polygon(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_polygon(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_polygon(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_polygon(parent_off, heap_data, allocator, obj)
    # member: points, type: Point32 (struct), offset: 0, size: 12
