...
module HakoPdu::SensorMsgs::ChannelFloat32Converter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_channel_float32(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::ChannelFloat32.new
    binary_read_recursive_channel_float32(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_channel_float32(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_channel_float32(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_channel_float32(meta, binary_data, obj, base_off)
    ...
    array_size = read_primitive('int32', binary_data.byteslice(base_off + 128, 4))
    offset_from_heap = read_primitive('int32', binary_data.byteslice(base_off + 128 + 4, 4))
    obj.values = read_primitive_array('float32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_channel_float32(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    ...
    values = obj.values || []
    add_binary(allocator, parent_off + 128, [values.length, offset_from_heap].pack('l<l<'))
