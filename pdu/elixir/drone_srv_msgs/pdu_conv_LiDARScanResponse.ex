defmodule HakoPdu.DroneSrvMsgs.LiDARScanResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_li_d_a_r_scan_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.LiDARScanResponse.new()
    binary_read_recursive_li_d_a_r_scan_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_li_d_a_r_scan_response(obj) do
    {base_data, heap_data} = binary_write_recursive_li_d_a_r_scan_response(0, "", "", obj)
  ...
  def binary_read_recursive_li_d_a_r_scan_response(meta, binary_data, obj, base_off) do
    ...
    point_cloud =
      HakoPdu.SensorMsgs.PointCloud2Converter.binary_read_recursive_point_cloud2(
        meta,
        binary_data,
        HakoPdu.SensorMsgs.PointCloud2.new(),
        base_off + 4
      )
    ...
    message = read_primitive("string", binary_part(binary_data, base_off + 240, 128))
  ...
  def binary_write_recursive_li_d_a_r_scan_response(parent_off, heap_data, allocator, obj) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.ok, 4))

    # member: point_cloud, type: sensor_msgs/PointCloud2 (struct), offset: 4, size: 176
    {allocator, heap_data} =
      HakoPdu.SensorMsgs.PointCloud2Converter.binary_write_recursive_point_cloud2(
        parent_off + 4,
        heap_data,
        allocator,
        obj.point_cloud
    ...
    # member: message, type: string (primitive), offset: 240, size: 128
    allocator = add_binary(allocator, parent_off + 240, primitive_to_bin("string", obj.message, 128))
