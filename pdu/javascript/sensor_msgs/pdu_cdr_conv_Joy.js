import { Joy } from './pdu_jstype_Joy.js';
import { PduCdrWriter, PduCdrReader } from '../pdu_cdr_runtime.js';
import { Header } from '../std_msgs/pdu_jstype_Header.js';
import { PduHeaderConverter } from '../std_msgs/pdu_cdr_conv_Header.js';
import { Time } from '../builtin_interfaces/pdu_jstype_Time.js';
import { PduTimeConverter } from '../builtin_interfaces/pdu_cdr_conv_Time.js';


export class PduJoyConverter {
    /**
     * @param {PduCdrWriter} writer
     * @param { Joy } src
     */
    static to_cdr_body(writer, src) {
        PduHeaderConverter.to_cdr_body(writer, src.header);
        writer.write_sequence_length(src.axes);
        for (const elem of src.axes) {
            writer.write_float32(elem);
        }
        writer.write_sequence_length(src.buttons);
        for (const elem of src.buttons) {
            writer.write_int32(elem);
        }
    }

    /**
     * @param {PduCdrReader} reader
     * @param { Joy } dst
     * @returns { Joy }
     */
    static cdr_body_to_js(reader, dst) {
        PduHeaderConverter.cdr_body_to_js(reader, dst.header);
        dst.axes = Array.from({ length: reader.read_uint32() }, () => reader.read_float32());
        dst.buttons = Array.from({ length: reader.read_uint32() }, () => reader.read_int32());
        return dst;
    }

    /**
     * @param { Joy } src
     * @returns {ArrayBuffer}
     */
    static to_cdr(src) {
        const writer = new PduCdrWriter();
        writer.write_encapsulation();
        this.to_cdr_body(writer, src);
        return writer.get_buf();
    }

    /**
     * @param {ArrayBuffer|ArrayBufferView} cdrPayload
     * @returns { Joy }
     */
    static from_cdr(cdrPayload) {
        const reader = new PduCdrReader(cdrPayload);
        reader.read_encapsulation();
        return this.cdr_body_to_js(reader, new Joy());
    }
}
