...
require_relative 'pdu_conv_MultiArrayDimension'


module HakoPdu::StdMsgs::MultiArrayLayoutConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_multi_array_layout(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::MultiArrayLayout.new
    binary_read_recursive_multi_array_layout(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_multi_array_layout(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_multi_array_layout(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_multi_array_layout(meta, binary_data, obj, base_off)
    ...
    array_size.times do |i|
      child = HakoPdu::StdMsgs::MultiArrayDimension.new
      HakoPdu::StdMsgs::MultiArrayDimensionConverter.binary_read_recursive_multi_array_dimension(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 136)
      obj.dim << child
    end

    # member: data_offset, type: uint32 (primitive), offset: 8, size: 4
    obj.data_offset = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))

    obj
  end

  def self.binary_write_recursive_multi_array_layout(parent_off, heap_data, allocator, obj)
    # member: dim, type: MultiArrayDimension (struct), offset: 0, size: 136
    ...
    values.each_with_index do |child, i|
      HakoPdu::StdMsgs::MultiArrayDimensionConverter.binary_write_recursive_multi_array_dimension(offset_from_heap + i * 136, heap_data, heap_data, child)
    end

    # member: data_offset, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.data_offset, 4))
