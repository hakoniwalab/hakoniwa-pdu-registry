require_relative 'pdu_type_PolygonStamped'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_Polygon'
require_relative 'pdu_conv_Polygon'


module HakoPdu::GeometryMsgs::PolygonStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_polygon_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::PolygonStamped.new
    binary_read_recursive_polygon_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_polygon_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_polygon_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_polygon_stamped(meta, binary_data, obj, base_off)
    ...
    # member: polygon, type: Polygon (struct), offset: 136, size: 8
    obj.polygon = HakoPdu::GeometryMsgs::Polygon.new
    HakoPdu::GeometryMsgs::PolygonConverter.binary_read_recursive_polygon(meta, binary_data, obj.polygon, base_off + 136)

    obj
  end

  def self.binary_write_recursive_polygon_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: polygon, type: Polygon (struct), offset: 136, size: 8
    HakoPdu::GeometryMsgs::PolygonConverter.binary_write_recursive_polygon(parent_off + 136, heap_data, allocator, obj.polygon)
