defmodule HakoPdu.GeometryMsgs.WrenchStampedConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_wrench_stamped(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.WrenchStamped.new()
    binary_read_recursive_wrench_stamped(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_wrench_stamped(obj) do
    {base_data, heap_data} = binary_write_recursive_wrench_stamped(0, "", "", obj)
  ...
  def binary_read_recursive_wrench_stamped(meta, binary_data, obj, base_off) do
  ...
  def binary_write_recursive_wrench_stamped(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
