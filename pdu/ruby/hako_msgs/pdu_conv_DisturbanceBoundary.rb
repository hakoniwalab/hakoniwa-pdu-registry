...
module HakoPdu::HakoMsgs::DisturbanceBoundaryConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_disturbance_boundary(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::DisturbanceBoundary.new
    binary_read_recursive_disturbance_boundary(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_disturbance_boundary(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_disturbance_boundary(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_disturbance_boundary(meta, binary_data, obj, base_off)
    # member: boundary_point, type: geometry_msgs/Point (struct), offset: 0, size: 24
    obj.boundary_point = HakoPdu::GeometryMsgs::Point.new
    HakoPdu::GeometryMsgs::PointConverter.binary_read_recursive_point(meta, binary_data, obj.boundary_point, base_off + 0)

    # member: boundary_normal, type: geometry_msgs/Vector3 (struct), offset: 24, size: 24
    obj.boundary_normal = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.boundary_normal, base_off + 24)

    obj
  end

  def self.binary_write_recursive_disturbance_boundary(parent_off, heap_data, allocator, obj)
    # member: boundary_point, type: geometry_msgs/Point (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::PointConverter.binary_write_recursive_point(parent_off + 0, heap_data, allocator, obj.boundary_point)

    # member: boundary_normal, type: geometry_msgs/Vector3 (struct), offset: 24, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 24, heap_data, allocator, obj.boundary_normal)
