defmodule HakoPdu.DroneSrvMsgs.DroneTakeOffRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_take_off_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.DroneTakeOffRequest.new()
    binary_read_recursive_drone_take_off_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_take_off_request(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_take_off_request(0, "", "", obj)
  ...
  def binary_read_recursive_drone_take_off_request(meta, binary_data, obj, base_off) do
    # member: alt_m, type: float64 (primitive), offset: 0, size: 8
    alt_m = read_primitive("float64", binary_part(binary_data, base_off + 0, 8))

    # member: drone_name, type: string (primitive), offset: 8, size: 128
    drone_name = read_primitive("string", binary_part(binary_data, base_off + 8, 128))
  ...
  def binary_write_recursive_drone_take_off_request(parent_off, heap_data, allocator, obj) do
    # member: alt_m, type: float64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float64", obj.alt_m, 8))

    # member: drone_name, type: string (primitive), offset: 8, size: 128
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("string", obj.drone_name, 128))
