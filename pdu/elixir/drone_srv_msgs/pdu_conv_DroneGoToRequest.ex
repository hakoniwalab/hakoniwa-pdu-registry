defmodule HakoPdu.DroneSrvMsgs.DroneGoToRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_go_to_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.DroneGoToRequest.new()
    binary_read_recursive_drone_go_to_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_go_to_request(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_go_to_request(0, "", "", obj)
  ...
  def binary_read_recursive_drone_go_to_request(meta, binary_data, obj, base_off) do
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    drone_name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))
    ...
    speed_m_s = read_primitive("float32", binary_part(binary_data, base_off + 152, 4))

    # member: yaw_deg, type: float32 (primitive), offset: 156, size: 4
    yaw_deg = read_primitive("float32", binary_part(binary_data, base_off + 156, 4))

    # member: tolerance_m, type: float32 (primitive), offset: 160, size: 4
    tolerance_m = read_primitive("float32", binary_part(binary_data, base_off + 160, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 164, size: 4
    timeout_sec = read_primitive("float32", binary_part(binary_data, base_off + 164, 4))
  ...
  def binary_write_recursive_drone_go_to_request(parent_off, heap_data, allocator, obj) do
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.drone_name, 128))
    ...
    # member: speed_m_s, type: float32 (primitive), offset: 152, size: 4
    allocator = add_binary(allocator, parent_off + 152, primitive_to_bin("float32", obj.speed_m_s, 4))

    # member: yaw_deg, type: float32 (primitive), offset: 156, size: 4
    allocator = add_binary(allocator, parent_off + 156, primitive_to_bin("float32", obj.yaw_deg, 4))

    # member: tolerance_m, type: float32 (primitive), offset: 160, size: 4
    allocator = add_binary(allocator, parent_off + 160, primitive_to_bin("float32", obj.tolerance_m, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 164, size: 4
    allocator = add_binary(allocator, parent_off + 164, primitive_to_bin("float32", obj.timeout_sec, 4))
