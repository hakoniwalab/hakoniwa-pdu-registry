defmodule HakoPdu.HakoMsgs.SimpleVarrayConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_simple_varray(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.SimpleVarray.new()
    binary_read_recursive_simple_varray(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_simple_varray(obj) do
    {base_data, heap_data} = binary_write_recursive_simple_varray(0, "", "", obj)
  ...
  def binary_read_recursive_simple_varray(meta, binary_data, obj, base_off) do
    # member: data, type: int8 (primitive), offset: 0, size: 1
    ...
    # member: fixed_array, type: int8 (primitive), offset: 8, size: 10
    fixed_array = read_primitive_array("int8", binary_part(binary_data, base_off + 8, 10), 10, div(10, 10))

    # member: p_mem1, type: int32 (primitive), offset: 20, size: 4
    p_mem1 = read_primitive("int32", binary_part(binary_data, base_off + 20, 4))
  ...
  def binary_write_recursive_simple_varray(parent_off, heap_data, allocator, obj) do
    # member: data, type: int8 (primitive), offset: 0, size: 1
    ...
    # member: fixed_array, type: int8 (primitive), offset: 8, size: 10
    allocator = add_binary(allocator, parent_off + 8, primitive_array_to_bin("int8", obj.fixed_array, div(10, 10)))

    # member: p_mem1, type: int32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("int32", obj.p_mem1, 4))
