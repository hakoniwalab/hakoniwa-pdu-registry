require_relative 'pdu_type_OccupancyGrid'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_MapMetaData'
require_relative 'pdu_conv_MapMetaData'


module HakoPdu::NavMsgs::OccupancyGridConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_occupancy_grid(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::NavMsgs::OccupancyGrid.new
    binary_read_recursive_occupancy_grid(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_occupancy_grid(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_occupancy_grid(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_occupancy_grid(meta, binary_data, obj, base_off)
    ...
    # member: info, type: MapMetaData (struct), offset: 136, size: 80
    obj.info = HakoPdu::NavMsgs::MapMetaData.new
    HakoPdu::NavMsgs::MapMetaDataConverter.binary_read_recursive_map_meta_data(meta, binary_data, obj.info, base_off + 136)

    # member: data, type: int8 (primitive), offset: 216, size: 1
    array_size = read_primitive('int32', binary_data.byteslice(base_off + 216, 4))
    offset_from_heap = read_primitive('int32', binary_data.byteslice(base_off + 216 + 4, 4))
    obj.data = read_primitive_array('int8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    obj
  end

  def self.binary_write_recursive_occupancy_grid(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: info, type: MapMetaData (struct), offset: 136, size: 80
    HakoPdu::NavMsgs::MapMetaDataConverter.binary_write_recursive_map_meta_data(parent_off + 136, heap_data, allocator, obj.info)

    # member: data, type: int8 (primitive), offset: 216, size: 1
    offset_from_heap = heap_data.bytesize
    values = obj.data || []
    add_binary(allocator, parent_off + 216, [values.length, offset_from_heap].pack('l<l<'))
    add_binary(heap_data, offset_from_heap, primitive_array_to_bin('int8', values, 1))
