...
module HakoPdu::HakoMsgs::SimpleVarrayConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_simple_varray(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::SimpleVarray.new
    binary_read_recursive_simple_varray(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_simple_varray(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_simple_varray(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_simple_varray(meta, binary_data, obj, base_off)
    ...
    obj.fixed_array = read_primitive_array('int8', binary_data.byteslice(base_off + 8, 10), 10, 10 / 10)

    # member: p_mem1, type: int32 (primitive), offset: 20, size: 4
    obj.p_mem1 = read_primitive('int32', binary_data.byteslice(base_off + 20, 4))

    obj
  end

  def self.binary_write_recursive_simple_varray(parent_off, heap_data, allocator, obj)
    # member: data, type: int8 (primitive), offset: 0, size: 1
    ...
    # member: fixed_array, type: int8 (primitive), offset: 8, size: 10
    add_binary(allocator, parent_off + 8, primitive_array_to_bin('int8', obj.fixed_array, 10 / 10))

    # member: p_mem1, type: int32 (primitive), offset: 20, size: 4
    add_binary(allocator, parent_off + 20, primitive_to_bin('int32', obj.p_mem1, 4))
