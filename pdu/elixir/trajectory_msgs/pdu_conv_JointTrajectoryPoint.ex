defmodule HakoPdu.TrajectoryMsgs.JointTrajectoryPointConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joint_trajectory_point(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.TrajectoryMsgs.JointTrajectoryPoint.new()
    binary_read_recursive_joint_trajectory_point(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joint_trajectory_point(obj) do
    {base_data, heap_data} = binary_write_recursive_joint_trajectory_point(0, "", "", obj)
  ...
  def binary_read_recursive_joint_trajectory_point(meta, binary_data, obj, base_off) do
    ...
    positions = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)
    ...
    velocities = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)
    ...
    accelerations = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)
    ...
    effort = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)

    # member: time_from_start, type: builtin_interfaces/Duration (struct), offset: 32, size: 8
    time_from_start =
      HakoPdu.BuiltinInterfaces.DurationConverter.binary_read_recursive_duration(
        meta,
        binary_data,
        HakoPdu.BuiltinInterfaces.Duration.new(),
        base_off + 32
      )
  ...
  def binary_write_recursive_joint_trajectory_point(parent_off, heap_data, allocator, obj) do
    # member: positions, type: float64 (primitive), offset: 0, size: 8
    ...
    # member: time_from_start, type: builtin_interfaces/Duration (struct), offset: 32, size: 8
    {allocator, heap_data} =
      HakoPdu.BuiltinInterfaces.DurationConverter.binary_write_recursive_duration(
        parent_off + 32,
        heap_data,
        allocator,
        obj.time_from_start
