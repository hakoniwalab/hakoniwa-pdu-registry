defmodule HakoPdu.HakoSrvMsgs.GetSimStateResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_get_sim_state_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.GetSimStateResponse.new()
    binary_read_recursive_get_sim_state_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_get_sim_state_response(obj) do
    {base_data, heap_data} = binary_write_recursive_get_sim_state_response(0, "", "", obj)
  ...
  def binary_read_recursive_get_sim_state_response(meta, binary_data, obj, base_off) do
    # member: sim_state, type: uint32 (primitive), offset: 0, size: 4
    sim_state = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: master_time, type: int64 (primitive), offset: 8, size: 8
    master_time = read_primitive("int64", binary_part(binary_data, base_off + 8, 8))

    # member: is_pdu_created, type: bool (primitive), offset: 16, size: 4
    is_pdu_created = read_primitive("bool", binary_part(binary_data, base_off + 16, 4))

    # member: is_simulation_mode, type: bool (primitive), offset: 20, size: 4
    is_simulation_mode = read_primitive("bool", binary_part(binary_data, base_off + 20, 4))

    # member: is_pdu_sync_mode, type: bool (primitive), offset: 24, size: 4
    is_pdu_sync_mode = read_primitive("bool", binary_part(binary_data, base_off + 24, 4))
  ...
  def binary_write_recursive_get_sim_state_response(parent_off, heap_data, allocator, obj) do
    # member: sim_state, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.sim_state, 4))

    # member: master_time, type: int64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("int64", obj.master_time, 8))

    # member: is_pdu_created, type: bool (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("bool", obj.is_pdu_created, 4))

    # member: is_simulation_mode, type: bool (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("bool", obj.is_simulation_mode, 4))

    # member: is_pdu_sync_mode, type: bool (primitive), offset: 24, size: 4
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("bool", obj.is_pdu_sync_mode, 4))
