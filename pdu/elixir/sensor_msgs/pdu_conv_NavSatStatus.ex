defmodule HakoPdu.SensorMsgs.NavSatStatusConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_nav_sat_status(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.NavSatStatus.new()
    binary_read_recursive_nav_sat_status(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_nav_sat_status(obj) do
    {base_data, heap_data} = binary_write_recursive_nav_sat_status(0, "", "", obj)
  ...
  def binary_read_recursive_nav_sat_status(meta, binary_data, obj, base_off) do
    # member: status, type: int8 (primitive), offset: 0, size: 1
    status = read_primitive("int8", binary_part(binary_data, base_off + 0, 1))

    # member: service, type: uint16 (primitive), offset: 2, size: 2
    service = read_primitive("uint16", binary_part(binary_data, base_off + 2, 2))
  ...
  def binary_write_recursive_nav_sat_status(parent_off, heap_data, allocator, obj) do
    # member: status, type: int8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int8", obj.status, 1))

    # member: service, type: uint16 (primitive), offset: 2, size: 2
    allocator = add_binary(allocator, parent_off + 2, primitive_to_bin("uint16", obj.service, 2))
