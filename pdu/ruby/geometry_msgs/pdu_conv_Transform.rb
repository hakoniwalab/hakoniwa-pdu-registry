require_relative 'pdu_type_Transform'
require_relative 'pdu_type_Vector3'
require_relative 'pdu_conv_Vector3'
require_relative 'pdu_type_Quaternion'
require_relative 'pdu_conv_Quaternion'


module HakoPdu::GeometryMsgs::TransformConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_transform(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Transform.new
    binary_read_recursive_transform(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_transform(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_transform(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_transform(meta, binary_data, obj, base_off)
    # member: translation, type: Vector3 (struct), offset: 0, size: 24
    obj.translation = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.translation, base_off + 0)

    # member: rotation, type: Quaternion (struct), offset: 24, size: 32
    obj.rotation = HakoPdu::GeometryMsgs::Quaternion.new
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_read_recursive_quaternion(meta, binary_data, obj.rotation, base_off + 24)

    obj
  end

  def self.binary_write_recursive_transform(parent_off, heap_data, allocator, obj)
    # member: translation, type: Vector3 (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 0, heap_data, allocator, obj.translation)

    # member: rotation, type: Quaternion (struct), offset: 24, size: 32
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_write_recursive_quaternion(parent_off + 24, heap_data, allocator, obj.rotation)
