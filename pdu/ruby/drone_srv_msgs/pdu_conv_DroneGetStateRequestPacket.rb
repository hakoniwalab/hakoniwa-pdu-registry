require_relative 'pdu_type_DroneGetStateRequestPacket'
require_relative '../hako_srv_msgs/pdu_type_ServiceRequestHeader'
require_relative '../hako_srv_msgs/pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_DroneGetStateRequest'
require_relative 'pdu_conv_DroneGetStateRequest'


module HakoPdu::DroneSrvMsgs::DroneGetStateRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_get_state_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneGetStateRequestPacket.new
    binary_read_recursive_drone_get_state_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_get_state_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_get_state_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_get_state_request_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: DroneGetStateRequest (struct), offset: 268, size: 128
    obj.body = HakoPdu::DroneSrvMsgs::DroneGetStateRequest.new
    HakoPdu::DroneSrvMsgs::DroneGetStateRequestConverter.binary_read_recursive_drone_get_state_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_drone_get_state_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DroneGetStateRequest (struct), offset: 268, size: 128
    HakoPdu::DroneSrvMsgs::DroneGetStateRequestConverter.binary_write_recursive_drone_get_state_request(parent_off + 268, heap_data, allocator, obj.body)
