...
module HakoPdu::HakoSrvMsgs::LogicalTimeConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_logical_time(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::LogicalTime.new
    binary_read_recursive_logical_time(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_logical_time(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_logical_time(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_logical_time(meta, binary_data, obj, base_off)
    # member: generation, type: uint64 (primitive), offset: 0, size: 8
    obj.generation = read_primitive('uint64', binary_data.byteslice(base_off + 0, 8))

    # member: offset_tick, type: uint64 (primitive), offset: 8, size: 8
    obj.offset_tick = read_primitive('uint64', binary_data.byteslice(base_off + 8, 8))

    obj
  end

  def self.binary_write_recursive_logical_time(parent_off, heap_data, allocator, obj)
    # member: generation, type: uint64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint64', obj.generation, 8))

    # member: offset_tick, type: uint64 (primitive), offset: 8, size: 8
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint64', obj.offset_tick, 8))
