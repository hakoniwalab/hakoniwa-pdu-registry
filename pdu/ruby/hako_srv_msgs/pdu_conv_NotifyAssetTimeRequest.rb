require_relative 'pdu_type_NotifyAssetTimeRequest'
require_relative 'pdu_type_LogicalTime'
require_relative 'pdu_conv_LogicalTime'


module HakoPdu::HakoSrvMsgs::NotifyAssetTimeRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_notify_asset_time_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::NotifyAssetTimeRequest.new
    binary_read_recursive_notify_asset_time_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_notify_asset_time_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_notify_asset_time_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_notify_asset_time_request(meta, binary_data, obj, base_off)
    # member: session_id, type: uint64 (primitive), offset: 0, size: 8
    obj.session_id = read_primitive('uint64', binary_data.byteslice(base_off + 0, 8))

    # member: asset_time, type: LogicalTime (struct), offset: 8, size: 16
    obj.asset_time = HakoPdu::HakoSrvMsgs::LogicalTime.new
    HakoPdu::HakoSrvMsgs::LogicalTimeConverter.binary_read_recursive_logical_time(meta, binary_data, obj.asset_time, base_off + 8)

    obj
  end

  def self.binary_write_recursive_notify_asset_time_request(parent_off, heap_data, allocator, obj)
    # member: session_id, type: uint64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint64', obj.session_id, 8))

    # member: asset_time, type: LogicalTime (struct), offset: 8, size: 16
    HakoPdu::HakoSrvMsgs::LogicalTimeConverter.binary_write_recursive_logical_time(parent_off + 8, heap_data, allocator, obj.asset_time)
