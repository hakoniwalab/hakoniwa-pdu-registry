defmodule HakoPdu.HakoSrvMsgs.RegisterClientRequestPacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_register_client_request_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.RegisterClientRequestPacket.new()
    binary_read_recursive_register_client_request_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_register_client_request_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_register_client_request_packet(0, "", "", obj)
  ...
  def binary_read_recursive_register_client_request_packet(meta, binary_data, obj, base_off) do
    ...
    # member: body, type: RegisterClientRequest (struct), offset: 268, size: 4
    body =
      HakoPdu.HakoSrvMsgs.RegisterClientRequestConverter.binary_read_recursive_register_client_request(
        meta,
        binary_data,
        HakoPdu.HakoSrvMsgs.RegisterClientRequest.new(),
        base_off + 268
      )
  ...
  def binary_write_recursive_register_client_request_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
    ...
    # member: body, type: RegisterClientRequest (struct), offset: 268, size: 4
    {allocator, heap_data} =
      HakoPdu.HakoSrvMsgs.RegisterClientRequestConverter.binary_write_recursive_register_client_request(
        parent_off + 268,
        heap_data,
        allocator,
        obj.body
