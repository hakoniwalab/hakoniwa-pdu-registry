require_relative 'pdu_type_QuaternionStamped'
...
module HakoPdu::GeometryMsgs::QuaternionStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_quaternion_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::QuaternionStamped.new
    binary_read_recursive_quaternion_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_quaternion_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_quaternion_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_quaternion_stamped(meta, binary_data, obj, base_off)
    ...
    # member: quaternion, type: Quaternion (struct), offset: 136, size: 32
    obj.quaternion = HakoPdu::GeometryMsgs::Quaternion.new
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_read_recursive_quaternion(meta, binary_data, obj.quaternion, base_off + 136)

    obj
  end

  def self.binary_write_recursive_quaternion_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: quaternion, type: Quaternion (struct), offset: 136, size: 32
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_write_recursive_quaternion(parent_off + 136, heap_data, allocator, obj.quaternion)
