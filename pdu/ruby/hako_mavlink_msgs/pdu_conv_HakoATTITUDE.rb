require_relative 'pdu_type_HakoATTITUDE'


module HakoPdu::HakoMavlinkMsgs::HakoATTITUDEConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_a_t_t_i_t_u_d_e(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMavlinkMsgs::HakoATTITUDE.new
    binary_read_recursive_hako_a_t_t_i_t_u_d_e(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_a_t_t_i_t_u_d_e(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_a_t_t_i_t_u_d_e(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_a_t_t_i_t_u_d_e(meta, binary_data, obj, base_off)
    ...
    obj.roll = read_primitive('float32', binary_data.byteslice(base_off + 4, 4))

    # member: pitch, type: float32 (primitive), offset: 8, size: 4
    obj.pitch = read_primitive('float32', binary_data.byteslice(base_off + 8, 4))

    # member: yaw, type: float32 (primitive), offset: 12, size: 4
    obj.yaw = read_primitive('float32', binary_data.byteslice(base_off + 12, 4))

    # member: rollspeed, type: float32 (primitive), offset: 16, size: 4
    obj.rollspeed = read_primitive('float32', binary_data.byteslice(base_off + 16, 4))

    # member: pitchspeed, type: float32 (primitive), offset: 20, size: 4
    obj.pitchspeed = read_primitive('float32', binary_data.byteslice(base_off + 20, 4))

    # member: yawspeed, type: float32 (primitive), offset: 24, size: 4
    obj.yawspeed = read_primitive('float32', binary_data.byteslice(base_off + 24, 4))

    obj
  end

  def self.binary_write_recursive_hako_a_t_t_i_t_u_d_e(parent_off, heap_data, allocator, obj)
    # member: time_boot_ms, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.time_boot_ms, 4))

    # member: roll, type: float32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('float32', obj.roll, 4))

    # member: pitch, type: float32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('float32', obj.pitch, 4))

    # member: yaw, type: float32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('float32', obj.yaw, 4))

    # member: rollspeed, type: float32 (primitive), offset: 16, size: 4
    add_binary(allocator, parent_off + 16, primitive_to_bin('float32', obj.rollspeed, 4))

    # member: pitchspeed, type: float32 (primitive), offset: 20, size: 4
    add_binary(allocator, parent_off + 20, primitive_to_bin('float32', obj.pitchspeed, 4))

    # member: yawspeed, type: float32 (primitive), offset: 24, size: 4
    add_binary(allocator, parent_off + 24, primitive_to_bin('float32', obj.yawspeed, 4))
