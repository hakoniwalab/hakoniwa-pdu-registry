defmodule HakoPdu.NavMsgs.OccupancyGridConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_occupancy_grid(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.NavMsgs.OccupancyGrid.new()
    binary_read_recursive_occupancy_grid(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_occupancy_grid(obj) do
    {base_data, heap_data} = binary_write_recursive_occupancy_grid(0, "", "", obj)
  ...
  def binary_read_recursive_occupancy_grid(meta, binary_data, obj, base_off) do
    ...
    info =
      HakoPdu.NavMsgs.MapMetaDataConverter.binary_read_recursive_map_meta_data(
        meta,
        binary_data,
        HakoPdu.NavMsgs.MapMetaData.new(),
        base_off + 136
      )

    # member: data, type: int8 (primitive), offset: 216, size: 1
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 216, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 216 + 4, 4))
    data = read_primitive_array("int8", binary_part(binary_data, meta.heap_off + offset_from_heap, 1 * array_size), array_size, 1)
  ...
  def binary_write_recursive_occupancy_grid(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: info, type: MapMetaData (struct), offset: 136, size: 80
    {allocator, heap_data} =
      HakoPdu.NavMsgs.MapMetaDataConverter.binary_write_recursive_map_meta_data(
        parent_off + 136,
        heap_data,
        allocator,
        obj.info
    ...
    values = obj.data || []
    allocator = add_binary(allocator, parent_off + 216, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    heap_data = add_binary(heap_data, offset_from_heap, primitive_array_to_bin("int8", values, 1))
