defmodule HakoPdu.DroneSrvMsgs.MagnetGrabResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_magnet_grab_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.MagnetGrabResponse.new()
    binary_read_recursive_magnet_grab_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_magnet_grab_response(obj) do
    {base_data, heap_data} = binary_write_recursive_magnet_grab_response(0, "", "", obj)
  ...
  def binary_read_recursive_magnet_grab_response(meta, binary_data, obj, base_off) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    ok = read_primitive("bool", binary_part(binary_data, base_off + 0, 4))

    # member: magnet_on, type: bool (primitive), offset: 4, size: 4
    magnet_on = read_primitive("bool", binary_part(binary_data, base_off + 4, 4))

    # member: contact_on, type: bool (primitive), offset: 8, size: 4
    contact_on = read_primitive("bool", binary_part(binary_data, base_off + 8, 4))

    # member: message, type: string (primitive), offset: 12, size: 128
    message = read_primitive("string", binary_part(binary_data, base_off + 12, 128))
  ...
  def binary_write_recursive_magnet_grab_response(parent_off, heap_data, allocator, obj) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.ok, 4))

    # member: magnet_on, type: bool (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("bool", obj.magnet_on, 4))

    # member: contact_on, type: bool (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("bool", obj.contact_on, 4))

    # member: message, type: string (primitive), offset: 12, size: 128
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("string", obj.message, 128))
