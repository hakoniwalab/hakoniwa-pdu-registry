defmodule HakoPdu.HakoMsgs.DisturbanceAtmConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_disturbance_atm(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DisturbanceAtm.new()
    binary_read_recursive_disturbance_atm(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_disturbance_atm(obj) do
    {base_data, heap_data} = binary_write_recursive_disturbance_atm(0, "", "", obj)
  ...
  def binary_read_recursive_disturbance_atm(meta, binary_data, obj, base_off) do
    # member: sea_level_atm, type: float64 (primitive), offset: 0, size: 8
    sea_level_atm = read_primitive("float64", binary_part(binary_data, base_off + 0, 8))
  ...
  def binary_write_recursive_disturbance_atm(parent_off, heap_data, allocator, obj) do
    # member: sea_level_atm, type: float64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float64", obj.sea_level_atm, 8))
