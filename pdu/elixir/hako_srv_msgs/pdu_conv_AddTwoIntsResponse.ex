defmodule HakoPdu.HakoSrvMsgs.AddTwoIntsResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_add_two_ints_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.AddTwoIntsResponse.new()
    binary_read_recursive_add_two_ints_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_add_two_ints_response(obj) do
    {base_data, heap_data} = binary_write_recursive_add_two_ints_response(0, "", "", obj)
  ...
  def binary_read_recursive_add_two_ints_response(meta, binary_data, obj, base_off) do
    # member: sum, type: int64 (primitive), offset: 0, size: 8
    sum = read_primitive("int64", binary_part(binary_data, base_off + 0, 8))
  ...
  def binary_write_recursive_add_two_ints_response(parent_off, heap_data, allocator, obj) do
    # member: sum, type: int64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int64", obj.sum, 8))
