defmodule HakoPdu.SensorMsgs.RegionOfInterestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_region_of_interest(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.RegionOfInterest.new()
    binary_read_recursive_region_of_interest(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_region_of_interest(obj) do
    {base_data, heap_data} = binary_write_recursive_region_of_interest(0, "", "", obj)
  ...
  def binary_read_recursive_region_of_interest(meta, binary_data, obj, base_off) do
    # member: x_offset, type: uint32 (primitive), offset: 0, size: 4
    x_offset = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: y_offset, type: uint32 (primitive), offset: 4, size: 4
    y_offset = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: height, type: uint32 (primitive), offset: 8, size: 4
    height = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))

    # member: width, type: uint32 (primitive), offset: 12, size: 4
    width = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))

    # member: do_rectify, type: bool (primitive), offset: 16, size: 4
    do_rectify = read_primitive("bool", binary_part(binary_data, base_off + 16, 4))
  ...
  def binary_write_recursive_region_of_interest(parent_off, heap_data, allocator, obj) do
    # member: x_offset, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.x_offset, 4))

    # member: y_offset, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.y_offset, 4))

    # member: height, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.height, 4))

    # member: width, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.width, 4))

    # member: do_rectify, type: bool (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("bool", obj.do_rectify, 4))
