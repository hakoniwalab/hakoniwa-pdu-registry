...
module HakoPdu::HakoMsgs::HakoDroneCmdHeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_drone_cmd_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoDroneCmdHeader.new
    binary_read_recursive_hako_drone_cmd_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_drone_cmd_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_drone_cmd_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_drone_cmd_header(meta, binary_data, obj, base_off)
    # member: request, type: bool (primitive), offset: 0, size: 4
    obj.request = read_primitive('bool', binary_data.byteslice(base_off + 0, 4))

    # member: result, type: bool (primitive), offset: 4, size: 4
    obj.result = read_primitive('bool', binary_data.byteslice(base_off + 4, 4))

    # member: result_code, type: int32 (primitive), offset: 8, size: 4
    obj.result_code = read_primitive('int32', binary_data.byteslice(base_off + 8, 4))

    obj
  end

  def self.binary_write_recursive_hako_drone_cmd_header(parent_off, heap_data, allocator, obj)
    # member: request, type: bool (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('bool', obj.request, 4))

    # member: result, type: bool (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('bool', obj.result, 4))

    # member: result_code, type: int32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('int32', obj.result_code, 4))
