defmodule HakoPdu.HakoMsgs.MonitorCameraCmdConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_monitor_camera_cmd(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.MonitorCameraCmd.new()
    binary_read_recursive_monitor_camera_cmd(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_monitor_camera_cmd(obj) do
    {base_data, heap_data} = binary_write_recursive_monitor_camera_cmd(0, "", "", obj)
  ...
  def binary_read_recursive_monitor_camera_cmd(meta, binary_data, obj, base_off) do
    # member: header, type: HakoCmdHeader (struct), offset: 0, size: 12
    header =
      HakoPdu.HakoMsgs.HakoCmdHeaderConverter.binary_read_recursive_hako_cmd_header(
        meta,
        binary_data,
        HakoPdu.HakoMsgs.HakoCmdHeader.new(),
        base_off + 0
      )

    # member: request_id, type: int32 (primitive), offset: 12, size: 4
    request_id = read_primitive("int32", binary_part(binary_data, base_off + 12, 4))

    # member: encode_type, type: int32 (primitive), offset: 16, size: 4
    encode_type = read_primitive("int32", binary_part(binary_data, base_off + 16, 4))

    # member: request_type, type: int32 (primitive), offset: 20, size: 4
    request_type = read_primitive("int32", binary_part(binary_data, base_off + 20, 4))
  ...
  def binary_write_recursive_monitor_camera_cmd(parent_off, heap_data, allocator, obj) do
    # member: header, type: HakoCmdHeader (struct), offset: 0, size: 12
    {allocator, heap_data} =
      HakoPdu.HakoMsgs.HakoCmdHeaderConverter.binary_write_recursive_hako_cmd_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: request_id, type: int32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("int32", obj.request_id, 4))

    # member: encode_type, type: int32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("int32", obj.encode_type, 4))

    # member: request_type, type: int32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("int32", obj.request_type, 4))
