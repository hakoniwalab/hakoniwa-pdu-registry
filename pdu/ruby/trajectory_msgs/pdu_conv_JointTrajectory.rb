require_relative 'pdu_type_JointTrajectory'
...
require_relative 'pdu_conv_JointTrajectoryPoint'


module HakoPdu::TrajectoryMsgs::JointTrajectoryConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_joint_trajectory(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::TrajectoryMsgs::JointTrajectory.new
    binary_read_recursive_joint_trajectory(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_joint_trajectory(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_joint_trajectory(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_joint_trajectory(meta, binary_data, obj, base_off)
    ...
    array_size.times do |i|
      child = HakoPdu::TrajectoryMsgs::JointTrajectoryPoint.new
      HakoPdu::TrajectoryMsgs::JointTrajectoryPointConverter.binary_read_recursive_joint_trajectory_point(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 40)
      obj.points << child
    end
  ...
  def self.binary_write_recursive_joint_trajectory(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    add_binary(heap_data, offset_from_heap, "\x00".b * (values.length * 40))
    values.each_with_index do |child, i|
      HakoPdu::TrajectoryMsgs::JointTrajectoryPointConverter.binary_write_recursive_joint_trajectory_point(offset_from_heap + i * 40, heap_data, heap_data, child)
    end
