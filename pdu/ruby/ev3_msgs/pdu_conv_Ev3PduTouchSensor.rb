...
module HakoPdu::Ev3Msgs::Ev3PduTouchSensorConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ev3_pdu_touch_sensor(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Ev3Msgs::Ev3PduTouchSensor.new
    binary_read_recursive_ev3_pdu_touch_sensor(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ev3_pdu_touch_sensor(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ev3_pdu_touch_sensor(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ev3_pdu_touch_sensor(meta, binary_data, obj, base_off)
    # member: value, type: uint32 (primitive), offset: 0, size: 4
    obj.value = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    obj
  end

  def self.binary_write_recursive_ev3_pdu_touch_sensor(parent_off, heap_data, allocator, obj)
    # member: value, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.value, 4))
