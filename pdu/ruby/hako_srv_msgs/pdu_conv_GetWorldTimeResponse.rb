...
module HakoPdu::HakoSrvMsgs::GetWorldTimeResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_get_world_time_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::GetWorldTimeResponse.new
    binary_read_recursive_get_world_time_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_get_world_time_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_get_world_time_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_get_world_time_response(meta, binary_data, obj, base_off)
    ...
    # member: world_time, type: LogicalTime (struct), offset: 8, size: 16
    obj.world_time = HakoPdu::HakoSrvMsgs::LogicalTime.new
    HakoPdu::HakoSrvMsgs::LogicalTimeConverter.binary_read_recursive_logical_time(meta, binary_data, obj.world_time, base_off + 8)

    obj
  end

  def self.binary_write_recursive_get_world_time_response(parent_off, heap_data, allocator, obj)
    # member: result_code, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.result_code, 4))

    # member: world_time, type: LogicalTime (struct), offset: 8, size: 16
    HakoPdu::HakoSrvMsgs::LogicalTimeConverter.binary_write_recursive_logical_time(parent_off + 8, heap_data, allocator, obj.world_time)
