require_relative 'pdu_type_PoseWithCovariance'
require_relative 'pdu_type_Pose'
require_relative 'pdu_conv_Pose'


module HakoPdu::GeometryMsgs::PoseWithCovarianceConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_pose_with_covariance(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::PoseWithCovariance.new
    binary_read_recursive_pose_with_covariance(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_pose_with_covariance(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_pose_with_covariance(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_pose_with_covariance(meta, binary_data, obj, base_off)
    # member: pose, type: Pose (struct), offset: 0, size: 56
    obj.pose = HakoPdu::GeometryMsgs::Pose.new
    HakoPdu::GeometryMsgs::PoseConverter.binary_read_recursive_pose(meta, binary_data, obj.pose, base_off + 0)

    # member: covariance, type: float64 (primitive), offset: 56, size: 288
    obj.covariance = read_primitive_array('float64', binary_data.byteslice(base_off + 56, 288), 36, 288 / 36)

    obj
  end

  def self.binary_write_recursive_pose_with_covariance(parent_off, heap_data, allocator, obj)
    # member: pose, type: Pose (struct), offset: 0, size: 56
    HakoPdu::GeometryMsgs::PoseConverter.binary_write_recursive_pose(parent_off + 0, heap_data, allocator, obj.pose)

    # member: covariance, type: float64 (primitive), offset: 56, size: 288
    add_binary(allocator, parent_off + 56, primitive_array_to_bin('float64', obj.covariance, 288 / 36))
