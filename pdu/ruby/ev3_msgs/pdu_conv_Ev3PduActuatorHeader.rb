...
module HakoPdu::Ev3Msgs::Ev3PduActuatorHeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ev3_pdu_actuator_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Ev3Msgs::Ev3PduActuatorHeader.new
    binary_read_recursive_ev3_pdu_actuator_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ev3_pdu_actuator_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ev3_pdu_actuator_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ev3_pdu_actuator_header(meta, binary_data, obj, base_off)
    # member: name, type: string (primitive), offset: 0, size: 128
    obj.name = read_primitive('string', binary_data.byteslice(base_off + 0, 128))

    # member: version, type: uint32 (primitive), offset: 128, size: 4
    obj.version = read_primitive('uint32', binary_data.byteslice(base_off + 128, 4))

    # member: asset_time, type: int64 (primitive), offset: 136, size: 8
    obj.asset_time = read_primitive('int64', binary_data.byteslice(base_off + 136, 8))

    # member: ext_off, type: uint32 (primitive), offset: 144, size: 4
    obj.ext_off = read_primitive('uint32', binary_data.byteslice(base_off + 144, 4))

    # member: ext_size, type: uint32 (primitive), offset: 148, size: 4
    obj.ext_size = read_primitive('uint32', binary_data.byteslice(base_off + 148, 4))

    obj
  end

  def self.binary_write_recursive_ev3_pdu_actuator_header(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.name, 128))

    # member: version, type: uint32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint32', obj.version, 4))

    # member: asset_time, type: int64 (primitive), offset: 136, size: 8
    add_binary(allocator, parent_off + 136, primitive_to_bin('int64', obj.asset_time, 8))

    # member: ext_off, type: uint32 (primitive), offset: 144, size: 4
    add_binary(allocator, parent_off + 144, primitive_to_bin('uint32', obj.ext_off, 4))

    # member: ext_size, type: uint32 (primitive), offset: 148, size: 4
    add_binary(allocator, parent_off + 148, primitive_to_bin('uint32', obj.ext_size, 4))
