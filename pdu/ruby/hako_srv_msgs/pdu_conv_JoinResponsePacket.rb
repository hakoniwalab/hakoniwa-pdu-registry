require_relative 'pdu_type_JoinResponsePacket'
require_relative 'pdu_type_ServiceResponseHeader'
require_relative 'pdu_conv_ServiceResponseHeader'
require_relative 'pdu_type_JoinResponse'
require_relative 'pdu_conv_JoinResponse'


module HakoPdu::HakoSrvMsgs::JoinResponsePacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_join_response_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::JoinResponsePacket.new
    binary_read_recursive_join_response_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_join_response_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_join_response_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_join_response_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: JoinResponse (struct), offset: 268, size: 132
    obj.body = HakoPdu::HakoSrvMsgs::JoinResponse.new
    HakoPdu::HakoSrvMsgs::JoinResponseConverter.binary_read_recursive_join_response(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_join_response_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceResponseHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: JoinResponse (struct), offset: 268, size: 132
    HakoPdu::HakoSrvMsgs::JoinResponseConverter.binary_write_recursive_join_response(parent_off + 268, heap_data, allocator, obj.body)
