defmodule HakoPdu.SensorMsgs.FluidPressureConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_fluid_pressure(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.FluidPressure.new()
    binary_read_recursive_fluid_pressure(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_fluid_pressure(obj) do
    {base_data, heap_data} = binary_write_recursive_fluid_pressure(0, "", "", obj)
  ...
  def binary_read_recursive_fluid_pressure(meta, binary_data, obj, base_off) do
    ...
    fluid_pressure = read_primitive("float64", binary_part(binary_data, base_off + 136, 8))
  ...
  def binary_write_recursive_fluid_pressure(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: fluid_pressure, type: float64 (primitive), offset: 136, size: 8
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("float64", obj.fluid_pressure, 8))
