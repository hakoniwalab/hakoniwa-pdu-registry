defmodule HakoPdu.Ev3Msgs.Ev3PduMotorConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ev3_pdu_motor(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Ev3Msgs.Ev3PduMotor.new()
    binary_read_recursive_ev3_pdu_motor(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ev3_pdu_motor(obj) do
    {base_data, heap_data} = binary_write_recursive_ev3_pdu_motor(0, "", "", obj)
  ...
  def binary_read_recursive_ev3_pdu_motor(meta, binary_data, obj, base_off) do
    # member: power, type: int32 (primitive), offset: 0, size: 4
    power = read_primitive("int32", binary_part(binary_data, base_off + 0, 4))

    # member: stop, type: uint32 (primitive), offset: 4, size: 4
    stop = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: reset_angle, type: uint32 (primitive), offset: 8, size: 4
    reset_angle = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))
  ...
  def binary_write_recursive_ev3_pdu_motor(parent_off, heap_data, allocator, obj) do
    # member: power, type: int32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int32", obj.power, 4))

    # member: stop, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.stop, 4))

    # member: reset_angle, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.reset_angle, 4))
