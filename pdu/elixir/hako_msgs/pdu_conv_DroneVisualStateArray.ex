defmodule HakoPdu.HakoMsgs.DroneVisualStateArrayConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_visual_state_array(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DroneVisualStateArray.new()
    binary_read_recursive_drone_visual_state_array(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_visual_state_array(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_visual_state_array(0, "", "", obj)
  ...
  def binary_read_recursive_drone_visual_state_array(meta, binary_data, obj, base_off) do
    # member: sequence_id, type: uint32 (primitive), offset: 0, size: 4
    sequence_id = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: chunk_index, type: uint32 (primitive), offset: 4, size: 4
    chunk_index = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: chunk_count, type: uint32 (primitive), offset: 8, size: 4
    chunk_count = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))

    # member: start_index, type: uint32 (primitive), offset: 12, size: 4
    start_index = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))

    # member: valid_count, type: uint32 (primitive), offset: 16, size: 4
    valid_count = read_primitive("uint32", binary_part(binary_data, base_off + 16, 4))

    # member: drones, type: hako_msgs/DroneVisualState (struct), offset: 20, size: 32
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 20, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 20 + 4, 4))
    drones =
      range_indices(array_size)
      |> Enum.map(fn i ->
        HakoPdu.HakoMsgs.DroneVisualStateConverter.binary_read_recursive_drone_visual_state(
          meta,
          binary_data,
          HakoPdu.HakoMsgs.DroneVisualState.new(),
          meta.heap_off + offset_from_heap + i * 32
        )
      end)
  ...
  def binary_write_recursive_drone_visual_state_array(parent_off, heap_data, allocator, obj) do
    # member: sequence_id, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.sequence_id, 4))

    # member: chunk_index, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.chunk_index, 4))

    # member: chunk_count, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.chunk_count, 4))

    # member: start_index, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.start_index, 4))

    # member: valid_count, type: uint32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("uint32", obj.valid_count, 4))

    # member: drones, type: hako_msgs/DroneVisualState (struct), offset: 20, size: 32
    offset_from_heap = byte_size(heap_data)
    values = obj.drones || []
    allocator = add_binary(allocator, parent_off + 20, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 32))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.HakoMsgs.DroneVisualStateConverter.binary_write_recursive_drone_visual_state(
