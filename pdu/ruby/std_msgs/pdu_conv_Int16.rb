require_relative 'pdu_type_Int16'


module HakoPdu::StdMsgs::Int16Converter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_int16(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Int16.new
    binary_read_recursive_int16(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_int16(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_int16(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_int16(meta, binary_data, obj, base_off)
    # member: data, type: int16 (primitive), offset: 0, size: 2
    obj.data = read_primitive('int16', binary_data.byteslice(base_off + 0, 2))

    obj
  end

  def self.binary_write_recursive_int16(parent_off, heap_data, allocator, obj)
    # member: data, type: int16 (primitive), offset: 0, size: 2
    add_binary(allocator, parent_off + 0, primitive_to_bin('int16', obj.data, 2))
