...
import { binary_read_recursive_JointTrajectoryPoint, binary_write_recursive_JointTrajectoryPoint } from './pdu_conv_JointTrajectoryPoint.js';


/**
 * Deserializes a binary PDU into a JointTrajectory object.
 * @param {ArrayBuffer} binary_data
 * @returns { JointTrajectory }
 */
export function pduToJs_JointTrajectory(binary_data) {
    const js_obj = new JointTrajectory();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_JointTrajectory(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_JointTrajectory(meta, binary_data, js_obj, base_off) {
...
/**
 * Serializes a JointTrajectory object into a binary PDU.
 * @param { JointTrajectory } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_JointTrajectory(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_JointTrajectory(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_JointTrajectory(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: header, type: std_msgs/Header (struct)
    ...
    { // varray
        ...
        for (let item_index = 0; item_index < array_size; item_index++) {
            ...
            binary_write_recursive_JointTrajectoryPoint(item_offset, bw_container, bw_container.heap_allocator, js_obj.points[item_index]);
        }
