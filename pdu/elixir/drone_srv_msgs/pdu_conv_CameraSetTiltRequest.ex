defmodule HakoPdu.DroneSrvMsgs.CameraSetTiltRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_camera_set_tilt_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.CameraSetTiltRequest.new()
    binary_read_recursive_camera_set_tilt_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_camera_set_tilt_request(obj) do
    {base_data, heap_data} = binary_write_recursive_camera_set_tilt_request(0, "", "", obj)
  ...
  def binary_read_recursive_camera_set_tilt_request(meta, binary_data, obj, base_off) do
    ...
    # member: tilt_angle_deg, type: float32 (primitive), offset: 128, size: 4
    tilt_angle_deg = read_primitive("float32", binary_part(binary_data, base_off + 128, 4))
  ...
  def binary_write_recursive_camera_set_tilt_request(parent_off, heap_data, allocator, obj) do
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.drone_name, 128))

    # member: tilt_angle_deg, type: float32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("float32", obj.tilt_angle_deg, 4))
