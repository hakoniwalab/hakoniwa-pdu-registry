defmodule HakoPdu.NavMsgs.MapMetaDataConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_map_meta_data(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.NavMsgs.MapMetaData.new()
    binary_read_recursive_map_meta_data(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_map_meta_data(obj) do
    {base_data, heap_data} = binary_write_recursive_map_meta_data(0, "", "", obj)
  ...
  def binary_read_recursive_map_meta_data(meta, binary_data, obj, base_off) do
    # member: map_load_time, type: builtin_interfaces/Time (struct), offset: 0, size: 8
    map_load_time =
      HakoPdu.BuiltinInterfaces.TimeConverter.binary_read_recursive_time(
        meta,
        binary_data,
        HakoPdu.BuiltinInterfaces.Time.new(),
        base_off + 0
      )

    # member: resolution, type: float32 (primitive), offset: 8, size: 4
    resolution = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: width, type: uint32 (primitive), offset: 12, size: 4
    width = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))

    # member: height, type: uint32 (primitive), offset: 16, size: 4
    height = read_primitive("uint32", binary_part(binary_data, base_off + 16, 4))
  ...
  def binary_write_recursive_map_meta_data(parent_off, heap_data, allocator, obj) do
    # member: map_load_time, type: builtin_interfaces/Time (struct), offset: 0, size: 8
    {allocator, heap_data} =
      HakoPdu.BuiltinInterfaces.TimeConverter.binary_write_recursive_time(
        parent_off + 0,
        heap_data,
        allocator,
        obj.map_load_time
    ...
    # member: resolution, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.resolution, 4))

    # member: width, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.width, 4))

    # member: height, type: uint32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("uint32", obj.height, 4))
