defmodule HakoPdu.HakoSrvMsgs.AckEventRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ack_event_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.AckEventRequest.new()
    binary_read_recursive_ack_event_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ack_event_request(obj) do
    {base_data, heap_data} = binary_write_recursive_ack_event_request(0, "", "", obj)
  ...
  def binary_read_recursive_ack_event_request(meta, binary_data, obj, base_off) do
    # member: name, type: string (primitive), offset: 0, size: 128
    name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: event_code, type: uint32 (primitive), offset: 128, size: 4
    event_code = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: result_code, type: uint32 (primitive), offset: 132, size: 4
    result_code = read_primitive("uint32", binary_part(binary_data, base_off + 132, 4))
  ...
  def binary_write_recursive_ack_event_request(parent_off, heap_data, allocator, obj) do
    # member: name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.name, 128))

    # member: event_code, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.event_code, 4))

    # member: result_code, type: uint32 (primitive), offset: 132, size: 4
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("uint32", obj.result_code, 4))
