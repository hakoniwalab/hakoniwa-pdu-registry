require_relative 'pdu_type_HakoCameraData'
require_relative '../sensor_msgs/pdu_type_CompressedImage'
require_relative '../sensor_msgs/pdu_conv_CompressedImage'


module HakoPdu::HakoMsgs::HakoCameraDataConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_camera_data(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoCameraData.new
    binary_read_recursive_hako_camera_data(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_camera_data(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_camera_data(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_camera_data(meta, binary_data, obj, base_off)
    ...
    HakoPdu::SensorMsgs::CompressedImageConverter.binary_read_recursive_compressed_image(meta, binary_data, obj.image, base_off + 4)

    obj
  end

  def self.binary_write_recursive_hako_camera_data(parent_off, heap_data, allocator, obj)
    # member: request_id, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.request_id, 4))

    # member: image, type: sensor_msgs/CompressedImage (struct), offset: 4, size: 272
    HakoPdu::SensorMsgs::CompressedImageConverter.binary_write_recursive_compressed_image(parent_off + 4, heap_data, allocator, obj.image)
