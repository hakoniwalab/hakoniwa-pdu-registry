defmodule HakoPdu.HakoMsgs.HakoBatteryStatusConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_battery_status(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.HakoBatteryStatus.new()
    binary_read_recursive_hako_battery_status(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_battery_status(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_battery_status(0, "", "", obj)
  ...
  def binary_read_recursive_hako_battery_status(meta, binary_data, obj, base_off) do
    # member: full_voltage, type: float64 (primitive), offset: 0, size: 8
    full_voltage = read_primitive("float64", binary_part(binary_data, base_off + 0, 8))

    # member: curr_voltage, type: float64 (primitive), offset: 8, size: 8
    curr_voltage = read_primitive("float64", binary_part(binary_data, base_off + 8, 8))

    # member: curr_temp, type: float64 (primitive), offset: 16, size: 8
    curr_temp = read_primitive("float64", binary_part(binary_data, base_off + 16, 8))

    # member: status, type: uint32 (primitive), offset: 24, size: 4
    status = read_primitive("uint32", binary_part(binary_data, base_off + 24, 4))

    # member: cycles, type: uint32 (primitive), offset: 28, size: 4
    cycles = read_primitive("uint32", binary_part(binary_data, base_off + 28, 4))
  ...
  def binary_write_recursive_hako_battery_status(parent_off, heap_data, allocator, obj) do
    # member: full_voltage, type: float64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float64", obj.full_voltage, 8))

    # member: curr_voltage, type: float64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float64", obj.curr_voltage, 8))

    # member: curr_temp, type: float64 (primitive), offset: 16, size: 8
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float64", obj.curr_temp, 8))

    # member: status, type: uint32 (primitive), offset: 24, size: 4
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("uint32", obj.status, 4))

    # member: cycles, type: uint32 (primitive), offset: 28, size: 4
    allocator = add_binary(allocator, parent_off + 28, primitive_to_bin("uint32", obj.cycles, 4))
