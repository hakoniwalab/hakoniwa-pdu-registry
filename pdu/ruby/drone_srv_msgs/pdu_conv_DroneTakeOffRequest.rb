...
module HakoPdu::DroneSrvMsgs::DroneTakeOffRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_take_off_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneTakeOffRequest.new
    binary_read_recursive_drone_take_off_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_take_off_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_take_off_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_take_off_request(meta, binary_data, obj, base_off)
    # member: alt_m, type: float64 (primitive), offset: 0, size: 8
    obj.alt_m = read_primitive('float64', binary_data.byteslice(base_off + 0, 8))

    # member: drone_name, type: string (primitive), offset: 8, size: 128
    obj.drone_name = read_primitive('string', binary_data.byteslice(base_off + 8, 128))

    obj
  end

  def self.binary_write_recursive_drone_take_off_request(parent_off, heap_data, allocator, obj)
    # member: alt_m, type: float64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('float64', obj.alt_m, 8))

    # member: drone_name, type: string (primitive), offset: 8, size: 128
    add_binary(allocator, parent_off + 8, primitive_to_bin('string', obj.drone_name, 128))
