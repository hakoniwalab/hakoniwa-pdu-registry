defmodule HakoPdu.HakoMsgs.DisturbanceUserCustomConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_disturbance_user_custom(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DisturbanceUserCustom.new()
    binary_read_recursive_disturbance_user_custom(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_disturbance_user_custom(obj) do
    {base_data, heap_data} = binary_write_recursive_disturbance_user_custom(0, "", "", obj)
  ...
  def binary_read_recursive_disturbance_user_custom(meta, binary_data, obj, base_off) do
    # member: data, type: float64 (primitive), offset: 0, size: 8
  ...
  def binary_write_recursive_disturbance_user_custom(parent_off, heap_data, allocator, obj) do
    # member: data, type: float64 (primitive), offset: 0, size: 8
