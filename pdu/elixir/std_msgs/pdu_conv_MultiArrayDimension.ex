defmodule HakoPdu.StdMsgs.MultiArrayDimensionConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_multi_array_dimension(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.MultiArrayDimension.new()
    binary_read_recursive_multi_array_dimension(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_multi_array_dimension(obj) do
    {base_data, heap_data} = binary_write_recursive_multi_array_dimension(0, "", "", obj)
  ...
  def binary_read_recursive_multi_array_dimension(meta, binary_data, obj, base_off) do
    # member: label, type: string (primitive), offset: 0, size: 128
    label = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: size, type: uint32 (primitive), offset: 128, size: 4
    size = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: stride, type: uint32 (primitive), offset: 132, size: 4
    stride = read_primitive("uint32", binary_part(binary_data, base_off + 132, 4))
  ...
  def binary_write_recursive_multi_array_dimension(parent_off, heap_data, allocator, obj) do
    # member: label, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.label, 128))

    # member: size, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.size, 4))

    # member: stride, type: uint32 (primitive), offset: 132, size: 4
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("uint32", obj.stride, 4))
