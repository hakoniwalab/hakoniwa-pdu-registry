require_relative 'pdu_type_HakoHeartbeat'


module HakoPdu::HakoMavlinkMsgs::HakoHeartbeatConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_heartbeat(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMavlinkMsgs::HakoHeartbeat.new
    binary_read_recursive_hako_heartbeat(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_heartbeat(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_heartbeat(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_heartbeat(meta, binary_data, obj, base_off)
    # member: type, type: uint8 (primitive), offset: 0, size: 1
    obj.type = read_primitive('uint8', binary_data.byteslice(base_off + 0, 1))

    # member: autopilot, type: uint8 (primitive), offset: 1, size: 1
    obj.autopilot = read_primitive('uint8', binary_data.byteslice(base_off + 1, 1))

    # member: base_mode, type: uint8 (primitive), offset: 2, size: 1
    obj.base_mode = read_primitive('uint8', binary_data.byteslice(base_off + 2, 1))

    # member: custom_mode, type: uint32 (primitive), offset: 4, size: 4
    obj.custom_mode = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: system_status, type: uint8 (primitive), offset: 8, size: 1
    obj.system_status = read_primitive('uint8', binary_data.byteslice(base_off + 8, 1))

    # member: mavlink_version, type: uint8 (primitive), offset: 9, size: 1
    obj.mavlink_version = read_primitive('uint8', binary_data.byteslice(base_off + 9, 1))

    obj
  end

  def self.binary_write_recursive_hako_heartbeat(parent_off, heap_data, allocator, obj)
    # member: type, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.type, 1))

    # member: autopilot, type: uint8 (primitive), offset: 1, size: 1
    add_binary(allocator, parent_off + 1, primitive_to_bin('uint8', obj.autopilot, 1))

    # member: base_mode, type: uint8 (primitive), offset: 2, size: 1
    add_binary(allocator, parent_off + 2, primitive_to_bin('uint8', obj.base_mode, 1))

    # member: custom_mode, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.custom_mode, 4))

    # member: system_status, type: uint8 (primitive), offset: 8, size: 1
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint8', obj.system_status, 1))

    # member: mavlink_version, type: uint8 (primitive), offset: 9, size: 1
    add_binary(allocator, parent_off + 9, primitive_to_bin('uint8', obj.mavlink_version, 1))
