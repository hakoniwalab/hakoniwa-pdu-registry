defmodule HakoPdu.SensorMsgs.MultiEchoLaserScanConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_multi_echo_laser_scan(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.MultiEchoLaserScan.new()
    binary_read_recursive_multi_echo_laser_scan(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_multi_echo_laser_scan(obj) do
    {base_data, heap_data} = binary_write_recursive_multi_echo_laser_scan(0, "", "", obj)
  ...
  def binary_read_recursive_multi_echo_laser_scan(meta, binary_data, obj, base_off) do
    ...
    ranges =
      ...
      |> Enum.map(fn i ->
        HakoPdu.SensorMsgs.LaserEchoConverter.binary_read_recursive_laser_echo(
          meta,
          binary_data,
          HakoPdu.SensorMsgs.LaserEcho.new(),
  ...
  def binary_write_recursive_multi_echo_laser_scan(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    heap_data =
      ...
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.SensorMsgs.LaserEchoConverter.binary_write_recursive_laser_echo(
