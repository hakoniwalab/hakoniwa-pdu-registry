require_relative 'pdu_type_ActionRequestHeader'


module HakoPdu::HakoActionMsgs::ActionRequestHeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_action_request_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoActionMsgs::ActionRequestHeader.new
    binary_read_recursive_action_request_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_action_request_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_action_request_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_action_request_header(meta, binary_data, obj, base_off)
    ...
    obj.request_kind = read_primitive('uint8', binary_data.byteslice(base_off + 1, 1))

    # member: reserved, type: uint8 (primitive), offset: 2, size: 2
    obj.reserved = read_primitive_array('uint8', binary_data.byteslice(base_off + 2, 2), 2, 2 / 2)
  ...
  def self.binary_write_recursive_action_request_header(parent_off, heap_data, allocator, obj)
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.version, 1))

    # member: request_kind, type: uint8 (primitive), offset: 1, size: 1
    add_binary(allocator, parent_off + 1, primitive_to_bin('uint8', obj.request_kind, 1))

    # member: reserved, type: uint8 (primitive), offset: 2, size: 2
    add_binary(allocator, parent_off + 2, primitive_array_to_bin('uint8', obj.reserved, 2 / 2))
