require_relative 'pdu_type_PointStamped'
...
module HakoPdu::GeometryMsgs::PointStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_point_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::PointStamped.new
    binary_read_recursive_point_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_point_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_point_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_point_stamped(meta, binary_data, obj, base_off)
    ...
    # member: point, type: Point (struct), offset: 136, size: 24
    obj.point = HakoPdu::GeometryMsgs::Point.new
    HakoPdu::GeometryMsgs::PointConverter.binary_read_recursive_point(meta, binary_data, obj.point, base_off + 136)

    obj
  end

  def self.binary_write_recursive_point_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: point, type: Point (struct), offset: 136, size: 24
    HakoPdu::GeometryMsgs::PointConverter.binary_write_recursive_point(parent_off + 136, heap_data, allocator, obj.point)
