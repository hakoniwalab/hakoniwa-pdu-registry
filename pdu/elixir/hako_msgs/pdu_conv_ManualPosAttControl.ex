defmodule HakoPdu.HakoMsgs.ManualPosAttControlConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_manual_pos_att_control(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.ManualPosAttControl.new()
    binary_read_recursive_manual_pos_att_control(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_manual_pos_att_control(obj) do
    {base_data, heap_data} = binary_write_recursive_manual_pos_att_control(0, "", "", obj)
  ...
  def binary_read_recursive_manual_pos_att_control(meta, binary_data, obj, base_off) do
    # member: do_operation, type: bool (primitive), offset: 0, size: 4
    do_operation = read_primitive("bool", binary_part(binary_data, base_off + 0, 4))
  ...
  def binary_write_recursive_manual_pos_att_control(parent_off, heap_data, allocator, obj) do
    # member: do_operation, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.do_operation, 4))
