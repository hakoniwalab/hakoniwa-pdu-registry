defmodule HakoPdu.HakoMavlinkMsgs.HakoGLOBAL_POSITION_INTConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMavlinkMsgs.HakoGLOBAL_POSITION_INT.new()
    binary_read_recursive_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(0, "", "", obj)
  ...
  def binary_read_recursive_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(meta, binary_data, obj, base_off) do
    ...
    lat = read_primitive("int32", binary_part(binary_data, base_off + 4, 4))

    # member: lon, type: int32 (primitive), offset: 8, size: 4
    lon = read_primitive("int32", binary_part(binary_data, base_off + 8, 4))

    # member: alt, type: int32 (primitive), offset: 12, size: 4
    alt = read_primitive("int32", binary_part(binary_data, base_off + 12, 4))

    # member: relative_alt, type: int32 (primitive), offset: 16, size: 4
    relative_alt = read_primitive("int32", binary_part(binary_data, base_off + 16, 4))

    # member: vx, type: int16 (primitive), offset: 20, size: 2
    vx = read_primitive("int16", binary_part(binary_data, base_off + 20, 2))

    # member: vy, type: int16 (primitive), offset: 22, size: 2
    vy = read_primitive("int16", binary_part(binary_data, base_off + 22, 2))

    # member: vz, type: int16 (primitive), offset: 24, size: 2
    vz = read_primitive("int16", binary_part(binary_data, base_off + 24, 2))

    # member: hdg, type: uint16 (primitive), offset: 26, size: 2
    hdg = read_primitive("uint16", binary_part(binary_data, base_off + 26, 2))
  ...
  def binary_write_recursive_hako_g_l_o_b_a_l__p_o_s_i_t_i_o_n__i_n_t(parent_off, heap_data, allocator, obj) do
    # member: time_boot_ms, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.time_boot_ms, 4))

    # member: lat, type: int32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("int32", obj.lat, 4))

    # member: lon, type: int32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("int32", obj.lon, 4))

    # member: alt, type: int32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("int32", obj.alt, 4))

    # member: relative_alt, type: int32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("int32", obj.relative_alt, 4))

    # member: vx, type: int16 (primitive), offset: 20, size: 2
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("int16", obj.vx, 2))

    # member: vy, type: int16 (primitive), offset: 22, size: 2
    allocator = add_binary(allocator, parent_off + 22, primitive_to_bin("int16", obj.vy, 2))

    # member: vz, type: int16 (primitive), offset: 24, size: 2
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("int16", obj.vz, 2))

    # member: hdg, type: uint16 (primitive), offset: 26, size: 2
    allocator = add_binary(allocator, parent_off + 26, primitive_to_bin("uint16", obj.hdg, 2))
