require_relative 'pdu_type_DroneSetReadyRequestPacket'
require_relative '../hako_srv_msgs/pdu_type_ServiceRequestHeader'
require_relative '../hako_srv_msgs/pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_DroneSetReadyRequest'
require_relative 'pdu_conv_DroneSetReadyRequest'


module HakoPdu::DroneSrvMsgs::DroneSetReadyRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_set_ready_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneSetReadyRequestPacket.new
    binary_read_recursive_drone_set_ready_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_set_ready_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_set_ready_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_set_ready_request_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: DroneSetReadyRequest (struct), offset: 268, size: 128
    obj.body = HakoPdu::DroneSrvMsgs::DroneSetReadyRequest.new
    HakoPdu::DroneSrvMsgs::DroneSetReadyRequestConverter.binary_read_recursive_drone_set_ready_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_drone_set_ready_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DroneSetReadyRequest (struct), offset: 268, size: 128
    HakoPdu::DroneSrvMsgs::DroneSetReadyRequestConverter.binary_write_recursive_drone_set_ready_request(parent_off + 268, heap_data, allocator, obj.body)
