defmodule HakoPdu.StdMsgs.HeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.Header.new()
    binary_read_recursive_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_header(obj) do
    {base_data, heap_data} = binary_write_recursive_header(0, "", "", obj)
  ...
  def binary_read_recursive_header(meta, binary_data, obj, base_off) do
    ...
    # member: frame_id, type: string (primitive), offset: 8, size: 128
    frame_id = read_primitive("string", binary_part(binary_data, base_off + 8, 128))
  ...
  def binary_write_recursive_header(parent_off, heap_data, allocator, obj) do
    # member: stamp, type: builtin_interfaces/Time (struct), offset: 0, size: 8
    {allocator, heap_data} =
    ...
    # member: frame_id, type: string (primitive), offset: 8, size: 128
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("string", obj.frame_id, 128))
