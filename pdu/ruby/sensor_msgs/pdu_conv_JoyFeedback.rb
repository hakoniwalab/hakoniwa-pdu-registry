require_relative 'pdu_type_JoyFeedback'


module HakoPdu::SensorMsgs::JoyFeedbackConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_joy_feedback(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::JoyFeedback.new
    binary_read_recursive_joy_feedback(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_joy_feedback(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_joy_feedback(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_joy_feedback(meta, binary_data, obj, base_off)
    # member: type, type: uint8 (primitive), offset: 0, size: 1
    obj.type = read_primitive('uint8', binary_data.byteslice(base_off + 0, 1))

    # member: id, type: uint8 (primitive), offset: 1, size: 1
    obj.id = read_primitive('uint8', binary_data.byteslice(base_off + 1, 1))

    # member: intensity, type: float32 (primitive), offset: 4, size: 4
    obj.intensity = read_primitive('float32', binary_data.byteslice(base_off + 4, 4))

    obj
  end

  def self.binary_write_recursive_joy_feedback(parent_off, heap_data, allocator, obj)
    # member: type, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.type, 1))

    # member: id, type: uint8 (primitive), offset: 1, size: 1
    add_binary(allocator, parent_off + 1, primitive_to_bin('uint8', obj.id, 1))

    # member: intensity, type: float32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('float32', obj.intensity, 4))
