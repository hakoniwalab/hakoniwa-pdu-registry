...
/**
 * Deserializes a binary PDU into a ExecutionUnitRuntimeEpoch object.
 * @param {ArrayBuffer} binary_data
 * @returns { ExecutionUnitRuntimeEpoch }
 */
export function pduToJs_ExecutionUnitRuntimeEpoch(binary_data) {
    const js_obj = new ExecutionUnitRuntimeEpoch();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_ExecutionUnitRuntimeEpoch(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_ExecutionUnitRuntimeEpoch(meta, binary_data, js_obj, base_off) {
...
/**
 * Serializes a ExecutionUnitRuntimeEpoch object into a binary PDU.
 * @param { ExecutionUnitRuntimeEpoch } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_ExecutionUnitRuntimeEpoch(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_ExecutionUnitRuntimeEpoch(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_ExecutionUnitRuntimeEpoch(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: epoch, type: uint8 (primitive)
