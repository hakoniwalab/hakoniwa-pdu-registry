defmodule HakoPdu.SensorMsgs.RangeConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_range(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.Range.new()
    binary_read_recursive_range(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_range(obj) do
    {base_data, heap_data} = binary_write_recursive_range(0, "", "", obj)
  ...
  def binary_read_recursive_range(meta, binary_data, obj, base_off) do
    ...
    radiation_type = read_primitive("uint8", binary_part(binary_data, base_off + 136, 1))

    # member: field_of_view, type: float32 (primitive), offset: 140, size: 4
    field_of_view = read_primitive("float32", binary_part(binary_data, base_off + 140, 4))

    # member: min_range, type: float32 (primitive), offset: 144, size: 4
    min_range = read_primitive("float32", binary_part(binary_data, base_off + 144, 4))

    # member: max_range, type: float32 (primitive), offset: 148, size: 4
    max_range = read_primitive("float32", binary_part(binary_data, base_off + 148, 4))

    # member: range, type: float32 (primitive), offset: 152, size: 4
    range = read_primitive("float32", binary_part(binary_data, base_off + 152, 4))

    # member: variance, type: float32 (primitive), offset: 156, size: 4
    variance = read_primitive("float32", binary_part(binary_data, base_off + 156, 4))
  ...
  def binary_write_recursive_range(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: radiation_type, type: uint8 (primitive), offset: 136, size: 1
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("uint8", obj.radiation_type, 1))

    # member: field_of_view, type: float32 (primitive), offset: 140, size: 4
    allocator = add_binary(allocator, parent_off + 140, primitive_to_bin("float32", obj.field_of_view, 4))

    # member: min_range, type: float32 (primitive), offset: 144, size: 4
    allocator = add_binary(allocator, parent_off + 144, primitive_to_bin("float32", obj.min_range, 4))

    # member: max_range, type: float32 (primitive), offset: 148, size: 4
    allocator = add_binary(allocator, parent_off + 148, primitive_to_bin("float32", obj.max_range, 4))

    # member: range, type: float32 (primitive), offset: 152, size: 4
    allocator = add_binary(allocator, parent_off + 152, primitive_to_bin("float32", obj.range, 4))

    # member: variance, type: float32 (primitive), offset: 156, size: 4
    allocator = add_binary(allocator, parent_off + 156, primitive_to_bin("float32", obj.variance, 4))
