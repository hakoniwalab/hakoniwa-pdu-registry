defmodule HakoPdu.HakoMsgs.ExecutionUnitRuntimeContextConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_execution_unit_runtime_context(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.ExecutionUnitRuntimeContext.new()
    binary_read_recursive_execution_unit_runtime_context(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_execution_unit_runtime_context(obj) do
    {base_data, heap_data} = binary_write_recursive_execution_unit_runtime_context(0, "", "", obj)
  ...
  def binary_read_recursive_execution_unit_runtime_context(meta, binary_data, obj, base_off) do
    ...
    # member: epoch, type: uint8 (primitive), offset: 4, size: 1
    epoch = read_primitive("uint8", binary_part(binary_data, base_off + 4, 1))

    # member: owner_id, type: uint8 (primitive), offset: 5, size: 1
    owner_id = read_primitive("uint8", binary_part(binary_data, base_off + 5, 1))

    # member: context, type: uint8 (primitive), offset: 8, size: 1
    ...
    context = read_primitive_array("uint8", binary_part(binary_data, meta.heap_off + offset_from_heap, 1 * array_size), array_size, 1)
  ...
  def binary_write_recursive_execution_unit_runtime_context(parent_off, heap_data, allocator, obj) do
    # member: config_hash, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.config_hash, 4))

    # member: epoch, type: uint8 (primitive), offset: 4, size: 1
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint8", obj.epoch, 1))

    # member: owner_id, type: uint8 (primitive), offset: 5, size: 1
    allocator = add_binary(allocator, parent_off + 5, primitive_to_bin("uint8", obj.owner_id, 1))
