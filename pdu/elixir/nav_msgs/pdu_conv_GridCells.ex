defmodule HakoPdu.NavMsgs.GridCellsConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_grid_cells(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.NavMsgs.GridCells.new()
    binary_read_recursive_grid_cells(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_grid_cells(obj) do
    {base_data, heap_data} = binary_write_recursive_grid_cells(0, "", "", obj)
  ...
  def binary_read_recursive_grid_cells(meta, binary_data, obj, base_off) do
    ...
    cell_width = read_primitive("float32", binary_part(binary_data, base_off + 136, 4))

    # member: cell_height, type: float32 (primitive), offset: 140, size: 4
    cell_height = read_primitive("float32", binary_part(binary_data, base_off + 140, 4))
  ...
  def binary_write_recursive_grid_cells(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: cell_width, type: float32 (primitive), offset: 136, size: 4
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("float32", obj.cell_width, 4))

    # member: cell_height, type: float32 (primitive), offset: 140, size: 4
    allocator = add_binary(allocator, parent_off + 140, primitive_to_bin("float32", obj.cell_height, 4))
