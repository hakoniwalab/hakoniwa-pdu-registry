defmodule HakoPdu.HakoMsgs.DroneStatusConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_status(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DroneStatus.new()
    binary_read_recursive_drone_status(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_status(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_status(0, "", "", obj)
  ...
  def binary_read_recursive_drone_status(meta, binary_data, obj, base_off) do
    # member: flight_mode, type: int32 (primitive), offset: 0, size: 4
    flight_mode = read_primitive("int32", binary_part(binary_data, base_off + 0, 4))

    # member: internal_state, type: int32 (primitive), offset: 4, size: 4
    internal_state = read_primitive("int32", binary_part(binary_data, base_off + 4, 4))
    ...
    collided_counts = read_primitive("int32", binary_part(binary_data, base_off + 32, 4))
  ...
  def binary_write_recursive_drone_status(parent_off, heap_data, allocator, obj) do
    # member: flight_mode, type: int32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int32", obj.flight_mode, 4))

    # member: internal_state, type: int32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("int32", obj.internal_state, 4))
    ...
    # member: collided_counts, type: int32 (primitive), offset: 32, size: 4
    allocator = add_binary(allocator, parent_off + 32, primitive_to_bin("int32", obj.collided_counts, 4))
