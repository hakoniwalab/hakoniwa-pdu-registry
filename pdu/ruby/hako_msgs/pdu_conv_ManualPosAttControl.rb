require_relative 'pdu_type_ManualPosAttControl'
require_relative '../geometry_msgs/pdu_type_Twist'
require_relative '../geometry_msgs/pdu_conv_Twist'


module HakoPdu::HakoMsgs::ManualPosAttControlConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_manual_pos_att_control(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ManualPosAttControl.new
    binary_read_recursive_manual_pos_att_control(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_manual_pos_att_control(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_manual_pos_att_control(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_manual_pos_att_control(meta, binary_data, obj, base_off)
    # member: do_operation, type: bool (primitive), offset: 0, size: 4
    obj.do_operation = read_primitive('bool', binary_data.byteslice(base_off + 0, 4))

    # member: posatt, type: geometry_msgs/Twist (struct), offset: 8, size: 48
    obj.posatt = HakoPdu::GeometryMsgs::Twist.new
    HakoPdu::GeometryMsgs::TwistConverter.binary_read_recursive_twist(meta, binary_data, obj.posatt, base_off + 8)

    obj
  end

  def self.binary_write_recursive_manual_pos_att_control(parent_off, heap_data, allocator, obj)
    # member: do_operation, type: bool (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('bool', obj.do_operation, 4))

    # member: posatt, type: geometry_msgs/Twist (struct), offset: 8, size: 48
    HakoPdu::GeometryMsgs::TwistConverter.binary_write_recursive_twist(parent_off + 8, heap_data, allocator, obj.posatt)
