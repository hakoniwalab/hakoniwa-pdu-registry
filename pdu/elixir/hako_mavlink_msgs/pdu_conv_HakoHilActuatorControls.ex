defmodule HakoPdu.HakoMavlinkMsgs.HakoHilActuatorControlsConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_hil_actuator_controls(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMavlinkMsgs.HakoHilActuatorControls.new()
    binary_read_recursive_hako_hil_actuator_controls(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_hil_actuator_controls(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_hil_actuator_controls(0, "", "", obj)
  ...
  def binary_read_recursive_hako_hil_actuator_controls(meta, binary_data, obj, base_off) do
    ...
    controls = read_primitive_array("float32", binary_part(binary_data, base_off + 8, 64), 16, div(64, 16))

    # member: mode, type: uint8 (primitive), offset: 72, size: 1
    mode = read_primitive("uint8", binary_part(binary_data, base_off + 72, 1))

    # member: flags, type: uint64 (primitive), offset: 80, size: 8
    flags = read_primitive("uint64", binary_part(binary_data, base_off + 80, 8))
  ...
  def binary_write_recursive_hako_hil_actuator_controls(parent_off, heap_data, allocator, obj) do
    # member: time_usec, type: uint64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint64", obj.time_usec, 8))

    # member: controls, type: float32 (primitive), offset: 8, size: 64
    allocator = add_binary(allocator, parent_off + 8, primitive_array_to_bin("float32", obj.controls, div(64, 16)))

    # member: mode, type: uint8 (primitive), offset: 72, size: 1
    allocator = add_binary(allocator, parent_off + 72, primitive_to_bin("uint8", obj.mode, 1))

    # member: flags, type: uint64 (primitive), offset: 80, size: 8
    allocator = add_binary(allocator, parent_off + 80, primitive_to_bin("uint64", obj.flags, 8))
