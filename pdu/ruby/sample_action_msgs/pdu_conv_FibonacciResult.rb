...
module HakoPdu::SampleActionMsgs::FibonacciResultConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_fibonacci_result(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SampleActionMsgs::FibonacciResult.new
    binary_read_recursive_fibonacci_result(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_fibonacci_result(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_fibonacci_result(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_fibonacci_result(meta, binary_data, obj, base_off)
    # member: sequence, type: int32 (primitive), offset: 0, size: 4
    ...
    obj.sequence = read_primitive_array('int32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_fibonacci_result(parent_off, heap_data, allocator, obj)
    # member: sequence, type: int32 (primitive), offset: 0, size: 4
