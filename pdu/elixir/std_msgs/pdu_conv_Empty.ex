defmodule HakoPdu.StdMsgs.EmptyConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_empty(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.Empty.new()
    binary_read_recursive_empty(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_empty(obj) do
    {base_data, heap_data} = binary_write_recursive_empty(0, "", "", obj)
  ...
  def binary_read_recursive_empty(meta, binary_data, obj, base_off) do
    # member: hako_dummy, type: uint8 (primitive), offset: 0, size: 1
    hako_dummy = read_primitive("uint8", binary_part(binary_data, base_off + 0, 1))
  ...
  def binary_write_recursive_empty(parent_off, heap_data, allocator, obj) do
    # member: hako_dummy, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.hako_dummy, 1))
