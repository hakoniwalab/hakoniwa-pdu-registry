require_relative 'pdu_type_CameraCaptureImageRequestPacket'
require_relative '../hako_srv_msgs/pdu_type_ServiceRequestHeader'
require_relative '../hako_srv_msgs/pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_CameraCaptureImageRequest'
require_relative 'pdu_conv_CameraCaptureImageRequest'


module HakoPdu::DroneSrvMsgs::CameraCaptureImageRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_camera_capture_image_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::CameraCaptureImageRequestPacket.new
    binary_read_recursive_camera_capture_image_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_camera_capture_image_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_camera_capture_image_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_camera_capture_image_request_packet(meta, binary_data, obj, base_off)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: CameraCaptureImageRequest (struct), offset: 268, size: 256
    obj.body = HakoPdu::DroneSrvMsgs::CameraCaptureImageRequest.new
    HakoPdu::DroneSrvMsgs::CameraCaptureImageRequestConverter.binary_read_recursive_camera_capture_image_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_camera_capture_image_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: CameraCaptureImageRequest (struct), offset: 268, size: 256
    HakoPdu::DroneSrvMsgs::CameraCaptureImageRequestConverter.binary_write_recursive_camera_capture_image_request(parent_off + 268, heap_data, allocator, obj.body)
