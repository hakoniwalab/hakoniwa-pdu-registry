require_relative 'pdu_type_Byte'


module HakoPdu::StdMsgs::ByteConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_byte(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Byte.new
    binary_read_recursive_byte(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_byte(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_byte(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_byte(meta, binary_data, obj, base_off)
    # member: data, type: byte (primitive), offset: 0, size: 1
    obj.data = read_primitive('byte', binary_data.byteslice(base_off + 0, 1))

    obj
  end

  def self.binary_write_recursive_byte(parent_off, heap_data, allocator, obj)
    # member: data, type: byte (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('byte', obj.data, 1))
