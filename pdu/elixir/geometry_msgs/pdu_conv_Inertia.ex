defmodule HakoPdu.GeometryMsgs.InertiaConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_inertia(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.Inertia.new()
    binary_read_recursive_inertia(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_inertia(obj) do
    {base_data, heap_data} = binary_write_recursive_inertia(0, "", "", obj)
  ...
  def binary_read_recursive_inertia(meta, binary_data, obj, base_off) do
    # member: m, type: float64 (primitive), offset: 0, size: 8
    m = read_primitive("float64", binary_part(binary_data, base_off + 0, 8))
    ...
    ixx = read_primitive("float64", binary_part(binary_data, base_off + 32, 8))

    # member: ixy, type: float64 (primitive), offset: 40, size: 8
    ixy = read_primitive("float64", binary_part(binary_data, base_off + 40, 8))

    # member: ixz, type: float64 (primitive), offset: 48, size: 8
    ixz = read_primitive("float64", binary_part(binary_data, base_off + 48, 8))

    # member: iyy, type: float64 (primitive), offset: 56, size: 8
    iyy = read_primitive("float64", binary_part(binary_data, base_off + 56, 8))

    # member: iyz, type: float64 (primitive), offset: 64, size: 8
    iyz = read_primitive("float64", binary_part(binary_data, base_off + 64, 8))

    # member: izz, type: float64 (primitive), offset: 72, size: 8
    izz = read_primitive("float64", binary_part(binary_data, base_off + 72, 8))
  ...
  def binary_write_recursive_inertia(parent_off, heap_data, allocator, obj) do
    # member: m, type: float64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float64", obj.m, 8))
    ...
    # member: ixx, type: float64 (primitive), offset: 32, size: 8
    allocator = add_binary(allocator, parent_off + 32, primitive_to_bin("float64", obj.ixx, 8))

    # member: ixy, type: float64 (primitive), offset: 40, size: 8
    allocator = add_binary(allocator, parent_off + 40, primitive_to_bin("float64", obj.ixy, 8))

    # member: ixz, type: float64 (primitive), offset: 48, size: 8
    allocator = add_binary(allocator, parent_off + 48, primitive_to_bin("float64", obj.ixz, 8))

    # member: iyy, type: float64 (primitive), offset: 56, size: 8
    allocator = add_binary(allocator, parent_off + 56, primitive_to_bin("float64", obj.iyy, 8))

    # member: iyz, type: float64 (primitive), offset: 64, size: 8
    allocator = add_binary(allocator, parent_off + 64, primitive_to_bin("float64", obj.iyz, 8))

    # member: izz, type: float64 (primitive), offset: 72, size: 8
    allocator = add_binary(allocator, parent_off + 72, primitive_to_bin("float64", obj.izz, 8))
