require_relative 'pdu_type_ExecutionUnitRuntimeNode'


module HakoPdu::HakoMsgs::ExecutionUnitRuntimeNodeConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_execution_unit_runtime_node(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ExecutionUnitRuntimeNode.new
    binary_read_recursive_execution_unit_runtime_node(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_execution_unit_runtime_node(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_execution_unit_runtime_node(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_execution_unit_runtime_node(meta, binary_data, obj, base_off)
    # member: node_id, type: uint32 (primitive), offset: 0, size: 4
    obj.node_id = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    obj
  end

  def self.binary_write_recursive_execution_unit_runtime_node(parent_off, heap_data, allocator, obj)
    # member: node_id, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.node_id, 4))
