defmodule HakoPdu.BuiltinInterfaces.DurationConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_duration(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.BuiltinInterfaces.Duration.new()
    binary_read_recursive_duration(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_duration(obj) do
    {base_data, heap_data} = binary_write_recursive_duration(0, "", "", obj)
  ...
  def binary_read_recursive_duration(meta, binary_data, obj, base_off) do
    # member: sec, type: int32 (primitive), offset: 0, size: 4
    sec = read_primitive("int32", binary_part(binary_data, base_off + 0, 4))

    # member: nanosec, type: uint32 (primitive), offset: 4, size: 4
    nanosec = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))
  ...
  def binary_write_recursive_duration(parent_off, heap_data, allocator, obj) do
    # member: sec, type: int32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int32", obj.sec, 4))

    # member: nanosec, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.nanosec, 4))
