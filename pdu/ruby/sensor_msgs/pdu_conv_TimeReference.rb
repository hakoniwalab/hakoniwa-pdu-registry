require_relative 'pdu_type_TimeReference'
...
module HakoPdu::SensorMsgs::TimeReferenceConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_time_reference(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::TimeReference.new
    binary_read_recursive_time_reference(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_time_reference(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_time_reference(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_time_reference(meta, binary_data, obj, base_off)
    ...
    # member: time_ref, type: builtin_interfaces/Time (struct), offset: 136, size: 8
    obj.time_ref = HakoPdu::BuiltinInterfaces::Time.new
    HakoPdu::BuiltinInterfaces::TimeConverter.binary_read_recursive_time(meta, binary_data, obj.time_ref, base_off + 136)

    # member: source, type: string (primitive), offset: 144, size: 128
    obj.source = read_primitive('string', binary_data.byteslice(base_off + 144, 128))

    obj
  end

  def self.binary_write_recursive_time_reference(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: time_ref, type: builtin_interfaces/Time (struct), offset: 136, size: 8
    HakoPdu::BuiltinInterfaces::TimeConverter.binary_write_recursive_time(parent_off + 136, heap_data, allocator, obj.time_ref)

    # member: source, type: string (primitive), offset: 144, size: 128
    add_binary(allocator, parent_off + 144, primitive_to_bin('string', obj.source, 128))
