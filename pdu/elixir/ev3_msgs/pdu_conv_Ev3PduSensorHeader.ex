defmodule HakoPdu.Ev3Msgs.Ev3PduSensorHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ev3_pdu_sensor_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Ev3Msgs.Ev3PduSensorHeader.new()
    binary_read_recursive_ev3_pdu_sensor_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ev3_pdu_sensor_header(obj) do
    {base_data, heap_data} = binary_write_recursive_ev3_pdu_sensor_header(0, "", "", obj)
  ...
  def binary_read_recursive_ev3_pdu_sensor_header(meta, binary_data, obj, base_off) do
    # member: name, type: string (primitive), offset: 0, size: 128
    name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: version, type: uint32 (primitive), offset: 128, size: 4
    version = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: hakoniwa_time, type: int64 (primitive), offset: 136, size: 8
    hakoniwa_time = read_primitive("int64", binary_part(binary_data, base_off + 136, 8))

    # member: ext_off, type: uint32 (primitive), offset: 144, size: 4
    ext_off = read_primitive("uint32", binary_part(binary_data, base_off + 144, 4))

    # member: ext_size, type: uint32 (primitive), offset: 148, size: 4
    ext_size = read_primitive("uint32", binary_part(binary_data, base_off + 148, 4))
  ...
  def binary_write_recursive_ev3_pdu_sensor_header(parent_off, heap_data, allocator, obj) do
    # member: name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.name, 128))

    # member: version, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.version, 4))

    # member: hakoniwa_time, type: int64 (primitive), offset: 136, size: 8
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("int64", obj.hakoniwa_time, 8))

    # member: ext_off, type: uint32 (primitive), offset: 144, size: 4
    allocator = add_binary(allocator, parent_off + 144, primitive_to_bin("uint32", obj.ext_off, 4))

    # member: ext_size, type: uint32 (primitive), offset: 148, size: 4
    allocator = add_binary(allocator, parent_off + 148, primitive_to_bin("uint32", obj.ext_size, 4))
