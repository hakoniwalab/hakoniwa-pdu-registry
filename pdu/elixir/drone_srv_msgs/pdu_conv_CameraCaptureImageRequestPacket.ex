defmodule HakoPdu.DroneSrvMsgs.CameraCaptureImageRequestPacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_camera_capture_image_request_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.CameraCaptureImageRequestPacket.new()
    binary_read_recursive_camera_capture_image_request_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_camera_capture_image_request_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_camera_capture_image_request_packet(0, "", "", obj)
  ...
  def binary_read_recursive_camera_capture_image_request_packet(meta, binary_data, obj, base_off) do
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    header =
      HakoPdu.HakoSrvMsgs.ServiceRequestHeaderConverter.binary_read_recursive_service_request_header(
        meta,
        binary_data,
        HakoPdu.HakoSrvMsgs.ServiceRequestHeader.new(),
        base_off + 0
      )

    # member: body, type: CameraCaptureImageRequest (struct), offset: 268, size: 256
    body =
      HakoPdu.DroneSrvMsgs.CameraCaptureImageRequestConverter.binary_read_recursive_camera_capture_image_request(
        meta,
        binary_data,
        HakoPdu.DroneSrvMsgs.CameraCaptureImageRequest.new(),
        base_off + 268
      )
  ...
  def binary_write_recursive_camera_capture_image_request_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
      HakoPdu.HakoSrvMsgs.ServiceRequestHeaderConverter.binary_write_recursive_service_request_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: body, type: CameraCaptureImageRequest (struct), offset: 268, size: 256
    {allocator, heap_data} =
      HakoPdu.DroneSrvMsgs.CameraCaptureImageRequestConverter.binary_write_recursive_camera_capture_image_request(
        parent_off + 268,
        heap_data,
        allocator,
        obj.body
