require_relative 'pdu_type_DetachRequestPacket'
require_relative 'pdu_type_ServiceRequestHeader'
require_relative 'pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_DetachRequest'
require_relative 'pdu_conv_DetachRequest'


module HakoPdu::HakoSrvMsgs::DetachRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_detach_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::DetachRequestPacket.new
    binary_read_recursive_detach_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_detach_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_detach_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_detach_request_packet(meta, binary_data, obj, base_off)
    ...
    obj.body = HakoPdu::HakoSrvMsgs::DetachRequest.new
    HakoPdu::HakoSrvMsgs::DetachRequestConverter.binary_read_recursive_detach_request(meta, binary_data, obj.body, base_off + 272)

    obj
  end

  def self.binary_write_recursive_detach_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DetachRequest (struct), offset: 272, size: 8
    HakoPdu::HakoSrvMsgs::DetachRequestConverter.binary_write_recursive_detach_request(parent_off + 272, heap_data, allocator, obj.body)
