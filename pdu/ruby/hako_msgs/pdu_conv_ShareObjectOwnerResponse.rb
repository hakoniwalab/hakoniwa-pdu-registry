require_relative 'pdu_type_ShareObjectOwnerResponse'


module HakoPdu::HakoMsgs::ShareObjectOwnerResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_share_object_owner_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ShareObjectOwnerResponse.new
    binary_read_recursive_share_object_owner_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_share_object_owner_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_share_object_owner_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_share_object_owner_response(meta, binary_data, obj, base_off)
    ...
    obj.owner_id = read_primitive('uint32', binary_data.byteslice(base_off + 132, 4))

    # member: accepted, type: bool (primitive), offset: 136, size: 4
    obj.accepted = read_primitive('bool', binary_data.byteslice(base_off + 136, 4))

    obj
  end

  def self.binary_write_recursive_share_object_owner_response(parent_off, heap_data, allocator, obj)
    # member: object_name, type: string (primitive), offset: 0, size: 128
    ...
    # member: owner_id, type: uint32 (primitive), offset: 132, size: 4
    add_binary(allocator, parent_off + 132, primitive_to_bin('uint32', obj.owner_id, 4))

    # member: accepted, type: bool (primitive), offset: 136, size: 4
    add_binary(allocator, parent_off + 136, primitive_to_bin('bool', obj.accepted, 4))
