require_relative 'pdu_type_GameControllerOperation'


module HakoPdu::HakoMsgs::GameControllerOperationConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_game_controller_operation(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::GameControllerOperation.new
    binary_read_recursive_game_controller_operation(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_game_controller_operation(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_game_controller_operation(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_game_controller_operation(meta, binary_data, obj, base_off)
    # member: axis, type: float64 (primitive), offset: 0, size: 48
    obj.axis = read_primitive_array('float64', binary_data.byteslice(base_off + 0, 48), 6, 48 / 6)

    # member: button, type: bool (primitive), offset: 48, size: 60
    obj.button = read_primitive_array('bool', binary_data.byteslice(base_off + 48, 60), 15, 60 / 15)

    obj
  end

  def self.binary_write_recursive_game_controller_operation(parent_off, heap_data, allocator, obj)
    # member: axis, type: float64 (primitive), offset: 0, size: 48
    add_binary(allocator, parent_off + 0, primitive_array_to_bin('float64', obj.axis, 48 / 6))

    # member: button, type: bool (primitive), offset: 48, size: 60
    add_binary(allocator, parent_off + 48, primitive_array_to_bin('bool', obj.button, 60 / 15))
