...
module HakoPdu::GeometryMsgs::TwistConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_twist(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Twist.new
    binary_read_recursive_twist(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_twist(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_twist(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_twist(meta, binary_data, obj, base_off)
    # member: linear, type: Vector3 (struct), offset: 0, size: 24
    obj.linear = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.linear, base_off + 0)

    # member: angular, type: Vector3 (struct), offset: 24, size: 24
    obj.angular = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.angular, base_off + 24)

    obj
  end

  def self.binary_write_recursive_twist(parent_off, heap_data, allocator, obj)
    # member: linear, type: Vector3 (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 0, heap_data, allocator, obj.linear)

    # member: angular, type: Vector3 (struct), offset: 24, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 24, heap_data, allocator, obj.angular)
