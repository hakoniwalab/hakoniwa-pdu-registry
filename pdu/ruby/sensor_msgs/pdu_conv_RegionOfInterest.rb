...
module HakoPdu::SensorMsgs::RegionOfInterestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_region_of_interest(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::RegionOfInterest.new
    binary_read_recursive_region_of_interest(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_region_of_interest(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_region_of_interest(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_region_of_interest(meta, binary_data, obj, base_off)
    # member: x_offset, type: uint32 (primitive), offset: 0, size: 4
    obj.x_offset = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: y_offset, type: uint32 (primitive), offset: 4, size: 4
    obj.y_offset = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: height, type: uint32 (primitive), offset: 8, size: 4
    obj.height = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))
    ...
    obj.do_rectify = read_primitive('bool', binary_data.byteslice(base_off + 16, 4))

    obj
  end

  def self.binary_write_recursive_region_of_interest(parent_off, heap_data, allocator, obj)
    # member: x_offset, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.x_offset, 4))

    # member: y_offset, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.y_offset, 4))

    # member: height, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.height, 4))

    # member: width, type: uint32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('uint32', obj.width, 4))

    # member: do_rectify, type: bool (primitive), offset: 16, size: 4
    add_binary(allocator, parent_off + 16, primitive_to_bin('bool', obj.do_rectify, 4))
