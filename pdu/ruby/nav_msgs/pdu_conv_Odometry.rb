require_relative 'pdu_type_Odometry'
...
require_relative '../geometry_msgs/pdu_conv_PoseWithCovariance'
require_relative '../geometry_msgs/pdu_type_TwistWithCovariance'
require_relative '../geometry_msgs/pdu_conv_TwistWithCovariance'


module HakoPdu::NavMsgs::OdometryConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_odometry(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::NavMsgs::Odometry.new
    binary_read_recursive_odometry(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_odometry(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_odometry(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_odometry(meta, binary_data, obj, base_off)
    ...
    # member: child_frame_id, type: string (primitive), offset: 136, size: 128
    obj.child_frame_id = read_primitive('string', binary_data.byteslice(base_off + 136, 128))

    # member: pose, type: geometry_msgs/PoseWithCovariance (struct), offset: 264, size: 344
    obj.pose = HakoPdu::GeometryMsgs::PoseWithCovariance.new
    HakoPdu::GeometryMsgs::PoseWithCovarianceConverter.binary_read_recursive_pose_with_covariance(meta, binary_data, obj.pose, base_off + 264)

    # member: twist, type: geometry_msgs/TwistWithCovariance (struct), offset: 608, size: 336
    obj.twist = HakoPdu::GeometryMsgs::TwistWithCovariance.new
    HakoPdu::GeometryMsgs::TwistWithCovarianceConverter.binary_read_recursive_twist_with_covariance(meta, binary_data, obj.twist, base_off + 608)

    obj
  end

  def self.binary_write_recursive_odometry(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: child_frame_id, type: string (primitive), offset: 136, size: 128
    add_binary(allocator, parent_off + 136, primitive_to_bin('string', obj.child_frame_id, 128))

    # member: pose, type: geometry_msgs/PoseWithCovariance (struct), offset: 264, size: 344
    HakoPdu::GeometryMsgs::PoseWithCovarianceConverter.binary_write_recursive_pose_with_covariance(parent_off + 264, heap_data, allocator, obj.pose)

    # member: twist, type: geometry_msgs/TwistWithCovariance (struct), offset: 608, size: 336
    HakoPdu::GeometryMsgs::TwistWithCovarianceConverter.binary_write_recursive_twist_with_covariance(parent_off + 608, heap_data, allocator, obj.twist)
