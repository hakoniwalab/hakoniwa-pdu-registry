defmodule HakoPdu.SampleActionMsgs.FibonacciActionFeedbackConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_fibonacci_action_feedback(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SampleActionMsgs.FibonacciActionFeedback.new()
    binary_read_recursive_fibonacci_action_feedback(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_fibonacci_action_feedback(obj) do
    {base_data, heap_data} = binary_write_recursive_fibonacci_action_feedback(0, "", "", obj)
  ...
  def binary_read_recursive_fibonacci_action_feedback(meta, binary_data, obj, base_off) do
    # member: header, type: hako_action_msgs/ActionFeedbackHeader (struct), offset: 0, size: 24
    header =
      HakoPdu.HakoActionMsgs.ActionFeedbackHeaderConverter.binary_read_recursive_action_feedback_header(
        meta,
        binary_data,
        HakoPdu.HakoActionMsgs.ActionFeedbackHeader.new(),
        base_off + 0
      )

    # member: body, type: FibonacciFeedback (struct), offset: 24, size: 8
    body =
      HakoPdu.SampleActionMsgs.FibonacciFeedbackConverter.binary_read_recursive_fibonacci_feedback(
        meta,
        binary_data,
        HakoPdu.SampleActionMsgs.FibonacciFeedback.new(),
        base_off + 24
      )
  ...
  def binary_write_recursive_fibonacci_action_feedback(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_action_msgs/ActionFeedbackHeader (struct), offset: 0, size: 24
    {allocator, heap_data} =
      HakoPdu.HakoActionMsgs.ActionFeedbackHeaderConverter.binary_write_recursive_action_feedback_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: body, type: FibonacciFeedback (struct), offset: 24, size: 8
    {allocator, heap_data} =
      HakoPdu.SampleActionMsgs.FibonacciFeedbackConverter.binary_write_recursive_fibonacci_feedback(
        parent_off + 24,
        heap_data,
        allocator,
        obj.body
