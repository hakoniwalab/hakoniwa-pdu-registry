require_relative 'pdu_type_GetWorldTimeRequestPacket'
require_relative 'pdu_type_ServiceRequestHeader'
require_relative 'pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_GetWorldTimeRequest'
require_relative 'pdu_conv_GetWorldTimeRequest'


module HakoPdu::HakoSrvMsgs::GetWorldTimeRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_get_world_time_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::GetWorldTimeRequestPacket.new
    binary_read_recursive_get_world_time_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_get_world_time_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_get_world_time_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_get_world_time_request_packet(meta, binary_data, obj, base_off)
    ...
    obj.body = HakoPdu::HakoSrvMsgs::GetWorldTimeRequest.new
    HakoPdu::HakoSrvMsgs::GetWorldTimeRequestConverter.binary_read_recursive_get_world_time_request(meta, binary_data, obj.body, base_off + 272)

    obj
  end

  def self.binary_write_recursive_get_world_time_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: GetWorldTimeRequest (struct), offset: 272, size: 8
    HakoPdu::HakoSrvMsgs::GetWorldTimeRequestConverter.binary_write_recursive_get_world_time_request(parent_off + 272, heap_data, allocator, obj.body)
