defmodule HakoPdu.HakoMsgs.CollisionConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_collision(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.Collision.new()
    binary_read_recursive_collision(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_collision(obj) do
    {base_data, heap_data} = binary_write_recursive_collision(0, "", "", obj)
  ...
  def binary_read_recursive_collision(meta, binary_data, obj, base_off) do
    ...
    contact_num = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))
  ...
  def binary_write_recursive_collision(parent_off, heap_data, allocator, obj) do
    # member: collision, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.collision, 4))

    # member: contact_num, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.contact_num, 4))
    ...
    # member: restitution_coefficient, type: float64 (primitive), offset: 272, size: 8
    allocator = add_binary(allocator, parent_off + 272, primitive_to_bin("float64", obj.restitution_coefficient, 8))
