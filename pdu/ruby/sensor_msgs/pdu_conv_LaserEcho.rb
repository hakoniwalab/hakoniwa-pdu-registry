...
module HakoPdu::SensorMsgs::LaserEchoConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_laser_echo(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::LaserEcho.new
    binary_read_recursive_laser_echo(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_laser_echo(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_laser_echo(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_laser_echo(meta, binary_data, obj, base_off)
    ...
    obj.echoes = read_primitive_array('float32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_laser_echo(parent_off, heap_data, allocator, obj)
    # member: echoes, type: float32 (primitive), offset: 0, size: 4
