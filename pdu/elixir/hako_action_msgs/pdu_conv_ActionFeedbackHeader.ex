defmodule HakoPdu.HakoActionMsgs.ActionFeedbackHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_action_feedback_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoActionMsgs.ActionFeedbackHeader.new()
    binary_read_recursive_action_feedback_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_action_feedback_header(obj) do
    {base_data, heap_data} = binary_write_recursive_action_feedback_header(0, "", "", obj)
  ...
  def binary_read_recursive_action_feedback_header(meta, binary_data, obj, base_off) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    version = read_primitive("uint8", binary_part(binary_data, base_off + 0, 1))

    # member: reserved, type: uint8 (primitive), offset: 1, size: 3
    reserved = read_primitive_array("uint8", binary_part(binary_data, base_off + 1, 3), 3, div(3, 3))
    ...
    # member: sequence_no, type: uint32 (primitive), offset: 20, size: 4
    sequence_no = read_primitive("uint32", binary_part(binary_data, base_off + 20, 4))
  ...
  def binary_write_recursive_action_feedback_header(parent_off, heap_data, allocator, obj) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.version, 1))

    # member: reserved, type: uint8 (primitive), offset: 1, size: 3
    allocator = add_binary(allocator, parent_off + 1, primitive_array_to_bin("uint8", obj.reserved, div(3, 3)))

    # member: goal_id, type: uint8 (primitive), offset: 4, size: 16
    allocator = add_binary(allocator, parent_off + 4, primitive_array_to_bin("uint8", obj.goal_id, div(16, 16)))

    # member: sequence_no, type: uint32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("uint32", obj.sequence_no, 4))
