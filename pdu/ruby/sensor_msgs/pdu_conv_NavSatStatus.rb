...
module HakoPdu::SensorMsgs::NavSatStatusConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_nav_sat_status(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::NavSatStatus.new
    binary_read_recursive_nav_sat_status(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_nav_sat_status(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_nav_sat_status(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_nav_sat_status(meta, binary_data, obj, base_off)
    # member: status, type: int8 (primitive), offset: 0, size: 1
    obj.status = read_primitive('int8', binary_data.byteslice(base_off + 0, 1))

    # member: service, type: uint16 (primitive), offset: 2, size: 2
    obj.service = read_primitive('uint16', binary_data.byteslice(base_off + 2, 2))

    obj
  end

  def self.binary_write_recursive_nav_sat_status(parent_off, heap_data, allocator, obj)
    # member: status, type: int8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('int8', obj.status, 1))

    # member: service, type: uint16 (primitive), offset: 2, size: 2
    add_binary(allocator, parent_off + 2, primitive_to_bin('uint16', obj.service, 2))
