defmodule HakoPdu.DroneSrvMsgs.DroneGetStateResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_get_state_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.DroneGetStateResponse.new()
    binary_read_recursive_drone_get_state_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_get_state_response(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_get_state_response(0, "", "", obj)
  ...
  def binary_read_recursive_drone_get_state_response(meta, binary_data, obj, base_off) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    ok = read_primitive("bool", binary_part(binary_data, base_off + 0, 4))

    # member: is_ready, type: bool (primitive), offset: 4, size: 4
    is_ready = read_primitive("bool", binary_part(binary_data, base_off + 4, 4))

    # member: current_pose, type: geometry_msgs/Pose (struct), offset: 8, size: 56
    current_pose =
      HakoPdu.GeometryMsgs.PoseConverter.binary_read_recursive_pose(
        meta,
        binary_data,
        HakoPdu.GeometryMsgs.Pose.new(),
        base_off + 8
      )

    # member: battery_status, type: hako_msgs/HakoBatteryStatus (struct), offset: 64, size: 32
    battery_status =
      HakoPdu.HakoMsgs.HakoBatteryStatusConverter.binary_read_recursive_hako_battery_status(
        meta,
        binary_data,
        HakoPdu.HakoMsgs.HakoBatteryStatus.new(),
        base_off + 64
      )

    # member: mode, type: string (primitive), offset: 96, size: 128
    mode = read_primitive("string", binary_part(binary_data, base_off + 96, 128))

    # member: message, type: string (primitive), offset: 224, size: 128
    message = read_primitive("string", binary_part(binary_data, base_off + 224, 128))
  ...
  def binary_write_recursive_drone_get_state_response(parent_off, heap_data, allocator, obj) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.ok, 4))

    # member: is_ready, type: bool (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("bool", obj.is_ready, 4))

    # member: current_pose, type: geometry_msgs/Pose (struct), offset: 8, size: 56
    {allocator, heap_data} =
      HakoPdu.GeometryMsgs.PoseConverter.binary_write_recursive_pose(
        parent_off + 8,
        heap_data,
        allocator,
        obj.current_pose
    ...
    # member: battery_status, type: hako_msgs/HakoBatteryStatus (struct), offset: 64, size: 32
    {allocator, heap_data} =
      HakoPdu.HakoMsgs.HakoBatteryStatusConverter.binary_write_recursive_hako_battery_status(
        parent_off + 64,
        heap_data,
        allocator,
        obj.battery_status
    ...
    # member: mode, type: string (primitive), offset: 96, size: 128
    allocator = add_binary(allocator, parent_off + 96, primitive_to_bin("string", obj.mode, 128))

    # member: message, type: string (primitive), offset: 224, size: 128
    allocator = add_binary(allocator, parent_off + 224, primitive_to_bin("string", obj.message, 128))
