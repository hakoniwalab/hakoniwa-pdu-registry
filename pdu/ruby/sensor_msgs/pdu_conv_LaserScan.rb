require_relative 'pdu_type_LaserScan'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'


module HakoPdu::SensorMsgs::LaserScanConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_laser_scan(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::LaserScan.new
    binary_read_recursive_laser_scan(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_laser_scan(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_laser_scan(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_laser_scan(meta, binary_data, obj, base_off)
    ...
    obj.ranges = read_primitive_array('float32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)
    ...
    obj.intensities = read_primitive_array('float32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_laser_scan(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
