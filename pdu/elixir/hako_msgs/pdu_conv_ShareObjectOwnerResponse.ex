defmodule HakoPdu.HakoMsgs.ShareObjectOwnerResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_share_object_owner_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.ShareObjectOwnerResponse.new()
    binary_read_recursive_share_object_owner_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_share_object_owner_response(obj) do
    {base_data, heap_data} = binary_write_recursive_share_object_owner_response(0, "", "", obj)
  ...
  def binary_read_recursive_share_object_owner_response(meta, binary_data, obj, base_off) do
    ...
    # member: owner_id, type: uint32 (primitive), offset: 132, size: 4
    owner_id = read_primitive("uint32", binary_part(binary_data, base_off + 132, 4))

    # member: accepted, type: bool (primitive), offset: 136, size: 4
    accepted = read_primitive("bool", binary_part(binary_data, base_off + 136, 4))
  ...
  def binary_write_recursive_share_object_owner_response(parent_off, heap_data, allocator, obj) do
    # member: object_name, type: string (primitive), offset: 0, size: 128
    ...
    # member: owner_id, type: uint32 (primitive), offset: 132, size: 4
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("uint32", obj.owner_id, 4))

    # member: accepted, type: bool (primitive), offset: 136, size: 4
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("bool", obj.accepted, 4))
