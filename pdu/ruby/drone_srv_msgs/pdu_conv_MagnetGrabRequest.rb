...
module HakoPdu::DroneSrvMsgs::MagnetGrabRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_magnet_grab_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::MagnetGrabRequest.new
    binary_read_recursive_magnet_grab_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_magnet_grab_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_magnet_grab_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_magnet_grab_request(meta, binary_data, obj, base_off)
    ...
    obj.grab_on = read_primitive('bool', binary_data.byteslice(base_off + 128, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 132, size: 4
    obj.timeout_sec = read_primitive('float32', binary_data.byteslice(base_off + 132, 4))

    obj
  end

  def self.binary_write_recursive_magnet_grab_request(parent_off, heap_data, allocator, obj)
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.drone_name, 128))

    # member: grab_on, type: bool (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('bool', obj.grab_on, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 132, size: 4
    add_binary(allocator, parent_off + 132, primitive_to_bin('float32', obj.timeout_sec, 4))
