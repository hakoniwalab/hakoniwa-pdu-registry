require_relative 'pdu_type_MultiDOFJointTrajectory'
...
require_relative 'pdu_conv_MultiDOFJointTrajectoryPoint'


module HakoPdu::TrajectoryMsgs::MultiDOFJointTrajectoryConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_multi_d_o_f_joint_trajectory(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::TrajectoryMsgs::MultiDOFJointTrajectory.new
    binary_read_recursive_multi_d_o_f_joint_trajectory(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_multi_d_o_f_joint_trajectory(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_multi_d_o_f_joint_trajectory(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_multi_d_o_f_joint_trajectory(meta, binary_data, obj, base_off)
    ...
    obj.joint_names = read_primitive_array('string', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 128 * array_size), array_size, 128)
    ...
    array_size.times do |i|
      child = HakoPdu::TrajectoryMsgs::MultiDOFJointTrajectoryPoint.new
      HakoPdu::TrajectoryMsgs::MultiDOFJointTrajectoryPointConverter.binary_read_recursive_multi_d_o_f_joint_trajectory_point(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 32)
      obj.points << child
    end
  ...
  def self.binary_write_recursive_multi_d_o_f_joint_trajectory(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    values.each_with_index do |child, i|
      HakoPdu::TrajectoryMsgs::MultiDOFJointTrajectoryPointConverter.binary_write_recursive_multi_d_o_f_joint_trajectory_point(offset_from_heap + i * 32, heap_data, heap_data, child)
    end
