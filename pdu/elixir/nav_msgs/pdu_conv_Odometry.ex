defmodule HakoPdu.NavMsgs.OdometryConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_odometry(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.NavMsgs.Odometry.new()
    binary_read_recursive_odometry(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_odometry(obj) do
    {base_data, heap_data} = binary_write_recursive_odometry(0, "", "", obj)
  ...
  def binary_read_recursive_odometry(meta, binary_data, obj, base_off) do
    ...
    child_frame_id = read_primitive("string", binary_part(binary_data, base_off + 136, 128))

    # member: pose, type: geometry_msgs/PoseWithCovariance (struct), offset: 264, size: 344
    pose =
      HakoPdu.GeometryMsgs.PoseWithCovarianceConverter.binary_read_recursive_pose_with_covariance(
        meta,
        binary_data,
        HakoPdu.GeometryMsgs.PoseWithCovariance.new(),
        base_off + 264
      )

    # member: twist, type: geometry_msgs/TwistWithCovariance (struct), offset: 608, size: 336
    twist =
      HakoPdu.GeometryMsgs.TwistWithCovarianceConverter.binary_read_recursive_twist_with_covariance(
        meta,
        binary_data,
        HakoPdu.GeometryMsgs.TwistWithCovariance.new(),
        base_off + 608
      )
  ...
  def binary_write_recursive_odometry(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: child_frame_id, type: string (primitive), offset: 136, size: 128
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("string", obj.child_frame_id, 128))

    # member: pose, type: geometry_msgs/PoseWithCovariance (struct), offset: 264, size: 344
    {allocator, heap_data} =
      HakoPdu.GeometryMsgs.PoseWithCovarianceConverter.binary_write_recursive_pose_with_covariance(
        parent_off + 264,
        heap_data,
        allocator,
        obj.pose
    ...
    # member: twist, type: geometry_msgs/TwistWithCovariance (struct), offset: 608, size: 336
    {allocator, heap_data} =
      HakoPdu.GeometryMsgs.TwistWithCovarianceConverter.binary_write_recursive_twist_with_covariance(
        parent_off + 608,
        heap_data,
        allocator,
        obj.twist
