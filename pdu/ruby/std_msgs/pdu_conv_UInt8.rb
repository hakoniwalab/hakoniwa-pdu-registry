require_relative 'pdu_type_UInt8'


module HakoPdu::StdMsgs::UInt8Converter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_u_int8(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::UInt8.new
    binary_read_recursive_u_int8(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_u_int8(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_u_int8(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_u_int8(meta, binary_data, obj, base_off)
    # member: data, type: uint8 (primitive), offset: 0, size: 1
    obj.data = read_primitive('uint8', binary_data.byteslice(base_off + 0, 1))

    obj
  end

  def self.binary_write_recursive_u_int8(parent_off, heap_data, allocator, obj)
    # member: data, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.data, 1))
