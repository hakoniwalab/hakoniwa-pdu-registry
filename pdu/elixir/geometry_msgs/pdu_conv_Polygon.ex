defmodule HakoPdu.GeometryMsgs.PolygonConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_polygon(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.Polygon.new()
    binary_read_recursive_polygon(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_polygon(obj) do
    {base_data, heap_data} = binary_write_recursive_polygon(0, "", "", obj)
  ...
  def binary_read_recursive_polygon(meta, binary_data, obj, base_off) do
    # member: points, type: Point32 (struct), offset: 0, size: 12
  ...
  def binary_write_recursive_polygon(parent_off, heap_data, allocator, obj) do
    # member: points, type: Point32 (struct), offset: 0, size: 12
