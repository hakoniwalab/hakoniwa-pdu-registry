defmodule HakoPdu.HakoMsgs.ShareObjectOwnerRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_share_object_owner_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.ShareObjectOwnerRequest.new()
    binary_read_recursive_share_object_owner_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_share_object_owner_request(obj) do
    {base_data, heap_data} = binary_write_recursive_share_object_owner_request(0, "", "", obj)
  ...
  def binary_read_recursive_share_object_owner_request(meta, binary_data, obj, base_off) do
    # member: object_name, type: string (primitive), offset: 0, size: 128
    object_name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: request_type, type: uint32 (primitive), offset: 128, size: 4
    request_type = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: new_owner_id, type: uint32 (primitive), offset: 132, size: 4
    new_owner_id = read_primitive("uint32", binary_part(binary_data, base_off + 132, 4))

    # member: request_time, type: uint64 (primitive), offset: 136, size: 8
    request_time = read_primitive("uint64", binary_part(binary_data, base_off + 136, 8))
  ...
  def binary_write_recursive_share_object_owner_request(parent_off, heap_data, allocator, obj) do
    # member: object_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.object_name, 128))

    # member: request_type, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.request_type, 4))

    # member: new_owner_id, type: uint32 (primitive), offset: 132, size: 4
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("uint32", obj.new_owner_id, 4))

    # member: request_time, type: uint64 (primitive), offset: 136, size: 8
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("uint64", obj.request_time, 8))
