defmodule HakoPdu.GeometryMsgs.Pose2DConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_pose2_d(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.Pose2D.new()
    binary_read_recursive_pose2_d(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_pose2_d(obj) do
    {base_data, heap_data} = binary_write_recursive_pose2_d(0, "", "", obj)
  ...
  def binary_read_recursive_pose2_d(meta, binary_data, obj, base_off) do
    ...
    # member: theta, type: float64 (primitive), offset: 16, size: 8
    theta = read_primitive("float64", binary_part(binary_data, base_off + 16, 8))
  ...
  def binary_write_recursive_pose2_d(parent_off, heap_data, allocator, obj) do
    # member: x, type: float64 (primitive), offset: 0, size: 8
    ...
    # member: theta, type: float64 (primitive), offset: 16, size: 8
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float64", obj.theta, 8))
