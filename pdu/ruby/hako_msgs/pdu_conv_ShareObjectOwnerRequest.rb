require_relative 'pdu_type_ShareObjectOwnerRequest'


module HakoPdu::HakoMsgs::ShareObjectOwnerRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_share_object_owner_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ShareObjectOwnerRequest.new
    binary_read_recursive_share_object_owner_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_share_object_owner_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_share_object_owner_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_share_object_owner_request(meta, binary_data, obj, base_off)
    ...
    obj.request_type = read_primitive('uint32', binary_data.byteslice(base_off + 128, 4))

    # member: new_owner_id, type: uint32 (primitive), offset: 132, size: 4
    obj.new_owner_id = read_primitive('uint32', binary_data.byteslice(base_off + 132, 4))

    # member: request_time, type: uint64 (primitive), offset: 136, size: 8
    obj.request_time = read_primitive('uint64', binary_data.byteslice(base_off + 136, 8))

    obj
  end

  def self.binary_write_recursive_share_object_owner_request(parent_off, heap_data, allocator, obj)
    # member: object_name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.object_name, 128))

    # member: request_type, type: uint32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint32', obj.request_type, 4))

    # member: new_owner_id, type: uint32 (primitive), offset: 132, size: 4
    add_binary(allocator, parent_off + 132, primitive_to_bin('uint32', obj.new_owner_id, 4))

    # member: request_time, type: uint64 (primitive), offset: 136, size: 8
    add_binary(allocator, parent_off + 136, primitive_to_bin('uint64', obj.request_time, 8))
