...
{
    public static class Int32MultiArrayConverter
    {
        public static Int32MultiArray PduToMsg(byte[] binaryData)
        {
        ...
        public static byte[] MsgToPdu(Int32MultiArray obj)
        {
        ...
        public static void BinaryReadRecursive(PduMetaData meta, byte[] binaryData, Int32MultiArray obj, int baseOff)
        {
            ...
            obj.data = new List<int>();
            {
                ...
                for (var i = 0; i < arraySize; i++) {
                    obj.data.Add(PduRuntime.ReadInt32(binaryData, meta.HeapOff + offsetFromHeap + (i * 4)));
                }
            }
        }

        public static void BinaryWriteRecursive(int parentOff, BinaryWriterContainer writer, DynamicAllocator allocator, Int32MultiArray obj)
        {
            MultiArrayLayoutConverter.BinaryWriteRecursive(parentOff + 0, writer, allocator, obj.layout);
            {
                ...
                for (var i = 0; i < obj.data.Count; i++) {
                    writer.HeapAllocator.Add(PduRuntime.GetBinaryForInt32(obj.data[i]));
                }
            }
        }
    }
