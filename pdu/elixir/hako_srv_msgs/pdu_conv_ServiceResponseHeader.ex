defmodule HakoPdu.HakoSrvMsgs.ServiceResponseHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_service_response_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.ServiceResponseHeader.new()
    binary_read_recursive_service_response_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_service_response_header(obj) do
    {base_data, heap_data} = binary_write_recursive_service_response_header(0, "", "", obj)
  ...
  def binary_read_recursive_service_response_header(meta, binary_data, obj, base_off) do
    # member: request_id, type: uint32 (primitive), offset: 0, size: 4
    request_id = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: service_name, type: string (primitive), offset: 4, size: 128
    service_name = read_primitive("string", binary_part(binary_data, base_off + 4, 128))

    # member: client_name, type: string (primitive), offset: 132, size: 128
    client_name = read_primitive("string", binary_part(binary_data, base_off + 132, 128))

    # member: status, type: uint8 (primitive), offset: 260, size: 1
    status = read_primitive("uint8", binary_part(binary_data, base_off + 260, 1))

    # member: processing_percentage, type: uint8 (primitive), offset: 261, size: 1
    processing_percentage = read_primitive("uint8", binary_part(binary_data, base_off + 261, 1))

    # member: result_code, type: int32 (primitive), offset: 264, size: 4
    result_code = read_primitive("int32", binary_part(binary_data, base_off + 264, 4))
  ...
  def binary_write_recursive_service_response_header(parent_off, heap_data, allocator, obj) do
    # member: request_id, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.request_id, 4))

    # member: service_name, type: string (primitive), offset: 4, size: 128
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("string", obj.service_name, 128))

    # member: client_name, type: string (primitive), offset: 132, size: 128
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("string", obj.client_name, 128))

    # member: status, type: uint8 (primitive), offset: 260, size: 1
    allocator = add_binary(allocator, parent_off + 260, primitive_to_bin("uint8", obj.status, 1))

    # member: processing_percentage, type: uint8 (primitive), offset: 261, size: 1
    allocator = add_binary(allocator, parent_off + 261, primitive_to_bin("uint8", obj.processing_percentage, 1))

    # member: result_code, type: int32 (primitive), offset: 264, size: 4
    allocator = add_binary(allocator, parent_off + 264, primitive_to_bin("int32", obj.result_code, 4))
