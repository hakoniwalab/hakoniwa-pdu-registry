defmodule HakoPdu.Ev3Msgs.Ev3PduColorSensorConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ev3_pdu_color_sensor(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Ev3Msgs.Ev3PduColorSensor.new()
    binary_read_recursive_ev3_pdu_color_sensor(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ev3_pdu_color_sensor(obj) do
    {base_data, heap_data} = binary_write_recursive_ev3_pdu_color_sensor(0, "", "", obj)
  ...
  def binary_read_recursive_ev3_pdu_color_sensor(meta, binary_data, obj, base_off) do
    # member: color, type: uint32 (primitive), offset: 0, size: 4
    color = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: reflect, type: uint32 (primitive), offset: 4, size: 4
    reflect = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: rgb_r, type: uint32 (primitive), offset: 8, size: 4
    rgb_r = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))

    # member: rgb_g, type: uint32 (primitive), offset: 12, size: 4
    rgb_g = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))

    # member: rgb_b, type: uint32 (primitive), offset: 16, size: 4
    rgb_b = read_primitive("uint32", binary_part(binary_data, base_off + 16, 4))
  ...
  def binary_write_recursive_ev3_pdu_color_sensor(parent_off, heap_data, allocator, obj) do
    # member: color, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.color, 4))

    # member: reflect, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.reflect, 4))

    # member: rgb_r, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.rgb_r, 4))

    # member: rgb_g, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.rgb_g, 4))

    # member: rgb_b, type: uint32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("uint32", obj.rgb_b, 4))
