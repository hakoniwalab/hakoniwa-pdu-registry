defmodule HakoPdu.SensorMsgs.ChannelFloat32Converter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_channel_float32(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.ChannelFloat32.new()
    binary_read_recursive_channel_float32(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_channel_float32(obj) do
    {base_data, heap_data} = binary_write_recursive_channel_float32(0, "", "", obj)
  ...
  def binary_read_recursive_channel_float32(meta, binary_data, obj, base_off) do
    # member: name, type: string (primitive), offset: 0, size: 128
    name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: values, type: float32 (primitive), offset: 128, size: 4
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 128, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 128 + 4, 4))
    values = read_primitive_array("float32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)
  ...
  def binary_write_recursive_channel_float32(parent_off, heap_data, allocator, obj) do
    # member: name, type: string (primitive), offset: 0, size: 128
    ...
    values = obj.values || []
    allocator = add_binary(allocator, parent_off + 128, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
