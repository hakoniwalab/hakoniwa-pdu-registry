defmodule HakoPdu.Tf2Msgs.TFMessageConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_tf_message(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Tf2Msgs.TFMessage.new()
    binary_read_recursive_tf_message(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_tf_message(obj) do
    {base_data, heap_data} = binary_write_recursive_tf_message(0, "", "", obj)
  ...
  def binary_read_recursive_tf_message(meta, binary_data, obj, base_off) do
    ...
    transforms =
      ...
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.TransformStampedConverter.binary_read_recursive_transform_stamped(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.TransformStamped.new(),
          meta.heap_off + offset_from_heap + i * 320
        )
      end)
  ...
  def binary_write_recursive_tf_message(parent_off, heap_data, allocator, obj) do
    # member: transforms, type: geometry_msgs/TransformStamped (struct), offset: 0, size: 320
    ...
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 320))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.TransformStampedConverter.binary_write_recursive_transform_stamped(
