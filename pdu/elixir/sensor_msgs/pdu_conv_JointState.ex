defmodule HakoPdu.SensorMsgs.JointStateConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joint_state(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.JointState.new()
    binary_read_recursive_joint_state(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joint_state(obj) do
    {base_data, heap_data} = binary_write_recursive_joint_state(0, "", "", obj)
  ...
  def binary_read_recursive_joint_state(meta, binary_data, obj, base_off) do
    ...
    name = read_primitive_array("string", binary_part(binary_data, meta.heap_off + offset_from_heap, 128 * array_size), array_size, 128)
    ...
    position = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)
    ...
    velocity = read_primitive_array("float64", binary_part(binary_data, meta.heap_off + offset_from_heap, 8 * array_size), array_size, 8)
  ...
  def binary_write_recursive_joint_state(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
