require_relative 'pdu_type_PoseArray'
...
module HakoPdu::GeometryMsgs::PoseArrayConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_pose_array(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::PoseArray.new
    binary_read_recursive_pose_array(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_pose_array(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_pose_array(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_pose_array(meta, binary_data, obj, base_off)
    ...
    array_size.times do |i|
      child = HakoPdu::GeometryMsgs::Pose.new
      HakoPdu::GeometryMsgs::PoseConverter.binary_read_recursive_pose(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 56)
      obj.poses << child
    end
  ...
  def self.binary_write_recursive_pose_array(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    values.each_with_index do |child, i|
      HakoPdu::GeometryMsgs::PoseConverter.binary_write_recursive_pose(offset_from_heap + i * 56, heap_data, heap_data, child)
    end
