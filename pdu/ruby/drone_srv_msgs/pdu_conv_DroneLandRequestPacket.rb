require_relative 'pdu_type_DroneLandRequestPacket'
require_relative '../hako_srv_msgs/pdu_type_ServiceRequestHeader'
require_relative '../hako_srv_msgs/pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_DroneLandRequest'
require_relative 'pdu_conv_DroneLandRequest'


module HakoPdu::DroneSrvMsgs::DroneLandRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_land_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneLandRequestPacket.new
    binary_read_recursive_drone_land_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_land_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_land_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_land_request_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: DroneLandRequest (struct), offset: 268, size: 128
    obj.body = HakoPdu::DroneSrvMsgs::DroneLandRequest.new
    HakoPdu::DroneSrvMsgs::DroneLandRequestConverter.binary_read_recursive_drone_land_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_drone_land_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DroneLandRequest (struct), offset: 268, size: 128
    HakoPdu::DroneSrvMsgs::DroneLandRequestConverter.binary_write_recursive_drone_land_request(parent_off + 268, heap_data, allocator, obj.body)
