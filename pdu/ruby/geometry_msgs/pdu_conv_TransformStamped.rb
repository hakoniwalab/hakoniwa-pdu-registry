require_relative 'pdu_type_TransformStamped'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_Transform'
require_relative 'pdu_conv_Transform'


module HakoPdu::GeometryMsgs::TransformStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_transform_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::TransformStamped.new
    binary_read_recursive_transform_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_transform_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_transform_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_transform_stamped(meta, binary_data, obj, base_off)
    ...
    # member: transform, type: Transform (struct), offset: 264, size: 56
    obj.transform = HakoPdu::GeometryMsgs::Transform.new
    HakoPdu::GeometryMsgs::TransformConverter.binary_read_recursive_transform(meta, binary_data, obj.transform, base_off + 264)

    obj
  end

  def self.binary_write_recursive_transform_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    # member: transform, type: Transform (struct), offset: 264, size: 56
    HakoPdu::GeometryMsgs::TransformConverter.binary_write_recursive_transform(parent_off + 264, heap_data, allocator, obj.transform)
