defmodule HakoPdu.DroneSrvMsgs.CameraCaptureImageResponsePacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_camera_capture_image_response_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.CameraCaptureImageResponsePacket.new()
    binary_read_recursive_camera_capture_image_response_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_camera_capture_image_response_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_camera_capture_image_response_packet(0, "", "", obj)
  ...
  def binary_read_recursive_camera_capture_image_response_packet(meta, binary_data, obj, base_off) do
    # member: header, type: hako_srv_msgs/ServiceResponseHeader (struct), offset: 0, size: 268
    header =
      HakoPdu.HakoSrvMsgs.ServiceResponseHeaderConverter.binary_read_recursive_service_response_header(
        meta,
        binary_data,
        HakoPdu.HakoSrvMsgs.ServiceResponseHeader.new(),
        base_off + 0
      )

    # member: body, type: CameraCaptureImageResponse (struct), offset: 268, size: 140
    body =
      HakoPdu.DroneSrvMsgs.CameraCaptureImageResponseConverter.binary_read_recursive_camera_capture_image_response(
        meta,
        binary_data,
        HakoPdu.DroneSrvMsgs.CameraCaptureImageResponse.new(),
        base_off + 268
      )
  ...
  def binary_write_recursive_camera_capture_image_response_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_srv_msgs/ServiceResponseHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
      HakoPdu.HakoSrvMsgs.ServiceResponseHeaderConverter.binary_write_recursive_service_response_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: body, type: CameraCaptureImageResponse (struct), offset: 268, size: 140
    {allocator, heap_data} =
      HakoPdu.DroneSrvMsgs.CameraCaptureImageResponseConverter.binary_write_recursive_camera_capture_image_response(
        parent_off + 268,
        heap_data,
        allocator,
        obj.body
