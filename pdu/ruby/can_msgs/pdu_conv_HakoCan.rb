require_relative 'pdu_type_HakoCan'
require_relative 'pdu_type_HakoCanHead'
require_relative 'pdu_conv_HakoCanHead'
require_relative 'pdu_type_HakoCanBody'
require_relative 'pdu_conv_HakoCanBody'


module HakoPdu::CanMsgs::HakoCanConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_can(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::CanMsgs::HakoCan.new
    binary_read_recursive_hako_can(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_can(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_can(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_can(meta, binary_data, obj, base_off)
    # member: head, type: HakoCanHead (struct), offset: 0, size: 20
    obj.head = HakoPdu::CanMsgs::HakoCanHead.new
    HakoPdu::CanMsgs::HakoCanHeadConverter.binary_read_recursive_hako_can_head(meta, binary_data, obj.head, base_off + 0)

    # member: body, type: HakoCanBody (struct), offset: 20, size: 8
    obj.body = HakoPdu::CanMsgs::HakoCanBody.new
    HakoPdu::CanMsgs::HakoCanBodyConverter.binary_read_recursive_hako_can_body(meta, binary_data, obj.body, base_off + 20)

    obj
  end

  def self.binary_write_recursive_hako_can(parent_off, heap_data, allocator, obj)
    # member: head, type: HakoCanHead (struct), offset: 0, size: 20
    HakoPdu::CanMsgs::HakoCanHeadConverter.binary_write_recursive_hako_can_head(parent_off + 0, heap_data, allocator, obj.head)

    # member: body, type: HakoCanBody (struct), offset: 20, size: 8
    HakoPdu::CanMsgs::HakoCanBodyConverter.binary_write_recursive_hako_can_body(parent_off + 20, heap_data, allocator, obj.body)
