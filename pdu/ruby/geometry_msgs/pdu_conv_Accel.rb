...
module HakoPdu::GeometryMsgs::AccelConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_accel(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Accel.new
    binary_read_recursive_accel(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_accel(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_accel(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_accel(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_accel(parent_off, heap_data, allocator, obj)
    # member: linear, type: Vector3 (struct), offset: 0, size: 24
