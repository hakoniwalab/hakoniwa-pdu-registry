defmodule HakoPdu.HakoSrvMsgs.ServiceRequestHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_service_request_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.ServiceRequestHeader.new()
    binary_read_recursive_service_request_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_service_request_header(obj) do
    {base_data, heap_data} = binary_write_recursive_service_request_header(0, "", "", obj)
  ...
  def binary_read_recursive_service_request_header(meta, binary_data, obj, base_off) do
    ...
    # member: opcode, type: uint8 (primitive), offset: 260, size: 1
    opcode = read_primitive("uint8", binary_part(binary_data, base_off + 260, 1))

    # member: status_poll_interval_msec, type: int32 (primitive), offset: 264, size: 4
    status_poll_interval_msec = read_primitive("int32", binary_part(binary_data, base_off + 264, 4))
  ...
  def binary_write_recursive_service_request_header(parent_off, heap_data, allocator, obj) do
    # member: request_id, type: uint32 (primitive), offset: 0, size: 4
    ...
    # member: opcode, type: uint8 (primitive), offset: 260, size: 1
    allocator = add_binary(allocator, parent_off + 260, primitive_to_bin("uint8", obj.opcode, 1))

    # member: status_poll_interval_msec, type: int32 (primitive), offset: 264, size: 4
    allocator = add_binary(allocator, parent_off + 264, primitive_to_bin("int32", obj.status_poll_interval_msec, 4))
