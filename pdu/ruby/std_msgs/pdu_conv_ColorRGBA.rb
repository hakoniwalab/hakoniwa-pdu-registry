require_relative 'pdu_type_ColorRGBA'


module HakoPdu::StdMsgs::ColorRGBAConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_color_r_g_b_a(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::ColorRGBA.new
    binary_read_recursive_color_r_g_b_a(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_color_r_g_b_a(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_color_r_g_b_a(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_color_r_g_b_a(meta, binary_data, obj, base_off)
    # member: r, type: float32 (primitive), offset: 0, size: 4
    obj.r = read_primitive('float32', binary_data.byteslice(base_off + 0, 4))

    # member: g, type: float32 (primitive), offset: 4, size: 4
    obj.g = read_primitive('float32', binary_data.byteslice(base_off + 4, 4))

    # member: b, type: float32 (primitive), offset: 8, size: 4
    obj.b = read_primitive('float32', binary_data.byteslice(base_off + 8, 4))

    # member: a, type: float32 (primitive), offset: 12, size: 4
    obj.a = read_primitive('float32', binary_data.byteslice(base_off + 12, 4))

    obj
  end

  def self.binary_write_recursive_color_r_g_b_a(parent_off, heap_data, allocator, obj)
    # member: r, type: float32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('float32', obj.r, 4))

    # member: g, type: float32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('float32', obj.g, 4))

    # member: b, type: float32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('float32', obj.b, 4))

    # member: a, type: float32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('float32', obj.a, 4))
