require_relative 'pdu_type_CompressedImage'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'


module HakoPdu::SensorMsgs::CompressedImageConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_compressed_image(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::CompressedImage.new
    binary_read_recursive_compressed_image(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_compressed_image(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_compressed_image(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_compressed_image(meta, binary_data, obj, base_off)
    ...
    obj.format = read_primitive('string', binary_data.byteslice(base_off + 136, 128))

    # member: data, type: uint8 (primitive), offset: 264, size: 1
    array_size = read_primitive('int32', binary_data.byteslice(base_off + 264, 4))
    offset_from_heap = read_primitive('int32', binary_data.byteslice(base_off + 264 + 4, 4))
    obj.data = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    obj
  end

  def self.binary_write_recursive_compressed_image(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: format, type: string (primitive), offset: 136, size: 128
    add_binary(allocator, parent_off + 136, primitive_to_bin('string', obj.format, 128))

    # member: data, type: uint8 (primitive), offset: 264, size: 1
    offset_from_heap = heap_data.bytesize
    values = obj.data || []
    add_binary(allocator, parent_off + 264, [values.length, offset_from_heap].pack('l<l<'))
