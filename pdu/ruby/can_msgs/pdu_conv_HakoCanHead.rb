require_relative 'pdu_type_HakoCanHead'


module HakoPdu::CanMsgs::HakoCanHeadConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_can_head(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::CanMsgs::HakoCanHead.new
    binary_read_recursive_hako_can_head(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_can_head(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_can_head(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_can_head(meta, binary_data, obj, base_off)
    # member: channel, type: uint32 (primitive), offset: 0, size: 4
    obj.channel = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: ide, type: uint32 (primitive), offset: 4, size: 4
    obj.ide = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: rtr, type: uint32 (primitive), offset: 8, size: 4
    obj.rtr = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))

    # member: dlc, type: uint32 (primitive), offset: 12, size: 4
    obj.dlc = read_primitive('uint32', binary_data.byteslice(base_off + 12, 4))

    # member: canid, type: uint32 (primitive), offset: 16, size: 4
    obj.canid = read_primitive('uint32', binary_data.byteslice(base_off + 16, 4))

    obj
  end

  def self.binary_write_recursive_hako_can_head(parent_off, heap_data, allocator, obj)
    # member: channel, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.channel, 4))

    # member: ide, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.ide, 4))

    # member: rtr, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.rtr, 4))

    # member: dlc, type: uint32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('uint32', obj.dlc, 4))

    # member: canid, type: uint32 (primitive), offset: 16, size: 4
    add_binary(allocator, parent_off + 16, primitive_to_bin('uint32', obj.canid, 4))
