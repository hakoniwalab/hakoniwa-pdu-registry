defmodule HakoPdu.SensorMsgs.ImageConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_image(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.Image.new()
    binary_read_recursive_image(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_image(obj) do
    {base_data, heap_data} = binary_write_recursive_image(0, "", "", obj)
  ...
  def binary_read_recursive_image(meta, binary_data, obj, base_off) do
    ...
    encoding = read_primitive("string", binary_part(binary_data, base_off + 144, 128))

    # member: is_bigendian, type: uint8 (primitive), offset: 272, size: 1
    is_bigendian = read_primitive("uint8", binary_part(binary_data, base_off + 272, 1))

    # member: step, type: uint32 (primitive), offset: 276, size: 4
    step = read_primitive("uint32", binary_part(binary_data, base_off + 276, 4))

    # member: data, type: uint8 (primitive), offset: 280, size: 1
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 280, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 280 + 4, 4))
  ...
  def binary_write_recursive_image(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: encoding, type: string (primitive), offset: 144, size: 128
    allocator = add_binary(allocator, parent_off + 144, primitive_to_bin("string", obj.encoding, 128))

    # member: is_bigendian, type: uint8 (primitive), offset: 272, size: 1
    allocator = add_binary(allocator, parent_off + 272, primitive_to_bin("uint8", obj.is_bigendian, 1))

    # member: step, type: uint32 (primitive), offset: 276, size: 4
    allocator = add_binary(allocator, parent_off + 276, primitive_to_bin("uint32", obj.step, 4))

    # member: data, type: uint8 (primitive), offset: 280, size: 1
    offset_from_heap = byte_size(heap_data)
    values = obj.data || []
    allocator = add_binary(allocator, parent_off + 280, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
