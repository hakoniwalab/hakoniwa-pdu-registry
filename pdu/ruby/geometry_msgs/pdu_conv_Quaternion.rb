...
module HakoPdu::GeometryMsgs::QuaternionConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_quaternion(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Quaternion.new
    binary_read_recursive_quaternion(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_quaternion(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_quaternion(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_quaternion(meta, binary_data, obj, base_off)
    # member: x, type: float64 (primitive), offset: 0, size: 8
    obj.x = read_primitive('float64', binary_data.byteslice(base_off + 0, 8))

    # member: y, type: float64 (primitive), offset: 8, size: 8
    obj.y = read_primitive('float64', binary_data.byteslice(base_off + 8, 8))

    # member: z, type: float64 (primitive), offset: 16, size: 8
    obj.z = read_primitive('float64', binary_data.byteslice(base_off + 16, 8))

    # member: w, type: float64 (primitive), offset: 24, size: 8
    obj.w = read_primitive('float64', binary_data.byteslice(base_off + 24, 8))

    obj
  end

  def self.binary_write_recursive_quaternion(parent_off, heap_data, allocator, obj)
    # member: x, type: float64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('float64', obj.x, 8))

    # member: y, type: float64 (primitive), offset: 8, size: 8
    add_binary(allocator, parent_off + 8, primitive_to_bin('float64', obj.y, 8))

    # member: z, type: float64 (primitive), offset: 16, size: 8
    add_binary(allocator, parent_off + 16, primitive_to_bin('float64', obj.z, 8))

    # member: w, type: float64 (primitive), offset: 24, size: 8
    add_binary(allocator, parent_off + 24, primitive_to_bin('float64', obj.w, 8))
