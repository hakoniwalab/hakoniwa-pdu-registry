defmodule HakoPdu.SampleActionMsgs.FibonacciGoalConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_fibonacci_goal(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SampleActionMsgs.FibonacciGoal.new()
    binary_read_recursive_fibonacci_goal(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_fibonacci_goal(obj) do
    {base_data, heap_data} = binary_write_recursive_fibonacci_goal(0, "", "", obj)
  ...
  def binary_read_recursive_fibonacci_goal(meta, binary_data, obj, base_off) do
    # member: order, type: int32 (primitive), offset: 0, size: 4
    order = read_primitive("int32", binary_part(binary_data, base_off + 0, 4))
  ...
  def binary_write_recursive_fibonacci_goal(parent_off, heap_data, allocator, obj) do
    # member: order, type: int32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int32", obj.order, 4))
