...
module HakoPdu::HakoSrvMsgs::AttachRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_attach_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AttachRequest.new
    binary_read_recursive_attach_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_attach_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_attach_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_attach_request(meta, binary_data, obj, base_off)
    # member: asset_name, type: string (primitive), offset: 0, size: 128
    obj.asset_name = read_primitive('string', binary_data.byteslice(base_off + 0, 128))

    # member: delta_asset_tick, type: uint64 (primitive), offset: 128, size: 8
    obj.delta_asset_tick = read_primitive('uint64', binary_data.byteslice(base_off + 128, 8))

    obj
  end

  def self.binary_write_recursive_attach_request(parent_off, heap_data, allocator, obj)
    # member: asset_name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.asset_name, 128))

    # member: delta_asset_tick, type: uint64 (primitive), offset: 128, size: 8
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint64', obj.delta_asset_tick, 8))
