require_relative 'pdu_type_HakoCameraInfo'
require_relative '../geometry_msgs/pdu_type_Vector3'
require_relative '../geometry_msgs/pdu_conv_Vector3'


module HakoPdu::HakoMsgs::HakoCameraInfoConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_camera_info(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoCameraInfo.new
    binary_read_recursive_hako_camera_info(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_camera_info(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_camera_info(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_camera_info(meta, binary_data, obj, base_off)
    ...
    # member: angle, type: geometry_msgs/Vector3 (struct), offset: 8, size: 24
    obj.angle = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.angle, base_off + 8)

    obj
  end

  def self.binary_write_recursive_hako_camera_info(parent_off, heap_data, allocator, obj)
    # member: request_id, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.request_id, 4))

    # member: angle, type: geometry_msgs/Vector3 (struct), offset: 8, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 8, heap_data, allocator, obj.angle)
