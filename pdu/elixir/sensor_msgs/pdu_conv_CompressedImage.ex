defmodule HakoPdu.SensorMsgs.CompressedImageConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_compressed_image(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.CompressedImage.new()
    binary_read_recursive_compressed_image(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_compressed_image(obj) do
    {base_data, heap_data} = binary_write_recursive_compressed_image(0, "", "", obj)
  ...
  def binary_read_recursive_compressed_image(meta, binary_data, obj, base_off) do
    ...
    # member: format, type: string (primitive), offset: 136, size: 128
    format = read_primitive("string", binary_part(binary_data, base_off + 136, 128))

    # member: data, type: uint8 (primitive), offset: 264, size: 1
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 264, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 264 + 4, 4))
  ...
  def binary_write_recursive_compressed_image(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: format, type: string (primitive), offset: 136, size: 128
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("string", obj.format, 128))

    # member: data, type: uint8 (primitive), offset: 264, size: 1
    offset_from_heap = byte_size(heap_data)
    values = obj.data || []
    allocator = add_binary(allocator, parent_off + 264, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
