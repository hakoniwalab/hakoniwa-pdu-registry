...
module HakoPdu::DroneSrvMsgs::CameraCaptureImageRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_camera_capture_image_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::CameraCaptureImageRequest.new
    binary_read_recursive_camera_capture_image_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_camera_capture_image_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_camera_capture_image_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_camera_capture_image_request(meta, binary_data, obj, base_off)
    ...
    obj.image_type = read_primitive('string', binary_data.byteslice(base_off + 128, 128))

    obj
  end

  def self.binary_write_recursive_camera_capture_image_request(parent_off, heap_data, allocator, obj)
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.drone_name, 128))

    # member: image_type, type: string (primitive), offset: 128, size: 128
    add_binary(allocator, parent_off + 128, primitive_to_bin('string', obj.image_type, 128))
