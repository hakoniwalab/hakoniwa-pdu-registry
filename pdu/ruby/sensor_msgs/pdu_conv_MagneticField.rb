require_relative 'pdu_type_MagneticField'
...
module HakoPdu::SensorMsgs::MagneticFieldConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_magnetic_field(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::MagneticField.new
    binary_read_recursive_magnetic_field(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_magnetic_field(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_magnetic_field(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_magnetic_field(meta, binary_data, obj, base_off)
    ...
    # member: magnetic_field, type: geometry_msgs/Vector3 (struct), offset: 136, size: 24
    obj.magnetic_field = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.magnetic_field, base_off + 136)

    # member: magnetic_field_covariance, type: float64 (primitive), offset: 160, size: 72
    obj.magnetic_field_covariance = read_primitive_array('float64', binary_data.byteslice(base_off + 160, 72), 9, 72 / 9)

    obj
  end

  def self.binary_write_recursive_magnetic_field(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: magnetic_field, type: geometry_msgs/Vector3 (struct), offset: 136, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 136, heap_data, allocator, obj.magnetic_field)

    # member: magnetic_field_covariance, type: float64 (primitive), offset: 160, size: 72
    add_binary(allocator, parent_off + 160, primitive_array_to_bin('float64', obj.magnetic_field_covariance, 72 / 9))
