require_relative 'pdu_type_Empty'


module HakoPdu::StdMsgs::EmptyConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_empty(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Empty.new
    binary_read_recursive_empty(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_empty(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_empty(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_empty(meta, binary_data, obj, base_off)
    # member: hako_dummy, type: uint8 (primitive), offset: 0, size: 1
    obj.hako_dummy = read_primitive('uint8', binary_data.byteslice(base_off + 0, 1))

    obj
  end

  def self.binary_write_recursive_empty(parent_off, heap_data, allocator, obj)
    # member: hako_dummy, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.hako_dummy, 1))
