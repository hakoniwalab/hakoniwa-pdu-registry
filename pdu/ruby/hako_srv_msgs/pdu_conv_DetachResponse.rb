...
module HakoPdu::HakoSrvMsgs::DetachResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_detach_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::DetachResponse.new
    binary_read_recursive_detach_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_detach_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_detach_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_detach_response(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_detach_response(parent_off, heap_data, allocator, obj)
    # member: result_code, type: uint32 (primitive), offset: 0, size: 4
