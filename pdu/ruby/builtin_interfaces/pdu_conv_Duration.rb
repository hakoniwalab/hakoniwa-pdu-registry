require_relative 'pdu_type_Duration'


module HakoPdu::BuiltinInterfaces::DurationConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_duration(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::BuiltinInterfaces::Duration.new
    binary_read_recursive_duration(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_duration(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_duration(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_duration(meta, binary_data, obj, base_off)
    # member: sec, type: int32 (primitive), offset: 0, size: 4
    obj.sec = read_primitive('int32', binary_data.byteslice(base_off + 0, 4))

    # member: nanosec, type: uint32 (primitive), offset: 4, size: 4
    obj.nanosec = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    obj
  end

  def self.binary_write_recursive_duration(parent_off, heap_data, allocator, obj)
    # member: sec, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.sec, 4))

    # member: nanosec, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.nanosec, 4))
