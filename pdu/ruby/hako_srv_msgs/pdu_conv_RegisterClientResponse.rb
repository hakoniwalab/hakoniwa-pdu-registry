...
module HakoPdu::HakoSrvMsgs::RegisterClientResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_register_client_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::RegisterClientResponse.new
    binary_read_recursive_register_client_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_register_client_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_register_client_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_register_client_response(meta, binary_data, obj, base_off)
    # member: service_id, type: uint32 (primitive), offset: 0, size: 4
    obj.service_id = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: client_id, type: uint32 (primitive), offset: 4, size: 4
    obj.client_id = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: request_channel_id, type: uint32 (primitive), offset: 8, size: 4
    obj.request_channel_id = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))

    # member: response_channel_id, type: uint32 (primitive), offset: 12, size: 4
    obj.response_channel_id = read_primitive('uint32', binary_data.byteslice(base_off + 12, 4))

    obj
  end

  def self.binary_write_recursive_register_client_response(parent_off, heap_data, allocator, obj)
    # member: service_id, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.service_id, 4))

    # member: client_id, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.client_id, 4))

    # member: request_channel_id, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.request_channel_id, 4))

    # member: response_channel_id, type: uint32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('uint32', obj.response_channel_id, 4))
