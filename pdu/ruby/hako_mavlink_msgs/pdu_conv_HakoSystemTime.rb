require_relative 'pdu_type_HakoSystemTime'


module HakoPdu::HakoMavlinkMsgs::HakoSystemTimeConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_system_time(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMavlinkMsgs::HakoSystemTime.new
    binary_read_recursive_hako_system_time(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_system_time(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_system_time(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_system_time(meta, binary_data, obj, base_off)
    # member: time_unix_usec, type: uint64 (primitive), offset: 0, size: 8
    obj.time_unix_usec = read_primitive('uint64', binary_data.byteslice(base_off + 0, 8))

    # member: time_boot_ms, type: uint64 (primitive), offset: 8, size: 8
    obj.time_boot_ms = read_primitive('uint64', binary_data.byteslice(base_off + 8, 8))

    obj
  end

  def self.binary_write_recursive_hako_system_time(parent_off, heap_data, allocator, obj)
    # member: time_unix_usec, type: uint64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint64', obj.time_unix_usec, 8))

    # member: time_boot_ms, type: uint64 (primitive), offset: 8, size: 8
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint64', obj.time_boot_ms, 8))
