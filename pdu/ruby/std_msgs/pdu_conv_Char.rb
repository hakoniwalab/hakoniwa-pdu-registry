require_relative 'pdu_type_Char'


module HakoPdu::StdMsgs::CharConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_char(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Char.new
    binary_read_recursive_char(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_char(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_char(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_char(meta, binary_data, obj, base_off)
    # member: data, type: char (primitive), offset: 0, size: 1
    obj.data = read_primitive('char', binary_data.byteslice(base_off + 0, 1))

    obj
  end

  def self.binary_write_recursive_char(parent_off, heap_data, allocator, obj)
    # member: data, type: char (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('char', obj.data, 1))
