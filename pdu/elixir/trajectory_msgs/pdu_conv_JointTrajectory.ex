defmodule HakoPdu.TrajectoryMsgs.JointTrajectoryConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joint_trajectory(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.TrajectoryMsgs.JointTrajectory.new()
    binary_read_recursive_joint_trajectory(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joint_trajectory(obj) do
    {base_data, heap_data} = binary_write_recursive_joint_trajectory(0, "", "", obj)
  ...
  def binary_read_recursive_joint_trajectory(meta, binary_data, obj, base_off) do
    ...
    # member: points, type: JointTrajectoryPoint (struct), offset: 144, size: 40
    ...
    points =
      ...
      |> Enum.map(fn i ->
        HakoPdu.TrajectoryMsgs.JointTrajectoryPointConverter.binary_read_recursive_joint_trajectory_point(
          meta,
          binary_data,
          HakoPdu.TrajectoryMsgs.JointTrajectoryPoint.new(),
          meta.heap_off + offset_from_heap + i * 40
        )
      end)
  ...
  def binary_write_recursive_joint_trajectory(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 40))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.TrajectoryMsgs.JointTrajectoryPointConverter.binary_write_recursive_joint_trajectory_point(
            offset_from_heap + i * 40,
            acc_heap,
            acc_heap,
            child
          )
