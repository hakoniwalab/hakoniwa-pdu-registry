defmodule HakoPdu.SensorMsgs.JoyFeedbackConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joy_feedback(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.JoyFeedback.new()
    binary_read_recursive_joy_feedback(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joy_feedback(obj) do
    {base_data, heap_data} = binary_write_recursive_joy_feedback(0, "", "", obj)
  ...
  def binary_read_recursive_joy_feedback(meta, binary_data, obj, base_off) do
    ...
    id = read_primitive("uint8", binary_part(binary_data, base_off + 1, 1))

    # member: intensity, type: float32 (primitive), offset: 4, size: 4
    intensity = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))
  ...
  def binary_write_recursive_joy_feedback(parent_off, heap_data, allocator, obj) do
    # member: type, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.type, 1))

    # member: id, type: uint8 (primitive), offset: 1, size: 1
    allocator = add_binary(allocator, parent_off + 1, primitive_to_bin("uint8", obj.id, 1))

    # member: intensity, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.intensity, 4))
