require_relative 'pdu_type_AttitudeTarget'
...
module HakoPdu::MavrosMsgs::AttitudeTargetConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_attitude_target(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::MavrosMsgs::AttitudeTarget.new
    binary_read_recursive_attitude_target(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_attitude_target(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_attitude_target(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_attitude_target(meta, binary_data, obj, base_off)
    ...
    obj.type_mask = read_primitive('uint8', binary_data.byteslice(base_off + 136, 1))
    ...
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_read_recursive_quaternion(meta, binary_data, obj.orientation, base_off + 144)

    # member: body_rate, type: geometry_msgs/Vector3 (struct), offset: 176, size: 24
    obj.body_rate = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.body_rate, base_off + 176)

    # member: thrust, type: float32 (primitive), offset: 200, size: 4
    obj.thrust = read_primitive('float32', binary_data.byteslice(base_off + 200, 4))

    obj
  end

  def self.binary_write_recursive_attitude_target(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: type_mask, type: uint8 (primitive), offset: 136, size: 1
    add_binary(allocator, parent_off + 136, primitive_to_bin('uint8', obj.type_mask, 1))

    # member: orientation, type: geometry_msgs/Quaternion (struct), offset: 144, size: 32
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_write_recursive_quaternion(parent_off + 144, heap_data, allocator, obj.orientation)

    # member: body_rate, type: geometry_msgs/Vector3 (struct), offset: 176, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 176, heap_data, allocator, obj.body_rate)

    # member: thrust, type: float32 (primitive), offset: 200, size: 4
    add_binary(allocator, parent_off + 200, primitive_to_bin('float32', obj.thrust, 4))
