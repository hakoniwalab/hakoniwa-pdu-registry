require_relative 'pdu_type_HakoCmdCameraMove'
...
module HakoPdu::HakoMsgs::HakoCmdCameraMoveConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_cmd_camera_move(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoCmdCameraMove.new
    binary_read_recursive_hako_cmd_camera_move(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_cmd_camera_move(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_cmd_camera_move(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_cmd_camera_move(meta, binary_data, obj, base_off)
    ...
    # member: angle, type: geometry_msgs/Vector3 (struct), offset: 16, size: 24
    obj.angle = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.angle, base_off + 16)

    obj
  end

  def self.binary_write_recursive_hako_cmd_camera_move(parent_off, heap_data, allocator, obj)
    # member: header, type: HakoCmdHeader (struct), offset: 0, size: 12
    ...
    # member: angle, type: geometry_msgs/Vector3 (struct), offset: 16, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 16, heap_data, allocator, obj.angle)
