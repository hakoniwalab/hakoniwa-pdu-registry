defmodule HakoPdu.StdMsgs.ColorRGBAConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_color_r_g_b_a(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.ColorRGBA.new()
    binary_read_recursive_color_r_g_b_a(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_color_r_g_b_a(obj) do
    {base_data, heap_data} = binary_write_recursive_color_r_g_b_a(0, "", "", obj)
  ...
  def binary_read_recursive_color_r_g_b_a(meta, binary_data, obj, base_off) do
    # member: r, type: float32 (primitive), offset: 0, size: 4
    r = read_primitive("float32", binary_part(binary_data, base_off + 0, 4))

    # member: g, type: float32 (primitive), offset: 4, size: 4
    g = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))

    # member: b, type: float32 (primitive), offset: 8, size: 4
    b = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: a, type: float32 (primitive), offset: 12, size: 4
    a = read_primitive("float32", binary_part(binary_data, base_off + 12, 4))
  ...
  def binary_write_recursive_color_r_g_b_a(parent_off, heap_data, allocator, obj) do
    # member: r, type: float32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float32", obj.r, 4))

    # member: g, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.g, 4))

    # member: b, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.b, 4))

    # member: a, type: float32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("float32", obj.a, 4))
