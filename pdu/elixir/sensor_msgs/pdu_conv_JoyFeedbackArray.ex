defmodule HakoPdu.SensorMsgs.JoyFeedbackArrayConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joy_feedback_array(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.JoyFeedbackArray.new()
    binary_read_recursive_joy_feedback_array(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joy_feedback_array(obj) do
    {base_data, heap_data} = binary_write_recursive_joy_feedback_array(0, "", "", obj)
  ...
  def binary_read_recursive_joy_feedback_array(meta, binary_data, obj, base_off) do
    ...
    array =
      ...
      |> Enum.map(fn i ->
        HakoPdu.SensorMsgs.JoyFeedbackConverter.binary_read_recursive_joy_feedback(
          meta,
          binary_data,
          HakoPdu.SensorMsgs.JoyFeedback.new(),
  ...
  def binary_write_recursive_joy_feedback_array(parent_off, heap_data, allocator, obj) do
    # member: array, type: JoyFeedback (struct), offset: 0, size: 8
    ...
    heap_data =
      ...
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.SensorMsgs.JoyFeedbackConverter.binary_write_recursive_joy_feedback(
