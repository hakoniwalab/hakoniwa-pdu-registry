require_relative 'pdu_type_RegisterClientRequestPacket'
require_relative 'pdu_type_ServiceRequestHeader'
require_relative 'pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_RegisterClientRequest'
require_relative 'pdu_conv_RegisterClientRequest'


module HakoPdu::HakoSrvMsgs::RegisterClientRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_register_client_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::RegisterClientRequestPacket.new
    binary_read_recursive_register_client_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_register_client_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_register_client_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_register_client_request_packet(meta, binary_data, obj, base_off)
    ...
    obj.body = HakoPdu::HakoSrvMsgs::RegisterClientRequest.new
    HakoPdu::HakoSrvMsgs::RegisterClientRequestConverter.binary_read_recursive_register_client_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_register_client_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: RegisterClientRequest (struct), offset: 268, size: 4
    HakoPdu::HakoSrvMsgs::RegisterClientRequestConverter.binary_write_recursive_register_client_request(parent_off + 268, heap_data, allocator, obj.body)
