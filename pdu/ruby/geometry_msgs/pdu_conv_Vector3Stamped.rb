require_relative 'pdu_type_Vector3Stamped'
...
module HakoPdu::GeometryMsgs::Vector3StampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_vector3_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Vector3Stamped.new
    binary_read_recursive_vector3_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_vector3_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_vector3_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_vector3_stamped(meta, binary_data, obj, base_off)
    ...
    # member: vector, type: Vector3 (struct), offset: 136, size: 24
    obj.vector = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.vector, base_off + 136)

    obj
  end

  def self.binary_write_recursive_vector3_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: vector, type: Vector3 (struct), offset: 136, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 136, heap_data, allocator, obj.vector)
