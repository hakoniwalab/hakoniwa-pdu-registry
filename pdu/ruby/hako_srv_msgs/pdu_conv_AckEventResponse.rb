...
module HakoPdu::HakoSrvMsgs::AckEventResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ack_event_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AckEventResponse.new
    binary_read_recursive_ack_event_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ack_event_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ack_event_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ack_event_response(meta, binary_data, obj, base_off)
    # member: ack_code, type: uint32 (primitive), offset: 0, size: 4
    obj.ack_code = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    obj
  end

  def self.binary_write_recursive_ack_event_response(parent_off, heap_data, allocator, obj)
    # member: ack_code, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.ack_code, 4))
