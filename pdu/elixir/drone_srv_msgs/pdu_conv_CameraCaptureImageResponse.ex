defmodule HakoPdu.DroneSrvMsgs.CameraCaptureImageResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_camera_capture_image_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.CameraCaptureImageResponse.new()
    binary_read_recursive_camera_capture_image_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_camera_capture_image_response(obj) do
    {base_data, heap_data} = binary_write_recursive_camera_capture_image_response(0, "", "", obj)
  ...
  def binary_read_recursive_camera_capture_image_response(meta, binary_data, obj, base_off) do
    ...
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 4, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 4 + 4, 4))
  ...
  def binary_write_recursive_camera_capture_image_response(parent_off, heap_data, allocator, obj) do
    # member: ok, type: bool (primitive), offset: 0, size: 4
    ...
    values = obj.data || []
    allocator = add_binary(allocator, parent_off + 4, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
