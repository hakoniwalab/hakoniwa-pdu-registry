require_relative 'pdu_type_Int8'


module HakoPdu::StdMsgs::Int8Converter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_int8(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Int8.new
    binary_read_recursive_int8(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_int8(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_int8(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_int8(meta, binary_data, obj, base_off)
    # member: data, type: int8 (primitive), offset: 0, size: 1
    obj.data = read_primitive('int8', binary_data.byteslice(base_off + 0, 1))

    obj
  end

  def self.binary_write_recursive_int8(parent_off, heap_data, allocator, obj)
    # member: data, type: int8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('int8', obj.data, 1))
