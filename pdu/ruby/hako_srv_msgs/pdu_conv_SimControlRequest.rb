...
module HakoPdu::HakoSrvMsgs::SimControlRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_sim_control_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::SimControlRequest.new
    binary_read_recursive_sim_control_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_sim_control_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_sim_control_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_sim_control_request(meta, binary_data, obj, base_off)
    # member: name, type: string (primitive), offset: 0, size: 128
    obj.name = read_primitive('string', binary_data.byteslice(base_off + 0, 128))

    # member: op, type: uint32 (primitive), offset: 128, size: 4
    obj.op = read_primitive('uint32', binary_data.byteslice(base_off + 128, 4))

    obj
  end

  def self.binary_write_recursive_sim_control_request(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.name, 128))

    # member: op, type: uint32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint32', obj.op, 4))
