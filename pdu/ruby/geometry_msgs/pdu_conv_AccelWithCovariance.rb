require_relative 'pdu_type_AccelWithCovariance'
require_relative 'pdu_type_Accel'
require_relative 'pdu_conv_Accel'


module HakoPdu::GeometryMsgs::AccelWithCovarianceConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_accel_with_covariance(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::AccelWithCovariance.new
    binary_read_recursive_accel_with_covariance(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_accel_with_covariance(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_accel_with_covariance(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_accel_with_covariance(meta, binary_data, obj, base_off)
    # member: accel, type: Accel (struct), offset: 0, size: 48
    obj.accel = HakoPdu::GeometryMsgs::Accel.new
    HakoPdu::GeometryMsgs::AccelConverter.binary_read_recursive_accel(meta, binary_data, obj.accel, base_off + 0)

    # member: covariance, type: float64 (primitive), offset: 48, size: 288
    obj.covariance = read_primitive_array('float64', binary_data.byteslice(base_off + 48, 288), 36, 288 / 36)

    obj
  end

  def self.binary_write_recursive_accel_with_covariance(parent_off, heap_data, allocator, obj)
    # member: accel, type: Accel (struct), offset: 0, size: 48
    HakoPdu::GeometryMsgs::AccelConverter.binary_write_recursive_accel(parent_off + 0, heap_data, allocator, obj.accel)

    # member: covariance, type: float64 (primitive), offset: 48, size: 288
    add_binary(allocator, parent_off + 48, primitive_array_to_bin('float64', obj.covariance, 288 / 36))
