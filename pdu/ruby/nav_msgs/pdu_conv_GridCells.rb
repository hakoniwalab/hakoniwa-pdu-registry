require_relative 'pdu_type_GridCells'
...
module HakoPdu::NavMsgs::GridCellsConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_grid_cells(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::NavMsgs::GridCells.new
    binary_read_recursive_grid_cells(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_grid_cells(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_grid_cells(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_grid_cells(meta, binary_data, obj, base_off)
    ...
    # member: cell_width, type: float32 (primitive), offset: 136, size: 4
    obj.cell_width = read_primitive('float32', binary_data.byteslice(base_off + 136, 4))

    # member: cell_height, type: float32 (primitive), offset: 140, size: 4
    obj.cell_height = read_primitive('float32', binary_data.byteslice(base_off + 140, 4))

    # member: cells, type: geometry_msgs/Point (struct), offset: 144, size: 24
    ...
    array_size.times do |i|
      ...
      HakoPdu::GeometryMsgs::PointConverter.binary_read_recursive_point(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 24)
      obj.cells << child
    end
  ...
  def self.binary_write_recursive_grid_cells(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: cell_width, type: float32 (primitive), offset: 136, size: 4
    add_binary(allocator, parent_off + 136, primitive_to_bin('float32', obj.cell_width, 4))

    # member: cell_height, type: float32 (primitive), offset: 140, size: 4
    add_binary(allocator, parent_off + 140, primitive_to_bin('float32', obj.cell_height, 4))
    ...
    values.each_with_index do |child, i|
      HakoPdu::GeometryMsgs::PointConverter.binary_write_recursive_point(offset_from_heap + i * 24, heap_data, heap_data, child)
    end
