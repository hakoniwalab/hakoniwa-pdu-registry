require_relative 'pdu_type_Wrench'
require_relative 'pdu_type_Vector3'
require_relative 'pdu_conv_Vector3'


module HakoPdu::GeometryMsgs::WrenchConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_wrench(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Wrench.new
    binary_read_recursive_wrench(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_wrench(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_wrench(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_wrench(meta, binary_data, obj, base_off)
    # member: force, type: Vector3 (struct), offset: 0, size: 24
    obj.force = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.force, base_off + 0)

    # member: torque, type: Vector3 (struct), offset: 24, size: 24
    obj.torque = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.torque, base_off + 24)

    obj
  end

  def self.binary_write_recursive_wrench(parent_off, heap_data, allocator, obj)
    # member: force, type: Vector3 (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 0, heap_data, allocator, obj.force)

    # member: torque, type: Vector3 (struct), offset: 24, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 24, heap_data, allocator, obj.torque)
