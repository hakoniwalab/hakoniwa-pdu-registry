require_relative 'pdu_type_AckEventRequestPacket'
...
require_relative 'pdu_conv_AckEventRequest'


module HakoPdu::HakoSrvMsgs::AckEventRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ack_event_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AckEventRequestPacket.new
    binary_read_recursive_ack_event_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ack_event_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ack_event_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ack_event_request_packet(meta, binary_data, obj, base_off)
    ...
    obj.body = HakoPdu::HakoSrvMsgs::AckEventRequest.new
    HakoPdu::HakoSrvMsgs::AckEventRequestConverter.binary_read_recursive_ack_event_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_ack_event_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: AckEventRequest (struct), offset: 268, size: 136
    HakoPdu::HakoSrvMsgs::AckEventRequestConverter.binary_write_recursive_ack_event_request(parent_off + 268, heap_data, allocator, obj.body)
