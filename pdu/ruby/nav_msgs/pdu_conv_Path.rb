require_relative 'pdu_type_Path'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative '../geometry_msgs/pdu_type_PoseStamped'
require_relative '../geometry_msgs/pdu_conv_PoseStamped'


module HakoPdu::NavMsgs::PathConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_path(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::NavMsgs::Path.new
    binary_read_recursive_path(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_path(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_path(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_path(meta, binary_data, obj, base_off)
    ...
    array_size.times do |i|
      child = HakoPdu::GeometryMsgs::PoseStamped.new
      HakoPdu::GeometryMsgs::PoseStampedConverter.binary_read_recursive_pose_stamped(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 192)
      obj.poses << child
    end
  ...
  def self.binary_write_recursive_path(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    add_binary(heap_data, offset_from_heap, "\x00".b * (values.length * 192))
    values.each_with_index do |child, i|
      HakoPdu::GeometryMsgs::PoseStampedConverter.binary_write_recursive_pose_stamped(offset_from_heap + i * 192, heap_data, heap_data, child)
    end
