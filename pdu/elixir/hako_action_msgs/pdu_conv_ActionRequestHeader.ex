defmodule HakoPdu.HakoActionMsgs.ActionRequestHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_action_request_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoActionMsgs.ActionRequestHeader.new()
    binary_read_recursive_action_request_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_action_request_header(obj) do
    {base_data, heap_data} = binary_write_recursive_action_request_header(0, "", "", obj)
  ...
  def binary_read_recursive_action_request_header(meta, binary_data, obj, base_off) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    version = read_primitive("uint8", binary_part(binary_data, base_off + 0, 1))

    # member: request_kind, type: uint8 (primitive), offset: 1, size: 1
    request_kind = read_primitive("uint8", binary_part(binary_data, base_off + 1, 1))

    # member: reserved, type: uint8 (primitive), offset: 2, size: 2
    reserved = read_primitive_array("uint8", binary_part(binary_data, base_off + 2, 2), 2, div(2, 2))
  ...
  def binary_write_recursive_action_request_header(parent_off, heap_data, allocator, obj) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.version, 1))

    # member: request_kind, type: uint8 (primitive), offset: 1, size: 1
    allocator = add_binary(allocator, parent_off + 1, primitive_to_bin("uint8", obj.request_kind, 1))

    # member: reserved, type: uint8 (primitive), offset: 2, size: 2
    allocator = add_binary(allocator, parent_off + 2, primitive_array_to_bin("uint8", obj.reserved, div(2, 2)))
