defmodule HakoPdu.HakoSrvMsgs.NotifyAssetTimeRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_notify_asset_time_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.NotifyAssetTimeRequest.new()
    binary_read_recursive_notify_asset_time_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_notify_asset_time_request(obj) do
    {base_data, heap_data} = binary_write_recursive_notify_asset_time_request(0, "", "", obj)
  ...
  def binary_read_recursive_notify_asset_time_request(meta, binary_data, obj, base_off) do
    # member: session_id, type: uint64 (primitive), offset: 0, size: 8
    session_id = read_primitive("uint64", binary_part(binary_data, base_off + 0, 8))
  ...
  def binary_write_recursive_notify_asset_time_request(parent_off, heap_data, allocator, obj) do
    # member: session_id, type: uint64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint64", obj.session_id, 8))
