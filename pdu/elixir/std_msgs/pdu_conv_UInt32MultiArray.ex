defmodule HakoPdu.StdMsgs.UInt32MultiArrayConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_u_int32_multi_array(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.UInt32MultiArray.new()
    binary_read_recursive_u_int32_multi_array(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_u_int32_multi_array(obj) do
    {base_data, heap_data} = binary_write_recursive_u_int32_multi_array(0, "", "", obj)
  ...
  def binary_read_recursive_u_int32_multi_array(meta, binary_data, obj, base_off) do
    ...
    # member: data, type: uint32 (primitive), offset: 12, size: 4
    ...
    data = read_primitive_array("uint32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)
  ...
  def binary_write_recursive_u_int32_multi_array(parent_off, heap_data, allocator, obj) do
    # member: layout, type: MultiArrayLayout (struct), offset: 0, size: 12
    {allocator, heap_data} =
    ...
    heap_data = add_binary(heap_data, offset_from_heap, primitive_array_to_bin("uint32", values, 4))
