defmodule HakoPdu.DroneSrvMsgs.DroneLandRequestPacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_land_request_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.DroneLandRequestPacket.new()
    binary_read_recursive_drone_land_request_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_land_request_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_land_request_packet(0, "", "", obj)
  ...
  def binary_read_recursive_drone_land_request_packet(meta, binary_data, obj, base_off) do
    ...
    body =
      HakoPdu.DroneSrvMsgs.DroneLandRequestConverter.binary_read_recursive_drone_land_request(
        meta,
        binary_data,
        HakoPdu.DroneSrvMsgs.DroneLandRequest.new(),
        base_off + 268
      )
  ...
  def binary_write_recursive_drone_land_request_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
    ...
    # member: body, type: DroneLandRequest (struct), offset: 268, size: 128
    {allocator, heap_data} =
      HakoPdu.DroneSrvMsgs.DroneLandRequestConverter.binary_write_recursive_drone_land_request(
        parent_off + 268,
        heap_data,
        allocator,
        obj.body
