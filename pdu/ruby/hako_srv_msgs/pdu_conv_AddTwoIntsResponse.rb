...
module HakoPdu::HakoSrvMsgs::AddTwoIntsResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_add_two_ints_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AddTwoIntsResponse.new
    binary_read_recursive_add_two_ints_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_add_two_ints_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_add_two_ints_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_add_two_ints_response(meta, binary_data, obj, base_off)
    # member: sum, type: int64 (primitive), offset: 0, size: 8
    obj.sum = read_primitive('int64', binary_data.byteslice(base_off + 0, 8))

    obj
  end

  def self.binary_write_recursive_add_two_ints_response(parent_off, heap_data, allocator, obj)
    # member: sum, type: int64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('int64', obj.sum, 8))
