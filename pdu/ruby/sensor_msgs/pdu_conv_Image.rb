require_relative 'pdu_type_Image'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'


module HakoPdu::SensorMsgs::ImageConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_image(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::Image.new
    binary_read_recursive_image(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_image(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_image(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_image(meta, binary_data, obj, base_off)
    ...
    # member: encoding, type: string (primitive), offset: 144, size: 128
    obj.encoding = read_primitive('string', binary_data.byteslice(base_off + 144, 128))

    # member: is_bigendian, type: uint8 (primitive), offset: 272, size: 1
    obj.is_bigendian = read_primitive('uint8', binary_data.byteslice(base_off + 272, 1))

    # member: step, type: uint32 (primitive), offset: 276, size: 4
    obj.step = read_primitive('uint32', binary_data.byteslice(base_off + 276, 4))

    # member: data, type: uint8 (primitive), offset: 280, size: 1
    array_size = read_primitive('int32', binary_data.byteslice(base_off + 280, 4))
    offset_from_heap = read_primitive('int32', binary_data.byteslice(base_off + 280 + 4, 4))
    obj.data = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    obj
  end

  def self.binary_write_recursive_image(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    # member: encoding, type: string (primitive), offset: 144, size: 128
    add_binary(allocator, parent_off + 144, primitive_to_bin('string', obj.encoding, 128))

    # member: is_bigendian, type: uint8 (primitive), offset: 272, size: 1
    add_binary(allocator, parent_off + 272, primitive_to_bin('uint8', obj.is_bigendian, 1))

    # member: step, type: uint32 (primitive), offset: 276, size: 4
    add_binary(allocator, parent_off + 276, primitive_to_bin('uint32', obj.step, 4))

    # member: data, type: uint8 (primitive), offset: 280, size: 1
    offset_from_heap = heap_data.bytesize
    values = obj.data || []
    add_binary(allocator, parent_off + 280, [values.length, offset_from_heap].pack('l<l<'))
