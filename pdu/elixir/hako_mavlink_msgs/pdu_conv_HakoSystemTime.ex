defmodule HakoPdu.HakoMavlinkMsgs.HakoSystemTimeConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_system_time(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMavlinkMsgs.HakoSystemTime.new()
    binary_read_recursive_hako_system_time(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_system_time(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_system_time(0, "", "", obj)
  ...
  def binary_read_recursive_hako_system_time(meta, binary_data, obj, base_off) do
    # member: time_unix_usec, type: uint64 (primitive), offset: 0, size: 8
    time_unix_usec = read_primitive("uint64", binary_part(binary_data, base_off + 0, 8))

    # member: time_boot_ms, type: uint64 (primitive), offset: 8, size: 8
    time_boot_ms = read_primitive("uint64", binary_part(binary_data, base_off + 8, 8))
  ...
  def binary_write_recursive_hako_system_time(parent_off, heap_data, allocator, obj) do
    # member: time_unix_usec, type: uint64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint64", obj.time_unix_usec, 8))

    # member: time_boot_ms, type: uint64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint64", obj.time_boot_ms, 8))
