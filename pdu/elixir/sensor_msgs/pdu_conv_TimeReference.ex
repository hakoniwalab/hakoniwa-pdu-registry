defmodule HakoPdu.SensorMsgs.TimeReferenceConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_time_reference(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.TimeReference.new()
    binary_read_recursive_time_reference(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_time_reference(obj) do
    {base_data, heap_data} = binary_write_recursive_time_reference(0, "", "", obj)
  ...
  def binary_read_recursive_time_reference(meta, binary_data, obj, base_off) do
    ...
    source = read_primitive("string", binary_part(binary_data, base_off + 144, 128))
  ...
  def binary_write_recursive_time_reference(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: source, type: string (primitive), offset: 144, size: 128
    allocator = add_binary(allocator, parent_off + 144, primitive_to_bin("string", obj.source, 128))
