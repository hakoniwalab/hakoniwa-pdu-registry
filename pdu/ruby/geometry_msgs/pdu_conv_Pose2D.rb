require_relative 'pdu_type_Pose2D'


module HakoPdu::GeometryMsgs::Pose2DConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_pose2_d(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Pose2D.new
    binary_read_recursive_pose2_d(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_pose2_d(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_pose2_d(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_pose2_d(meta, binary_data, obj, base_off)
    ...
    obj.theta = read_primitive('float64', binary_data.byteslice(base_off + 16, 8))

    obj
  end

  def self.binary_write_recursive_pose2_d(parent_off, heap_data, allocator, obj)
    # member: x, type: float64 (primitive), offset: 0, size: 8
    ...
    # member: theta, type: float64 (primitive), offset: 16, size: 8
    add_binary(allocator, parent_off + 16, primitive_to_bin('float64', obj.theta, 8))
