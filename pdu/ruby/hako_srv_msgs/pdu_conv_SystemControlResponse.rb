...
module HakoPdu::HakoSrvMsgs::SystemControlResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_system_control_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::SystemControlResponse.new
    binary_read_recursive_system_control_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_system_control_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_system_control_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_system_control_response(meta, binary_data, obj, base_off)
    # member: status_code, type: uint32 (primitive), offset: 0, size: 4
    obj.status_code = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: message, type: string (primitive), offset: 4, size: 128
    obj.message = read_primitive('string', binary_data.byteslice(base_off + 4, 128))

    obj
  end

  def self.binary_write_recursive_system_control_response(parent_off, heap_data, allocator, obj)
    # member: status_code, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.status_code, 4))

    # member: message, type: string (primitive), offset: 4, size: 128
    add_binary(allocator, parent_off + 4, primitive_to_bin('string', obj.message, 128))
