require_relative 'pdu_type_AccelWithCovarianceStamped'
...
require_relative 'pdu_conv_AccelWithCovariance'


module HakoPdu::GeometryMsgs::AccelWithCovarianceStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_accel_with_covariance_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::AccelWithCovarianceStamped.new
    binary_read_recursive_accel_with_covariance_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_accel_with_covariance_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_accel_with_covariance_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_accel_with_covariance_stamped(meta, binary_data, obj, base_off)
    ...
    # member: accel, type: AccelWithCovariance (struct), offset: 136, size: 336
    obj.accel = HakoPdu::GeometryMsgs::AccelWithCovariance.new
    HakoPdu::GeometryMsgs::AccelWithCovarianceConverter.binary_read_recursive_accel_with_covariance(meta, binary_data, obj.accel, base_off + 136)

    obj
  end

  def self.binary_write_recursive_accel_with_covariance_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: accel, type: AccelWithCovariance (struct), offset: 136, size: 336
    HakoPdu::GeometryMsgs::AccelWithCovarianceConverter.binary_write_recursive_accel_with_covariance(parent_off + 136, heap_data, allocator, obj.accel)
