defmodule HakoPdu.HakoSrvMsgs.AttachRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_attach_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.AttachRequest.new()
    binary_read_recursive_attach_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_attach_request(obj) do
    {base_data, heap_data} = binary_write_recursive_attach_request(0, "", "", obj)
  ...
  def binary_read_recursive_attach_request(meta, binary_data, obj, base_off) do
    # member: asset_name, type: string (primitive), offset: 0, size: 128
    asset_name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: delta_asset_tick, type: uint64 (primitive), offset: 128, size: 8
    delta_asset_tick = read_primitive("uint64", binary_part(binary_data, base_off + 128, 8))
  ...
  def binary_write_recursive_attach_request(parent_off, heap_data, allocator, obj) do
    # member: asset_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.asset_name, 128))

    # member: delta_asset_tick, type: uint64 (primitive), offset: 128, size: 8
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint64", obj.delta_asset_tick, 8))
