require_relative 'pdu_type_LiDARScanRequestPacket'
require_relative '../hako_srv_msgs/pdu_type_ServiceRequestHeader'
require_relative '../hako_srv_msgs/pdu_conv_ServiceRequestHeader'
require_relative 'pdu_type_LiDARScanRequest'
require_relative 'pdu_conv_LiDARScanRequest'


module HakoPdu::DroneSrvMsgs::LiDARScanRequestPacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_li_d_a_r_scan_request_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::LiDARScanRequestPacket.new
    binary_read_recursive_li_d_a_r_scan_request_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_li_d_a_r_scan_request_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_li_d_a_r_scan_request_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_li_d_a_r_scan_request_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: LiDARScanRequest (struct), offset: 268, size: 128
    obj.body = HakoPdu::DroneSrvMsgs::LiDARScanRequest.new
    HakoPdu::DroneSrvMsgs::LiDARScanRequestConverter.binary_read_recursive_li_d_a_r_scan_request(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_li_d_a_r_scan_request_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: LiDARScanRequest (struct), offset: 268, size: 128
    HakoPdu::DroneSrvMsgs::LiDARScanRequestConverter.binary_write_recursive_li_d_a_r_scan_request(parent_off + 268, heap_data, allocator, obj.body)
