require_relative 'pdu_type_AckEventRequest'


module HakoPdu::HakoSrvMsgs::AckEventRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ack_event_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AckEventRequest.new
    binary_read_recursive_ack_event_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ack_event_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ack_event_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ack_event_request(meta, binary_data, obj, base_off)
    ...
    obj.event_code = read_primitive('uint32', binary_data.byteslice(base_off + 128, 4))

    # member: result_code, type: uint32 (primitive), offset: 132, size: 4
    obj.result_code = read_primitive('uint32', binary_data.byteslice(base_off + 132, 4))

    obj
  end

  def self.binary_write_recursive_ack_event_request(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.name, 128))

    # member: event_code, type: uint32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint32', obj.event_code, 4))

    # member: result_code, type: uint32 (primitive), offset: 132, size: 4
    add_binary(allocator, parent_off + 132, primitive_to_bin('uint32', obj.result_code, 4))
