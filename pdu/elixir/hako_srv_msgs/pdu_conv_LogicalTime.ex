defmodule HakoPdu.HakoSrvMsgs.LogicalTimeConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_logical_time(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.LogicalTime.new()
    binary_read_recursive_logical_time(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_logical_time(obj) do
    {base_data, heap_data} = binary_write_recursive_logical_time(0, "", "", obj)
  ...
  def binary_read_recursive_logical_time(meta, binary_data, obj, base_off) do
    # member: generation, type: uint64 (primitive), offset: 0, size: 8
    generation = read_primitive("uint64", binary_part(binary_data, base_off + 0, 8))

    # member: offset_tick, type: uint64 (primitive), offset: 8, size: 8
    offset_tick = read_primitive("uint64", binary_part(binary_data, base_off + 8, 8))
  ...
  def binary_write_recursive_logical_time(parent_off, heap_data, allocator, obj) do
    # member: generation, type: uint64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint64", obj.generation, 8))

    # member: offset_tick, type: uint64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint64", obj.offset_tick, 8))
