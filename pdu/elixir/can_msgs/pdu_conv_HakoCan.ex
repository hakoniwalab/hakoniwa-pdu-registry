defmodule HakoPdu.CanMsgs.HakoCanConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_can(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.CanMsgs.HakoCan.new()
    binary_read_recursive_hako_can(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_can(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_can(0, "", "", obj)
  ...
  def binary_read_recursive_hako_can(meta, binary_data, obj, base_off) do
    # member: head, type: HakoCanHead (struct), offset: 0, size: 20
    head =
      HakoPdu.CanMsgs.HakoCanHeadConverter.binary_read_recursive_hako_can_head(
        meta,
        binary_data,
        HakoPdu.CanMsgs.HakoCanHead.new(),
        base_off + 0
      )

    # member: body, type: HakoCanBody (struct), offset: 20, size: 8
    body =
      HakoPdu.CanMsgs.HakoCanBodyConverter.binary_read_recursive_hako_can_body(
        meta,
        binary_data,
        HakoPdu.CanMsgs.HakoCanBody.new(),
        base_off + 20
      )
  ...
  def binary_write_recursive_hako_can(parent_off, heap_data, allocator, obj) do
    # member: head, type: HakoCanHead (struct), offset: 0, size: 20
    {allocator, heap_data} =
      HakoPdu.CanMsgs.HakoCanHeadConverter.binary_write_recursive_hako_can_head(
        parent_off + 0,
        heap_data,
        allocator,
        obj.head
    ...
    # member: body, type: HakoCanBody (struct), offset: 20, size: 8
    {allocator, heap_data} =
      HakoPdu.CanMsgs.HakoCanBodyConverter.binary_write_recursive_hako_can_body(
        parent_off + 20,
        heap_data,
        allocator,
        obj.body
