defmodule HakoPdu.Ev3Msgs.Ev3PduTouchSensorConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ev3_pdu_touch_sensor(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Ev3Msgs.Ev3PduTouchSensor.new()
    binary_read_recursive_ev3_pdu_touch_sensor(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ev3_pdu_touch_sensor(obj) do
    {base_data, heap_data} = binary_write_recursive_ev3_pdu_touch_sensor(0, "", "", obj)
  ...
  def binary_read_recursive_ev3_pdu_touch_sensor(meta, binary_data, obj, base_off) do
    # member: value, type: uint32 (primitive), offset: 0, size: 4
    value = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))
  ...
  def binary_write_recursive_ev3_pdu_touch_sensor(parent_off, heap_data, allocator, obj) do
    # member: value, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.value, 4))
