defmodule HakoPdu.HakoMsgs.HakoDroneCmdMoveConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_drone_cmd_move(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.HakoDroneCmdMove.new()
    binary_read_recursive_hako_drone_cmd_move(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_drone_cmd_move(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_drone_cmd_move(0, "", "", obj)
  ...
  def binary_read_recursive_hako_drone_cmd_move(meta, binary_data, obj, base_off) do
    # member: header, type: HakoDroneCmdHeader (struct), offset: 0, size: 12
    header =
      HakoPdu.HakoMsgs.HakoDroneCmdHeaderConverter.binary_read_recursive_hako_drone_cmd_header(
        meta,
        binary_data,
        HakoPdu.HakoMsgs.HakoDroneCmdHeader.new(),
        base_off + 0
      )

    # member: x, type: float64 (primitive), offset: 16, size: 8
    x = read_primitive("float64", binary_part(binary_data, base_off + 16, 8))

    # member: y, type: float64 (primitive), offset: 24, size: 8
    y = read_primitive("float64", binary_part(binary_data, base_off + 24, 8))

    # member: z, type: float64 (primitive), offset: 32, size: 8
    z = read_primitive("float64", binary_part(binary_data, base_off + 32, 8))

    # member: speed, type: float64 (primitive), offset: 40, size: 8
    speed = read_primitive("float64", binary_part(binary_data, base_off + 40, 8))

    # member: yaw_deg, type: float64 (primitive), offset: 48, size: 8
    yaw_deg = read_primitive("float64", binary_part(binary_data, base_off + 48, 8))
  ...
  def binary_write_recursive_hako_drone_cmd_move(parent_off, heap_data, allocator, obj) do
    # member: header, type: HakoDroneCmdHeader (struct), offset: 0, size: 12
    {allocator, heap_data} =
      HakoPdu.HakoMsgs.HakoDroneCmdHeaderConverter.binary_write_recursive_hako_drone_cmd_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: x, type: float64 (primitive), offset: 16, size: 8
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float64", obj.x, 8))

    # member: y, type: float64 (primitive), offset: 24, size: 8
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("float64", obj.y, 8))

    # member: z, type: float64 (primitive), offset: 32, size: 8
    allocator = add_binary(allocator, parent_off + 32, primitive_to_bin("float64", obj.z, 8))

    # member: speed, type: float64 (primitive), offset: 40, size: 8
    allocator = add_binary(allocator, parent_off + 40, primitive_to_bin("float64", obj.speed, 8))

    # member: yaw_deg, type: float64 (primitive), offset: 48, size: 8
    allocator = add_binary(allocator, parent_off + 48, primitive_to_bin("float64", obj.yaw_deg, 8))
