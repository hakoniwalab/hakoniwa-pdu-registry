defmodule HakoPdu.DroneSrvMsgs.MagnetGrabRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_magnet_grab_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.MagnetGrabRequest.new()
    binary_read_recursive_magnet_grab_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_magnet_grab_request(obj) do
    {base_data, heap_data} = binary_write_recursive_magnet_grab_request(0, "", "", obj)
  ...
  def binary_read_recursive_magnet_grab_request(meta, binary_data, obj, base_off) do
    ...
    # member: grab_on, type: bool (primitive), offset: 128, size: 4
    grab_on = read_primitive("bool", binary_part(binary_data, base_off + 128, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 132, size: 4
    timeout_sec = read_primitive("float32", binary_part(binary_data, base_off + 132, 4))
  ...
  def binary_write_recursive_magnet_grab_request(parent_off, heap_data, allocator, obj) do
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.drone_name, 128))

    # member: grab_on, type: bool (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("bool", obj.grab_on, 4))

    # member: timeout_sec, type: float32 (primitive), offset: 132, size: 4
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("float32", obj.timeout_sec, 4))
