defmodule HakoPdu.SensorMsgs.JoyConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_joy(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.Joy.new()
    binary_read_recursive_joy(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_joy(obj) do
    {base_data, heap_data} = binary_write_recursive_joy(0, "", "", obj)
  ...
  def binary_read_recursive_joy(meta, binary_data, obj, base_off) do
    ...
    # member: axes, type: float32 (primitive), offset: 136, size: 4
    ...
    axes = read_primitive_array("float32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)

    # member: buttons, type: int32 (primitive), offset: 144, size: 4
    ...
    buttons = read_primitive_array("int32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)
  ...
  def binary_write_recursive_joy(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
