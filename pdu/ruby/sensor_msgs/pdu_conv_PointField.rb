...
module HakoPdu::SensorMsgs::PointFieldConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_point_field(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::PointField.new
    binary_read_recursive_point_field(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_point_field(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_point_field(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_point_field(meta, binary_data, obj, base_off)
    ...
    obj.offset = read_primitive('uint32', binary_data.byteslice(base_off + 128, 4))

    # member: datatype, type: uint8 (primitive), offset: 132, size: 1
    obj.datatype = read_primitive('uint8', binary_data.byteslice(base_off + 132, 1))

    # member: count, type: uint32 (primitive), offset: 136, size: 4
    obj.count = read_primitive('uint32', binary_data.byteslice(base_off + 136, 4))

    obj
  end

  def self.binary_write_recursive_point_field(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.name, 128))

    # member: offset, type: uint32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('uint32', obj.offset, 4))

    # member: datatype, type: uint8 (primitive), offset: 132, size: 1
    add_binary(allocator, parent_off + 132, primitive_to_bin('uint8', obj.datatype, 1))

    # member: count, type: uint32 (primitive), offset: 136, size: 4
    add_binary(allocator, parent_off + 136, primitive_to_bin('uint32', obj.count, 4))
