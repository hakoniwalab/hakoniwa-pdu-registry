require_relative 'pdu_type_HakoCmdCamera'
require_relative 'pdu_type_HakoCmdHeader'
require_relative 'pdu_conv_HakoCmdHeader'


module HakoPdu::HakoMsgs::HakoCmdCameraConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_cmd_camera(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoCmdCamera.new
    binary_read_recursive_hako_cmd_camera(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_cmd_camera(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_cmd_camera(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_cmd_camera(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_hako_cmd_camera(parent_off, heap_data, allocator, obj)
    # member: header, type: HakoCmdHeader (struct), offset: 0, size: 12
