defmodule HakoPdu.HakoSrvMsgs.SystemControlResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_system_control_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.SystemControlResponse.new()
    binary_read_recursive_system_control_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_system_control_response(obj) do
    {base_data, heap_data} = binary_write_recursive_system_control_response(0, "", "", obj)
  ...
  def binary_read_recursive_system_control_response(meta, binary_data, obj, base_off) do
    # member: status_code, type: uint32 (primitive), offset: 0, size: 4
    status_code = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: message, type: string (primitive), offset: 4, size: 128
    message = read_primitive("string", binary_part(binary_data, base_off + 4, 128))
  ...
  def binary_write_recursive_system_control_response(parent_off, heap_data, allocator, obj) do
    # member: status_code, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.status_code, 4))

    # member: message, type: string (primitive), offset: 4, size: 128
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("string", obj.message, 128))
