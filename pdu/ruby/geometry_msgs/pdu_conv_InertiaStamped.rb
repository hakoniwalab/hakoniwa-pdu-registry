require_relative 'pdu_type_InertiaStamped'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_Inertia'
require_relative 'pdu_conv_Inertia'


module HakoPdu::GeometryMsgs::InertiaStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_inertia_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::InertiaStamped.new
    binary_read_recursive_inertia_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_inertia_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_inertia_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_inertia_stamped(meta, binary_data, obj, base_off)
    ...
    # member: inertia, type: Inertia (struct), offset: 136, size: 80
    obj.inertia = HakoPdu::GeometryMsgs::Inertia.new
    HakoPdu::GeometryMsgs::InertiaConverter.binary_read_recursive_inertia(meta, binary_data, obj.inertia, base_off + 136)

    obj
  end

  def self.binary_write_recursive_inertia_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: inertia, type: Inertia (struct), offset: 136, size: 80
    HakoPdu::GeometryMsgs::InertiaConverter.binary_write_recursive_inertia(parent_off + 136, heap_data, allocator, obj.inertia)
