defmodule HakoPdu.GeometryMsgs.PolygonStampedConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_polygon_stamped(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.PolygonStamped.new()
    binary_read_recursive_polygon_stamped(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_polygon_stamped(obj) do
    {base_data, heap_data} = binary_write_recursive_polygon_stamped(0, "", "", obj)
  ...
  def binary_read_recursive_polygon_stamped(meta, binary_data, obj, base_off) do
    ...
    polygon =
      HakoPdu.GeometryMsgs.PolygonConverter.binary_read_recursive_polygon(
        meta,
        binary_data,
        HakoPdu.GeometryMsgs.Polygon.new(),
        base_off + 136
      )
  ...
  def binary_write_recursive_polygon_stamped(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: polygon, type: Polygon (struct), offset: 136, size: 8
    {allocator, heap_data} =
      HakoPdu.GeometryMsgs.PolygonConverter.binary_write_recursive_polygon(
        parent_off + 136,
        heap_data,
        allocator,
        obj.polygon
