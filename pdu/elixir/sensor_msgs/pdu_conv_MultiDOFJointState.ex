defmodule HakoPdu.SensorMsgs.MultiDOFJointStateConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_multi_d_o_f_joint_state(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.MultiDOFJointState.new()
    binary_read_recursive_multi_d_o_f_joint_state(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_multi_d_o_f_joint_state(obj) do
    {base_data, heap_data} = binary_write_recursive_multi_d_o_f_joint_state(0, "", "", obj)
  ...
  def binary_read_recursive_multi_d_o_f_joint_state(meta, binary_data, obj, base_off) do
    ...
    # member: joint_names, type: string (primitive), offset: 136, size: 128
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 136, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 136 + 4, 4))
    joint_names = read_primitive_array("string", binary_part(binary_data, meta.heap_off + offset_from_heap, 128 * array_size), array_size, 128)

    # member: transforms, type: geometry_msgs/Transform (struct), offset: 144, size: 56
    ...
    transforms =
      ...
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.TransformConverter.binary_read_recursive_transform(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.Transform.new(),
          meta.heap_off + offset_from_heap + i * 56
        )
      end)

    # member: twist, type: geometry_msgs/Twist (struct), offset: 152, size: 48
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 152, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 152 + 4, 4))
    twist =
      range_indices(array_size)
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.TwistConverter.binary_read_recursive_twist(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.Twist.new(),
          meta.heap_off + offset_from_heap + i * 48
        )
      end)

    # member: wrench, type: geometry_msgs/Wrench (struct), offset: 160, size: 48
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 160, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 160 + 4, 4))
    wrench =
      range_indices(array_size)
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.WrenchConverter.binary_read_recursive_wrench(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.Wrench.new(),
          meta.heap_off + offset_from_heap + i * 48
        )
      end)
  ...
  def binary_write_recursive_multi_d_o_f_joint_state(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    values = obj.joint_names || []
    allocator = add_binary(allocator, parent_off + 136, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    ...
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 56))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.TransformConverter.binary_write_recursive_transform(
    ...
    offset_from_heap = byte_size(heap_data)
    values = obj.twist || []
    allocator = add_binary(allocator, parent_off + 152, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 48))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.TwistConverter.binary_write_recursive_twist(
    ...
    offset_from_heap = byte_size(heap_data)
    values = obj.wrench || []
    allocator = add_binary(allocator, parent_off + 160, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 48))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.WrenchConverter.binary_write_recursive_wrench(
            offset_from_heap + i * 48,
            acc_heap,
            acc_heap,
            child
