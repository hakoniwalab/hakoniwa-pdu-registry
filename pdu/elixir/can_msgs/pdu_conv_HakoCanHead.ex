defmodule HakoPdu.CanMsgs.HakoCanHeadConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_can_head(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.CanMsgs.HakoCanHead.new()
    binary_read_recursive_hako_can_head(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_can_head(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_can_head(0, "", "", obj)
  ...
  def binary_read_recursive_hako_can_head(meta, binary_data, obj, base_off) do
    # member: channel, type: uint32 (primitive), offset: 0, size: 4
    channel = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: ide, type: uint32 (primitive), offset: 4, size: 4
    ide = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: rtr, type: uint32 (primitive), offset: 8, size: 4
    rtr = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))

    # member: dlc, type: uint32 (primitive), offset: 12, size: 4
    dlc = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))

    # member: canid, type: uint32 (primitive), offset: 16, size: 4
    canid = read_primitive("uint32", binary_part(binary_data, base_off + 16, 4))
  ...
  def binary_write_recursive_hako_can_head(parent_off, heap_data, allocator, obj) do
    # member: channel, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.channel, 4))

    # member: ide, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.ide, 4))

    # member: rtr, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.rtr, 4))

    # member: dlc, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.dlc, 4))

    # member: canid, type: uint32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("uint32", obj.canid, 4))
