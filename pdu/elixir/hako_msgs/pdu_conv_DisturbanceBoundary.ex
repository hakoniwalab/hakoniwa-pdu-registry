defmodule HakoPdu.HakoMsgs.DisturbanceBoundaryConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_disturbance_boundary(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DisturbanceBoundary.new()
    binary_read_recursive_disturbance_boundary(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_disturbance_boundary(obj) do
    {base_data, heap_data} = binary_write_recursive_disturbance_boundary(0, "", "", obj)
  ...
  def binary_read_recursive_disturbance_boundary(meta, binary_data, obj, base_off) do
  ...
  def binary_write_recursive_disturbance_boundary(parent_off, heap_data, allocator, obj) do
    # member: boundary_point, type: geometry_msgs/Point (struct), offset: 0, size: 24
    {allocator, heap_data} =
