defmodule HakoPdu.StdMsgs.ByteConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_byte(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.Byte.new()
    binary_read_recursive_byte(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_byte(obj) do
    {base_data, heap_data} = binary_write_recursive_byte(0, "", "", obj)
  ...
  def binary_read_recursive_byte(meta, binary_data, obj, base_off) do
    # member: data, type: byte (primitive), offset: 0, size: 1
    data = read_primitive("byte", binary_part(binary_data, base_off + 0, 1))
  ...
  def binary_write_recursive_byte(parent_off, heap_data, allocator, obj) do
    # member: data, type: byte (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("byte", obj.data, 1))
