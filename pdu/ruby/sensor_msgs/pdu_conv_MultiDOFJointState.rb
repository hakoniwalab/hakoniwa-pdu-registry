require_relative 'pdu_type_MultiDOFJointState'
...
require_relative '../geometry_msgs/pdu_conv_Wrench'


module HakoPdu::SensorMsgs::MultiDOFJointStateConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_multi_d_o_f_joint_state(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::MultiDOFJointState.new
    binary_read_recursive_multi_d_o_f_joint_state(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_multi_d_o_f_joint_state(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_multi_d_o_f_joint_state(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_multi_d_o_f_joint_state(meta, binary_data, obj, base_off)
    ...
    obj.wrench = []
    array_size.times do |i|
      child = HakoPdu::GeometryMsgs::Wrench.new
      HakoPdu::GeometryMsgs::WrenchConverter.binary_read_recursive_wrench(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 48)
      obj.wrench << child
    end
  ...
  def self.binary_write_recursive_multi_d_o_f_joint_state(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    add_binary(heap_data, offset_from_heap, "\x00".b * (values.length * 48))
    values.each_with_index do |child, i|
      HakoPdu::GeometryMsgs::WrenchConverter.binary_write_recursive_wrench(offset_from_heap + i * 48, heap_data, heap_data, child)
    end
