defmodule HakoPdu.StdMsgs.StringConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_string(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.String.new()
    binary_read_recursive_string(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_string(obj) do
    {base_data, heap_data} = binary_write_recursive_string(0, "", "", obj)
  ...
  def binary_read_recursive_string(meta, binary_data, obj, base_off) do
    # member: data, type: string (primitive), offset: 0, size: 128
    data = read_primitive("string", binary_part(binary_data, base_off + 0, 128))
  ...
  def binary_write_recursive_string(parent_off, heap_data, allocator, obj) do
    # member: data, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.data, 128))
