defmodule HakoPdu.HakoMsgs.HakoDroneCmdTakeoffConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_drone_cmd_takeoff(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.HakoDroneCmdTakeoff.new()
    binary_read_recursive_hako_drone_cmd_takeoff(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_drone_cmd_takeoff(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_drone_cmd_takeoff(0, "", "", obj)
  ...
  def binary_read_recursive_hako_drone_cmd_takeoff(meta, binary_data, obj, base_off) do
    ...
    height = read_primitive("float64", binary_part(binary_data, base_off + 16, 8))

    # member: speed, type: float64 (primitive), offset: 24, size: 8
    speed = read_primitive("float64", binary_part(binary_data, base_off + 24, 8))

    # member: yaw_deg, type: float64 (primitive), offset: 32, size: 8
    yaw_deg = read_primitive("float64", binary_part(binary_data, base_off + 32, 8))
  ...
  def binary_write_recursive_hako_drone_cmd_takeoff(parent_off, heap_data, allocator, obj) do
    # member: header, type: HakoDroneCmdHeader (struct), offset: 0, size: 12
    {allocator, heap_data} =
    ...
    # member: height, type: float64 (primitive), offset: 16, size: 8
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float64", obj.height, 8))

    # member: speed, type: float64 (primitive), offset: 24, size: 8
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("float64", obj.speed, 8))

    # member: yaw_deg, type: float64 (primitive), offset: 32, size: 8
    allocator = add_binary(allocator, parent_off + 32, primitive_to_bin("float64", obj.yaw_deg, 8))
