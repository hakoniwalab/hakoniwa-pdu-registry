defmodule HakoPdu.NavMsgs.PathConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_path(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.NavMsgs.Path.new()
    binary_read_recursive_path(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_path(obj) do
    {base_data, heap_data} = binary_write_recursive_path(0, "", "", obj)
  ...
  def binary_read_recursive_path(meta, binary_data, obj, base_off) do
    ...
    # member: poses, type: geometry_msgs/PoseStamped (struct), offset: 136, size: 192
    ...
    poses =
      ...
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.PoseStampedConverter.binary_read_recursive_pose_stamped(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.PoseStamped.new(),
          meta.heap_off + offset_from_heap + i * 192
        )
      end)
  ...
  def binary_write_recursive_path(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 192))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.PoseStampedConverter.binary_write_recursive_pose_stamped(
            offset_from_heap + i * 192,
            acc_heap,
            acc_heap,
            child
          )
