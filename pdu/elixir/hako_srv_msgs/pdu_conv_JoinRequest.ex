defmodule HakoPdu.HakoSrvMsgs.JoinRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_join_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.JoinRequest.new()
    binary_read_recursive_join_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_join_request(obj) do
    {base_data, heap_data} = binary_write_recursive_join_request(0, "", "", obj)
  ...
  def binary_read_recursive_join_request(meta, binary_data, obj, base_off) do
  ...
  def binary_write_recursive_join_request(parent_off, heap_data, allocator, obj) do
    # member: name, type: string (primitive), offset: 0, size: 128
