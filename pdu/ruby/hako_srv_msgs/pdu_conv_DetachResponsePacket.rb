require_relative 'pdu_type_DetachResponsePacket'
require_relative 'pdu_type_ServiceResponseHeader'
require_relative 'pdu_conv_ServiceResponseHeader'
require_relative 'pdu_type_DetachResponse'
require_relative 'pdu_conv_DetachResponse'


module HakoPdu::HakoSrvMsgs::DetachResponsePacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_detach_response_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::DetachResponsePacket.new
    binary_read_recursive_detach_response_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_detach_response_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_detach_response_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_detach_response_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: DetachResponse (struct), offset: 268, size: 4
    obj.body = HakoPdu::HakoSrvMsgs::DetachResponse.new
    HakoPdu::HakoSrvMsgs::DetachResponseConverter.binary_read_recursive_detach_response(meta, binary_data, obj.body, base_off + 268)

    obj
  end

  def self.binary_write_recursive_detach_response_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceResponseHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: DetachResponse (struct), offset: 268, size: 4
    HakoPdu::HakoSrvMsgs::DetachResponseConverter.binary_write_recursive_detach_response(parent_off + 268, heap_data, allocator, obj.body)
