defmodule HakoPdu.HakoSrvMsgs.AckEventResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ack_event_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.AckEventResponse.new()
    binary_read_recursive_ack_event_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ack_event_response(obj) do
    {base_data, heap_data} = binary_write_recursive_ack_event_response(0, "", "", obj)
  ...
  def binary_read_recursive_ack_event_response(meta, binary_data, obj, base_off) do
    # member: ack_code, type: uint32 (primitive), offset: 0, size: 4
    ack_code = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))
  ...
  def binary_write_recursive_ack_event_response(parent_off, heap_data, allocator, obj) do
    # member: ack_code, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.ack_code, 4))
