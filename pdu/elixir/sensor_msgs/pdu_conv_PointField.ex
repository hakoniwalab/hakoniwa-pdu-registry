defmodule HakoPdu.SensorMsgs.PointFieldConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_point_field(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.PointField.new()
    binary_read_recursive_point_field(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_point_field(obj) do
    {base_data, heap_data} = binary_write_recursive_point_field(0, "", "", obj)
  ...
  def binary_read_recursive_point_field(meta, binary_data, obj, base_off) do
    # member: name, type: string (primitive), offset: 0, size: 128
    name = read_primitive("string", binary_part(binary_data, base_off + 0, 128))

    # member: offset, type: uint32 (primitive), offset: 128, size: 4
    offset = read_primitive("uint32", binary_part(binary_data, base_off + 128, 4))

    # member: datatype, type: uint8 (primitive), offset: 132, size: 1
    datatype = read_primitive("uint8", binary_part(binary_data, base_off + 132, 1))

    # member: count, type: uint32 (primitive), offset: 136, size: 4
    count = read_primitive("uint32", binary_part(binary_data, base_off + 136, 4))
  ...
  def binary_write_recursive_point_field(parent_off, heap_data, allocator, obj) do
    # member: name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.name, 128))

    # member: offset, type: uint32 (primitive), offset: 128, size: 4
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("uint32", obj.offset, 4))

    # member: datatype, type: uint8 (primitive), offset: 132, size: 1
    allocator = add_binary(allocator, parent_off + 132, primitive_to_bin("uint8", obj.datatype, 1))

    # member: count, type: uint32 (primitive), offset: 136, size: 4
    allocator = add_binary(allocator, parent_off + 136, primitive_to_bin("uint32", obj.count, 4))
