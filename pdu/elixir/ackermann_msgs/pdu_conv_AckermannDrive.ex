defmodule HakoPdu.AckermannMsgs.AckermannDriveConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_ackermann_drive(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.AckermannMsgs.AckermannDrive.new()
    binary_read_recursive_ackermann_drive(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_ackermann_drive(obj) do
    {base_data, heap_data} = binary_write_recursive_ackermann_drive(0, "", "", obj)
  ...
  def binary_read_recursive_ackermann_drive(meta, binary_data, obj, base_off) do
    # member: steering_angle, type: float32 (primitive), offset: 0, size: 4
    steering_angle = read_primitive("float32", binary_part(binary_data, base_off + 0, 4))

    # member: steering_angle_velocity, type: float32 (primitive), offset: 4, size: 4
    steering_angle_velocity = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))

    # member: speed, type: float32 (primitive), offset: 8, size: 4
    speed = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: acceleration, type: float32 (primitive), offset: 12, size: 4
    acceleration = read_primitive("float32", binary_part(binary_data, base_off + 12, 4))

    # member: jerk, type: float32 (primitive), offset: 16, size: 4
    jerk = read_primitive("float32", binary_part(binary_data, base_off + 16, 4))
  ...
  def binary_write_recursive_ackermann_drive(parent_off, heap_data, allocator, obj) do
    # member: steering_angle, type: float32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float32", obj.steering_angle, 4))

    # member: steering_angle_velocity, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.steering_angle_velocity, 4))

    # member: speed, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.speed, 4))

    # member: acceleration, type: float32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("float32", obj.acceleration, 4))

    # member: jerk, type: float32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float32", obj.jerk, 4))
