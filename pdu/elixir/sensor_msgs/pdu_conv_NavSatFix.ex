defmodule HakoPdu.SensorMsgs.NavSatFixConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_nav_sat_fix(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.NavSatFix.new()
    binary_read_recursive_nav_sat_fix(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_nav_sat_fix(obj) do
    {base_data, heap_data} = binary_write_recursive_nav_sat_fix(0, "", "", obj)
  ...
  def binary_read_recursive_nav_sat_fix(meta, binary_data, obj, base_off) do
    ...
    status =
      HakoPdu.SensorMsgs.NavSatStatusConverter.binary_read_recursive_nav_sat_status(
        meta,
        binary_data,
        HakoPdu.SensorMsgs.NavSatStatus.new(),
        base_off + 136
      )
    ...
    altitude = read_primitive("float64", binary_part(binary_data, base_off + 160, 8))

    # member: position_covariance, type: float64 (primitive), offset: 168, size: 72
    position_covariance = read_primitive_array("float64", binary_part(binary_data, base_off + 168, 72), 9, div(72, 9))

    # member: position_covariance_type, type: uint8 (primitive), offset: 240, size: 1
    position_covariance_type = read_primitive("uint8", binary_part(binary_data, base_off + 240, 1))
  ...
  def binary_write_recursive_nav_sat_fix(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    # member: status, type: NavSatStatus (struct), offset: 136, size: 4
    {allocator, heap_data} =
      HakoPdu.SensorMsgs.NavSatStatusConverter.binary_write_recursive_nav_sat_status(
        parent_off + 136,
        heap_data,
        allocator,
        obj.status
    ...
    # member: altitude, type: float64 (primitive), offset: 160, size: 8
    allocator = add_binary(allocator, parent_off + 160, primitive_to_bin("float64", obj.altitude, 8))

    # member: position_covariance, type: float64 (primitive), offset: 168, size: 72
    allocator = add_binary(allocator, parent_off + 168, primitive_array_to_bin("float64", obj.position_covariance, div(72, 9)))

    # member: position_covariance_type, type: uint8 (primitive), offset: 240, size: 1
    allocator = add_binary(allocator, parent_off + 240, primitive_to_bin("uint8", obj.position_covariance_type, 1))
