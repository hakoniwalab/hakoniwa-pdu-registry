...
module HakoPdu::Ev3Msgs::Ev3PduColorSensorConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ev3_pdu_color_sensor(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Ev3Msgs::Ev3PduColorSensor.new
    binary_read_recursive_ev3_pdu_color_sensor(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ev3_pdu_color_sensor(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ev3_pdu_color_sensor(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ev3_pdu_color_sensor(meta, binary_data, obj, base_off)
    # member: color, type: uint32 (primitive), offset: 0, size: 4
    obj.color = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: reflect, type: uint32 (primitive), offset: 4, size: 4
    obj.reflect = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: rgb_r, type: uint32 (primitive), offset: 8, size: 4
    obj.rgb_r = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))

    # member: rgb_g, type: uint32 (primitive), offset: 12, size: 4
    obj.rgb_g = read_primitive('uint32', binary_data.byteslice(base_off + 12, 4))

    # member: rgb_b, type: uint32 (primitive), offset: 16, size: 4
    obj.rgb_b = read_primitive('uint32', binary_data.byteslice(base_off + 16, 4))

    obj
  end

  def self.binary_write_recursive_ev3_pdu_color_sensor(parent_off, heap_data, allocator, obj)
    # member: color, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.color, 4))

    # member: reflect, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.reflect, 4))

    # member: rgb_r, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.rgb_r, 4))

    # member: rgb_g, type: uint32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('uint32', obj.rgb_g, 4))

    # member: rgb_b, type: uint32 (primitive), offset: 16, size: 4
    add_binary(allocator, parent_off + 16, primitive_to_bin('uint32', obj.rgb_b, 4))
