require_relative 'pdu_type_ExecutionUnitRuntimeEpoch'


module HakoPdu::HakoMsgs::ExecutionUnitRuntimeEpochConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_execution_unit_runtime_epoch(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ExecutionUnitRuntimeEpoch.new
    binary_read_recursive_execution_unit_runtime_epoch(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_execution_unit_runtime_epoch(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_execution_unit_runtime_epoch(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_execution_unit_runtime_epoch(meta, binary_data, obj, base_off)
    # member: epoch, type: uint8 (primitive), offset: 0, size: 1
  ...
  def self.binary_write_recursive_execution_unit_runtime_epoch(parent_off, heap_data, allocator, obj)
    # member: epoch, type: uint8 (primitive), offset: 0, size: 1
