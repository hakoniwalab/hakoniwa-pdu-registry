require_relative 'pdu_type_ExecutionUnitRuntimeContext'


module HakoPdu::HakoMsgs::ExecutionUnitRuntimeContextConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_execution_unit_runtime_context(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ExecutionUnitRuntimeContext.new
    binary_read_recursive_execution_unit_runtime_context(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_execution_unit_runtime_context(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_execution_unit_runtime_context(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_execution_unit_runtime_context(meta, binary_data, obj, base_off)
    ...
    obj.epoch = read_primitive('uint8', binary_data.byteslice(base_off + 4, 1))

    # member: owner_id, type: uint8 (primitive), offset: 5, size: 1
    obj.owner_id = read_primitive('uint8', binary_data.byteslice(base_off + 5, 1))
    ...
    obj.context = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    obj
  end

  def self.binary_write_recursive_execution_unit_runtime_context(parent_off, heap_data, allocator, obj)
    # member: config_hash, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.config_hash, 4))

    # member: epoch, type: uint8 (primitive), offset: 4, size: 1
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint8', obj.epoch, 1))

    # member: owner_id, type: uint8 (primitive), offset: 5, size: 1
    add_binary(allocator, parent_off + 5, primitive_to_bin('uint8', obj.owner_id, 1))
