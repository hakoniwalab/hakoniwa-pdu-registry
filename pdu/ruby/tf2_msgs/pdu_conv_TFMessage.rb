require_relative 'pdu_type_TFMessage'
require_relative '../geometry_msgs/pdu_type_TransformStamped'
require_relative '../geometry_msgs/pdu_conv_TransformStamped'


module HakoPdu::Tf2Msgs::TFMessageConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_tf_message(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Tf2Msgs::TFMessage.new
    binary_read_recursive_tf_message(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_tf_message(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_tf_message(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_tf_message(meta, binary_data, obj, base_off)
    # member: transforms, type: geometry_msgs/TransformStamped (struct), offset: 0, size: 320
    ...
    array_size.times do |i|
      child = HakoPdu::GeometryMsgs::TransformStamped.new
      HakoPdu::GeometryMsgs::TransformStampedConverter.binary_read_recursive_transform_stamped(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 320)
  ...
  def self.binary_write_recursive_tf_message(parent_off, heap_data, allocator, obj)
    # member: transforms, type: geometry_msgs/TransformStamped (struct), offset: 0, size: 320
    ...
    add_binary(heap_data, offset_from_heap, "\x00".b * (values.length * 320))
    values.each_with_index do |child, i|
      HakoPdu::GeometryMsgs::TransformStampedConverter.binary_write_recursive_transform_stamped(offset_from_heap + i * 320, heap_data, heap_data, child)
    end
