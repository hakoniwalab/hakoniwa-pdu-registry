...
module HakoPdu::DroneSrvMsgs::MagnetGrabResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_magnet_grab_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::MagnetGrabResponse.new
    binary_read_recursive_magnet_grab_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_magnet_grab_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_magnet_grab_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_magnet_grab_response(meta, binary_data, obj, base_off)
    ...
    obj.magnet_on = read_primitive('bool', binary_data.byteslice(base_off + 4, 4))

    # member: contact_on, type: bool (primitive), offset: 8, size: 4
    obj.contact_on = read_primitive('bool', binary_data.byteslice(base_off + 8, 4))
  ...
  def self.binary_write_recursive_magnet_grab_response(parent_off, heap_data, allocator, obj)
    # member: ok, type: bool (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('bool', obj.ok, 4))

    # member: magnet_on, type: bool (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('bool', obj.magnet_on, 4))

    # member: contact_on, type: bool (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('bool', obj.contact_on, 4))
