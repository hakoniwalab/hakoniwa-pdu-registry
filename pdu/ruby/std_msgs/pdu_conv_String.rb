require_relative 'pdu_type_String'


module HakoPdu::StdMsgs::StringConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_string(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::String.new
    binary_read_recursive_string(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_string(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_string(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_string(meta, binary_data, obj, base_off)
    # member: data, type: string (primitive), offset: 0, size: 128
    obj.data = read_primitive('string', binary_data.byteslice(base_off + 0, 128))

    obj
  end

  def self.binary_write_recursive_string(parent_off, heap_data, allocator, obj)
    # member: data, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.data, 128))
