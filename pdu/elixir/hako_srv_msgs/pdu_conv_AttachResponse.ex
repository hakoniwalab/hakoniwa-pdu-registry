defmodule HakoPdu.HakoSrvMsgs.AttachResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_attach_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.AttachResponse.new()
    binary_read_recursive_attach_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_attach_response(obj) do
    {base_data, heap_data} = binary_write_recursive_attach_response(0, "", "", obj)
  ...
  def binary_read_recursive_attach_response(meta, binary_data, obj, base_off) do
    # member: result_code, type: uint32 (primitive), offset: 0, size: 4
    result_code = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: session_id, type: uint64 (primitive), offset: 8, size: 8
    session_id = read_primitive("uint64", binary_part(binary_data, base_off + 8, 8))

    # member: world_time, type: LogicalTime (struct), offset: 16, size: 16
    world_time =
      HakoPdu.HakoSrvMsgs.LogicalTimeConverter.binary_read_recursive_logical_time(
        meta,
        binary_data,
        HakoPdu.HakoSrvMsgs.LogicalTime.new(),
        base_off + 16
      )

    # member: delta_asset_tick, type: uint64 (primitive), offset: 32, size: 8
    delta_asset_tick = read_primitive("uint64", binary_part(binary_data, base_off + 32, 8))
  ...
  def binary_write_recursive_attach_response(parent_off, heap_data, allocator, obj) do
    # member: result_code, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.result_code, 4))

    # member: session_id, type: uint64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint64", obj.session_id, 8))

    # member: world_time, type: LogicalTime (struct), offset: 16, size: 16
    {allocator, heap_data} =
      HakoPdu.HakoSrvMsgs.LogicalTimeConverter.binary_write_recursive_logical_time(
        parent_off + 16,
        heap_data,
        allocator,
        obj.world_time
    ...
    # member: delta_asset_tick, type: uint64 (primitive), offset: 32, size: 8
    allocator = add_binary(allocator, parent_off + 32, primitive_to_bin("uint64", obj.delta_asset_tick, 8))
