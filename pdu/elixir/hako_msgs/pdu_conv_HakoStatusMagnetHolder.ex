defmodule HakoPdu.HakoMsgs.HakoStatusMagnetHolderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_status_magnet_holder(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.HakoStatusMagnetHolder.new()
    binary_read_recursive_hako_status_magnet_holder(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_status_magnet_holder(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_status_magnet_holder(0, "", "", obj)
  ...
  def binary_read_recursive_hako_status_magnet_holder(meta, binary_data, obj, base_off) do
    # member: magnet_on, type: bool (primitive), offset: 0, size: 4
    magnet_on = read_primitive("bool", binary_part(binary_data, base_off + 0, 4))

    # member: contact_on, type: bool (primitive), offset: 4, size: 4
    contact_on = read_primitive("bool", binary_part(binary_data, base_off + 4, 4))
  ...
  def binary_write_recursive_hako_status_magnet_holder(parent_off, heap_data, allocator, obj) do
    # member: magnet_on, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.magnet_on, 4))

    # member: contact_on, type: bool (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("bool", obj.contact_on, 4))
