...
module HakoPdu::HakoMsgs::DisturbanceAtmConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_disturbance_atm(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::DisturbanceAtm.new
    binary_read_recursive_disturbance_atm(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_disturbance_atm(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_disturbance_atm(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_disturbance_atm(meta, binary_data, obj, base_off)
    # member: sea_level_atm, type: float64 (primitive), offset: 0, size: 8
    obj.sea_level_atm = read_primitive('float64', binary_data.byteslice(base_off + 0, 8))

    obj
  end

  def self.binary_write_recursive_disturbance_atm(parent_off, heap_data, allocator, obj)
    # member: sea_level_atm, type: float64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('float64', obj.sea_level_atm, 8))
