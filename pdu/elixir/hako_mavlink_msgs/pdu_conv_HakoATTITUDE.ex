defmodule HakoPdu.HakoMavlinkMsgs.HakoATTITUDEConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_a_t_t_i_t_u_d_e(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMavlinkMsgs.HakoATTITUDE.new()
    binary_read_recursive_hako_a_t_t_i_t_u_d_e(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_a_t_t_i_t_u_d_e(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_a_t_t_i_t_u_d_e(0, "", "", obj)
  ...
  def binary_read_recursive_hako_a_t_t_i_t_u_d_e(meta, binary_data, obj, base_off) do
    # member: time_boot_ms, type: uint32 (primitive), offset: 0, size: 4
    time_boot_ms = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: roll, type: float32 (primitive), offset: 4, size: 4
    roll = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))

    # member: pitch, type: float32 (primitive), offset: 8, size: 4
    pitch = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: yaw, type: float32 (primitive), offset: 12, size: 4
    yaw = read_primitive("float32", binary_part(binary_data, base_off + 12, 4))

    # member: rollspeed, type: float32 (primitive), offset: 16, size: 4
    rollspeed = read_primitive("float32", binary_part(binary_data, base_off + 16, 4))

    # member: pitchspeed, type: float32 (primitive), offset: 20, size: 4
    pitchspeed = read_primitive("float32", binary_part(binary_data, base_off + 20, 4))

    # member: yawspeed, type: float32 (primitive), offset: 24, size: 4
    yawspeed = read_primitive("float32", binary_part(binary_data, base_off + 24, 4))
  ...
  def binary_write_recursive_hako_a_t_t_i_t_u_d_e(parent_off, heap_data, allocator, obj) do
    # member: time_boot_ms, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.time_boot_ms, 4))

    # member: roll, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.roll, 4))

    # member: pitch, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.pitch, 4))

    # member: yaw, type: float32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("float32", obj.yaw, 4))

    # member: rollspeed, type: float32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float32", obj.rollspeed, 4))

    # member: pitchspeed, type: float32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("float32", obj.pitchspeed, 4))

    # member: yawspeed, type: float32 (primitive), offset: 24, size: 4
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("float32", obj.yawspeed, 4))
