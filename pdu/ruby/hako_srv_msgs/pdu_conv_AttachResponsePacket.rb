require_relative 'pdu_type_AttachResponsePacket'
...
require_relative 'pdu_conv_AttachResponse'


module HakoPdu::HakoSrvMsgs::AttachResponsePacketConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_attach_response_packet(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AttachResponsePacket.new
    binary_read_recursive_attach_response_packet(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_attach_response_packet(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_attach_response_packet(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_attach_response_packet(meta, binary_data, obj, base_off)
    ...
    # member: body, type: AttachResponse (struct), offset: 272, size: 40
    obj.body = HakoPdu::HakoSrvMsgs::AttachResponse.new
    HakoPdu::HakoSrvMsgs::AttachResponseConverter.binary_read_recursive_attach_response(meta, binary_data, obj.body, base_off + 272)

    obj
  end

  def self.binary_write_recursive_attach_response_packet(parent_off, heap_data, allocator, obj)
    # member: header, type: ServiceResponseHeader (struct), offset: 0, size: 268
    ...
    # member: body, type: AttachResponse (struct), offset: 272, size: 40
    HakoPdu::HakoSrvMsgs::AttachResponseConverter.binary_write_recursive_attach_response(parent_off + 272, heap_data, allocator, obj.body)
