defmodule HakoPdu.GeometryMsgs.QuaternionConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_quaternion(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.Quaternion.new()
    binary_read_recursive_quaternion(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_quaternion(obj) do
    {base_data, heap_data} = binary_write_recursive_quaternion(0, "", "", obj)
  ...
  def binary_read_recursive_quaternion(meta, binary_data, obj, base_off) do
    # member: x, type: float64 (primitive), offset: 0, size: 8
    x = read_primitive("float64", binary_part(binary_data, base_off + 0, 8))

    # member: y, type: float64 (primitive), offset: 8, size: 8
    y = read_primitive("float64", binary_part(binary_data, base_off + 8, 8))

    # member: z, type: float64 (primitive), offset: 16, size: 8
    z = read_primitive("float64", binary_part(binary_data, base_off + 16, 8))

    # member: w, type: float64 (primitive), offset: 24, size: 8
    w = read_primitive("float64", binary_part(binary_data, base_off + 24, 8))
  ...
  def binary_write_recursive_quaternion(parent_off, heap_data, allocator, obj) do
    # member: x, type: float64 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float64", obj.x, 8))

    # member: y, type: float64 (primitive), offset: 8, size: 8
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float64", obj.y, 8))

    # member: z, type: float64 (primitive), offset: 16, size: 8
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float64", obj.z, 8))

    # member: w, type: float64 (primitive), offset: 24, size: 8
    allocator = add_binary(allocator, parent_off + 24, primitive_to_bin("float64", obj.w, 8))
