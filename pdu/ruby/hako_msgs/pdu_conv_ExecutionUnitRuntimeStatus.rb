require_relative 'pdu_type_ExecutionUnitRuntimeStatus'


module HakoPdu::HakoMsgs::ExecutionUnitRuntimeStatusConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_execution_unit_runtime_status(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::ExecutionUnitRuntimeStatus.new
    binary_read_recursive_execution_unit_runtime_status(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_execution_unit_runtime_status(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_execution_unit_runtime_status(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_execution_unit_runtime_status(meta, binary_data, obj, base_off)
    # member: config_hash, type: uint32 (primitive), offset: 0, size: 4
    obj.config_hash = read_primitive('uint32', binary_data.byteslice(base_off + 0, 4))

    # member: unit_count, type: uint16 (primitive), offset: 4, size: 2
    obj.unit_count = read_primitive('uint16', binary_data.byteslice(base_off + 4, 2))
    ...
    obj.status = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)
    ...
    obj.epoch = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)
    ...
    obj.curr_owner_node_id = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    # member: next_owner_node_id, type: uint8 (primitive), offset: 32, size: 1
    array_size = read_primitive('int32', binary_data.byteslice(base_off + 32, 4))
    offset_from_heap = read_primitive('int32', binary_data.byteslice(base_off + 32 + 4, 4))
    obj.next_owner_node_id = read_primitive_array('uint8', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 1 * array_size), array_size, 1)

    obj
  end

  def self.binary_write_recursive_execution_unit_runtime_status(parent_off, heap_data, allocator, obj)
    # member: config_hash, type: uint32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint32', obj.config_hash, 4))

    # member: unit_count, type: uint16 (primitive), offset: 4, size: 2
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint16', obj.unit_count, 2))
    ...
    add_binary(allocator, parent_off + 32, [values.length, offset_from_heap].pack('l<l<'))
    add_binary(heap_data, offset_from_heap, primitive_array_to_bin('uint8', values, 1))
