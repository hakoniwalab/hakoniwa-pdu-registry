defmodule HakoPdu.SensorMsgs.LaserEchoConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_laser_echo(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.LaserEcho.new()
    binary_read_recursive_laser_echo(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_laser_echo(obj) do
    {base_data, heap_data} = binary_write_recursive_laser_echo(0, "", "", obj)
  ...
  def binary_read_recursive_laser_echo(meta, binary_data, obj, base_off) do
    # member: echoes, type: float32 (primitive), offset: 0, size: 4
    ...
    echoes = read_primitive_array("float32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)
  ...
  def binary_write_recursive_laser_echo(parent_off, heap_data, allocator, obj) do
    # member: echoes, type: float32 (primitive), offset: 0, size: 4
