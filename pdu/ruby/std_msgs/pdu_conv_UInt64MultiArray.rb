require_relative 'pdu_type_UInt64MultiArray'
require_relative 'pdu_type_MultiArrayLayout'
require_relative 'pdu_conv_MultiArrayLayout'


module HakoPdu::StdMsgs::UInt64MultiArrayConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_u_int64_multi_array(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::UInt64MultiArray.new
    binary_read_recursive_u_int64_multi_array(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_u_int64_multi_array(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_u_int64_multi_array(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_u_int64_multi_array(meta, binary_data, obj, base_off)
    ...
    obj.data = read_primitive_array('uint64', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 8 * array_size), array_size, 8)

    obj
  end

  def self.binary_write_recursive_u_int64_multi_array(parent_off, heap_data, allocator, obj)
    # member: layout, type: MultiArrayLayout (struct), offset: 0, size: 12
    ...
    add_binary(heap_data, offset_from_heap, primitive_array_to_bin('uint64', values, 8))
