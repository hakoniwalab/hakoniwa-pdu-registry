...
module HakoPdu::HakoMsgs::DroneVisualStateConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_visual_state(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::DroneVisualState.new
    binary_read_recursive_drone_visual_state(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_visual_state(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_visual_state(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_visual_state(meta, binary_data, obj, base_off)
    # member: x, type: float32 (primitive), offset: 0, size: 4
    obj.x = read_primitive('float32', binary_data.byteslice(base_off + 0, 4))

    # member: y, type: float32 (primitive), offset: 4, size: 4
    obj.y = read_primitive('float32', binary_data.byteslice(base_off + 4, 4))

    # member: z, type: float32 (primitive), offset: 8, size: 4
    obj.z = read_primitive('float32', binary_data.byteslice(base_off + 8, 4))

    # member: roll, type: float32 (primitive), offset: 12, size: 4
    obj.roll = read_primitive('float32', binary_data.byteslice(base_off + 12, 4))

    # member: pitch, type: float32 (primitive), offset: 16, size: 4
    obj.pitch = read_primitive('float32', binary_data.byteslice(base_off + 16, 4))

    # member: yaw, type: float32 (primitive), offset: 20, size: 4
    obj.yaw = read_primitive('float32', binary_data.byteslice(base_off + 20, 4))
    ...
    obj.pwm_duty = read_primitive_array('float32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_drone_visual_state(parent_off, heap_data, allocator, obj)
    # member: x, type: float32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('float32', obj.x, 4))

    # member: y, type: float32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('float32', obj.y, 4))

    # member: z, type: float32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('float32', obj.z, 4))

    # member: roll, type: float32 (primitive), offset: 12, size: 4
    add_binary(allocator, parent_off + 12, primitive_to_bin('float32', obj.roll, 4))

    # member: pitch, type: float32 (primitive), offset: 16, size: 4
    add_binary(allocator, parent_off + 16, primitive_to_bin('float32', obj.pitch, 4))

    # member: yaw, type: float32 (primitive), offset: 20, size: 4
    add_binary(allocator, parent_off + 20, primitive_to_bin('float32', obj.yaw, 4))
