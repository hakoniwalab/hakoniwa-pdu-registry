require_relative 'pdu_type_DroneStatus'
require_relative '../geometry_msgs/pdu_type_Vector3'
require_relative '../geometry_msgs/pdu_conv_Vector3'


module HakoPdu::HakoMsgs::DroneStatusConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_status(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::DroneStatus.new
    binary_read_recursive_drone_status(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_status(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_status(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_status(meta, binary_data, obj, base_off)
    # member: flight_mode, type: int32 (primitive), offset: 0, size: 4
    obj.flight_mode = read_primitive('int32', binary_data.byteslice(base_off + 0, 4))

    # member: internal_state, type: int32 (primitive), offset: 4, size: 4
    obj.internal_state = read_primitive('int32', binary_data.byteslice(base_off + 4, 4))

    # member: propeller_wind, type: geometry_msgs/Vector3 (struct), offset: 8, size: 24
    obj.propeller_wind = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.propeller_wind, base_off + 8)

    # member: collided_counts, type: int32 (primitive), offset: 32, size: 4
    obj.collided_counts = read_primitive('int32', binary_data.byteslice(base_off + 32, 4))

    obj
  end

  def self.binary_write_recursive_drone_status(parent_off, heap_data, allocator, obj)
    # member: flight_mode, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.flight_mode, 4))

    # member: internal_state, type: int32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('int32', obj.internal_state, 4))

    # member: propeller_wind, type: geometry_msgs/Vector3 (struct), offset: 8, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 8, heap_data, allocator, obj.propeller_wind)

    # member: collided_counts, type: int32 (primitive), offset: 32, size: 4
    add_binary(allocator, parent_off + 32, primitive_to_bin('int32', obj.collided_counts, 4))
