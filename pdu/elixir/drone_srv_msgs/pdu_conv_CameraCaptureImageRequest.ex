defmodule HakoPdu.DroneSrvMsgs.CameraCaptureImageRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_camera_capture_image_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.CameraCaptureImageRequest.new()
    binary_read_recursive_camera_capture_image_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_camera_capture_image_request(obj) do
    {base_data, heap_data} = binary_write_recursive_camera_capture_image_request(0, "", "", obj)
  ...
  def binary_read_recursive_camera_capture_image_request(meta, binary_data, obj, base_off) do
    ...
    # member: image_type, type: string (primitive), offset: 128, size: 128
    image_type = read_primitive("string", binary_part(binary_data, base_off + 128, 128))
  ...
  def binary_write_recursive_camera_capture_image_request(parent_off, heap_data, allocator, obj) do
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("string", obj.drone_name, 128))

    # member: image_type, type: string (primitive), offset: 128, size: 128
    allocator = add_binary(allocator, parent_off + 128, primitive_to_bin("string", obj.image_type, 128))
