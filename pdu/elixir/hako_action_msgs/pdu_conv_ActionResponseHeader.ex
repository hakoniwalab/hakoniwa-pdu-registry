defmodule HakoPdu.HakoActionMsgs.ActionResponseHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_action_response_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoActionMsgs.ActionResponseHeader.new()
    binary_read_recursive_action_response_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_action_response_header(obj) do
    {base_data, heap_data} = binary_write_recursive_action_response_header(0, "", "", obj)
  ...
  def binary_read_recursive_action_response_header(meta, binary_data, obj, base_off) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    version = read_primitive("uint8", binary_part(binary_data, base_off + 0, 1))

    # member: response_kind, type: uint8 (primitive), offset: 1, size: 1
    response_kind = read_primitive("uint8", binary_part(binary_data, base_off + 1, 1))

    # member: status, type: uint8 (primitive), offset: 2, size: 1
    status = read_primitive("uint8", binary_part(binary_data, base_off + 2, 1))

    # member: reserved, type: uint8 (primitive), offset: 3, size: 1
    reserved = read_primitive("uint8", binary_part(binary_data, base_off + 3, 1))

    # member: goal_id, type: uint8 (primitive), offset: 4, size: 16
    goal_id = read_primitive_array("uint8", binary_part(binary_data, base_off + 4, 16), 16, div(16, 16))
  ...
  def binary_write_recursive_action_response_header(parent_off, heap_data, allocator, obj) do
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.version, 1))

    # member: response_kind, type: uint8 (primitive), offset: 1, size: 1
    allocator = add_binary(allocator, parent_off + 1, primitive_to_bin("uint8", obj.response_kind, 1))

    # member: status, type: uint8 (primitive), offset: 2, size: 1
    allocator = add_binary(allocator, parent_off + 2, primitive_to_bin("uint8", obj.status, 1))

    # member: reserved, type: uint8 (primitive), offset: 3, size: 1
    allocator = add_binary(allocator, parent_off + 3, primitive_to_bin("uint8", obj.reserved, 1))

    # member: goal_id, type: uint8 (primitive), offset: 4, size: 16
    allocator = add_binary(allocator, parent_off + 4, primitive_array_to_bin("uint8", obj.goal_id, div(16, 16)))
