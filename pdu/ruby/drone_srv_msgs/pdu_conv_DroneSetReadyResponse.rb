...
module HakoPdu::DroneSrvMsgs::DroneSetReadyResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_drone_set_ready_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::DroneSetReadyResponse.new
    binary_read_recursive_drone_set_ready_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_drone_set_ready_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_drone_set_ready_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_drone_set_ready_response(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_drone_set_ready_response(parent_off, heap_data, allocator, obj)
    # member: ok, type: bool (primitive), offset: 0, size: 4
