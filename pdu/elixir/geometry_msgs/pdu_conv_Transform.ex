defmodule HakoPdu.GeometryMsgs.TransformConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_transform(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.Transform.new()
    binary_read_recursive_transform(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_transform(obj) do
    {base_data, heap_data} = binary_write_recursive_transform(0, "", "", obj)
  ...
  def binary_read_recursive_transform(meta, binary_data, obj, base_off) do
  ...
  def binary_write_recursive_transform(parent_off, heap_data, allocator, obj) do
    # member: translation, type: Vector3 (struct), offset: 0, size: 24
    {allocator, heap_data} =
