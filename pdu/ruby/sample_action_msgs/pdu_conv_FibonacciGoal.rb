...
module HakoPdu::SampleActionMsgs::FibonacciGoalConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_fibonacci_goal(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SampleActionMsgs::FibonacciGoal.new
    binary_read_recursive_fibonacci_goal(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_fibonacci_goal(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_fibonacci_goal(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_fibonacci_goal(meta, binary_data, obj, base_off)
    # member: order, type: int32 (primitive), offset: 0, size: 4
    obj.order = read_primitive('int32', binary_data.byteslice(base_off + 0, 4))

    obj
  end

  def self.binary_write_recursive_fibonacci_goal(parent_off, heap_data, allocator, obj)
    # member: order, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.order, 4))
