require_relative 'pdu_type_Header'
require_relative '../builtin_interfaces/pdu_type_Time'
require_relative '../builtin_interfaces/pdu_conv_Time'


module HakoPdu::StdMsgs::HeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Header.new
    binary_read_recursive_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_header(meta, binary_data, obj, base_off)
    ...
    obj.frame_id = read_primitive('string', binary_data.byteslice(base_off + 8, 128))

    obj
  end

  def self.binary_write_recursive_header(parent_off, heap_data, allocator, obj)
    # member: stamp, type: builtin_interfaces/Time (struct), offset: 0, size: 8
    HakoPdu::BuiltinInterfaces::TimeConverter.binary_write_recursive_time(parent_off + 0, heap_data, allocator, obj.stamp)

    # member: frame_id, type: string (primitive), offset: 8, size: 128
    add_binary(allocator, parent_off + 8, primitive_to_bin('string', obj.frame_id, 128))
