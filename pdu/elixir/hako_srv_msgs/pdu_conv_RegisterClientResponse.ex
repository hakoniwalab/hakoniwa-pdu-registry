defmodule HakoPdu.HakoSrvMsgs.RegisterClientResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_register_client_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.RegisterClientResponse.new()
    binary_read_recursive_register_client_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_register_client_response(obj) do
    {base_data, heap_data} = binary_write_recursive_register_client_response(0, "", "", obj)
  ...
  def binary_read_recursive_register_client_response(meta, binary_data, obj, base_off) do
    # member: service_id, type: uint32 (primitive), offset: 0, size: 4
    service_id = read_primitive("uint32", binary_part(binary_data, base_off + 0, 4))

    # member: client_id, type: uint32 (primitive), offset: 4, size: 4
    client_id = read_primitive("uint32", binary_part(binary_data, base_off + 4, 4))

    # member: request_channel_id, type: uint32 (primitive), offset: 8, size: 4
    request_channel_id = read_primitive("uint32", binary_part(binary_data, base_off + 8, 4))

    # member: response_channel_id, type: uint32 (primitive), offset: 12, size: 4
    response_channel_id = read_primitive("uint32", binary_part(binary_data, base_off + 12, 4))
  ...
  def binary_write_recursive_register_client_response(parent_off, heap_data, allocator, obj) do
    # member: service_id, type: uint32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint32", obj.service_id, 4))

    # member: client_id, type: uint32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("uint32", obj.client_id, 4))

    # member: request_channel_id, type: uint32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("uint32", obj.request_channel_id, 4))

    # member: response_channel_id, type: uint32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("uint32", obj.response_channel_id, 4))
