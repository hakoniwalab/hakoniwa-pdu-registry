require_relative 'pdu_type_PoseWithCovarianceStamped'
...
require_relative 'pdu_conv_PoseWithCovariance'


module HakoPdu::GeometryMsgs::PoseWithCovarianceStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_pose_with_covariance_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::PoseWithCovarianceStamped.new
    binary_read_recursive_pose_with_covariance_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_pose_with_covariance_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_pose_with_covariance_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_pose_with_covariance_stamped(meta, binary_data, obj, base_off)
    ...
    # member: pose, type: PoseWithCovariance (struct), offset: 136, size: 344
    obj.pose = HakoPdu::GeometryMsgs::PoseWithCovariance.new
    HakoPdu::GeometryMsgs::PoseWithCovarianceConverter.binary_read_recursive_pose_with_covariance(meta, binary_data, obj.pose, base_off + 136)

    obj
  end

  def self.binary_write_recursive_pose_with_covariance_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: pose, type: PoseWithCovariance (struct), offset: 136, size: 344
    HakoPdu::GeometryMsgs::PoseWithCovarianceConverter.binary_write_recursive_pose_with_covariance(parent_off + 136, heap_data, allocator, obj.pose)
