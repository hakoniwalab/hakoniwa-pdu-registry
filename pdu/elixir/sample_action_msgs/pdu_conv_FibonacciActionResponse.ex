defmodule HakoPdu.SampleActionMsgs.FibonacciActionResponseConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_fibonacci_action_response(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SampleActionMsgs.FibonacciActionResponse.new()
    binary_read_recursive_fibonacci_action_response(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_fibonacci_action_response(obj) do
    {base_data, heap_data} = binary_write_recursive_fibonacci_action_response(0, "", "", obj)
  ...
  def binary_read_recursive_fibonacci_action_response(meta, binary_data, obj, base_off) do
    # member: header, type: hako_action_msgs/ActionResponseHeader (struct), offset: 0, size: 20
    header =
      HakoPdu.HakoActionMsgs.ActionResponseHeaderConverter.binary_read_recursive_action_response_header(
        meta,
        binary_data,
        HakoPdu.HakoActionMsgs.ActionResponseHeader.new(),
        base_off + 0
      )

    # member: body, type: FibonacciResult (struct), offset: 20, size: 8
    body =
      HakoPdu.SampleActionMsgs.FibonacciResultConverter.binary_read_recursive_fibonacci_result(
        meta,
        binary_data,
        HakoPdu.SampleActionMsgs.FibonacciResult.new(),
        base_off + 20
      )
  ...
  def binary_write_recursive_fibonacci_action_response(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_action_msgs/ActionResponseHeader (struct), offset: 0, size: 20
    {allocator, heap_data} =
      HakoPdu.HakoActionMsgs.ActionResponseHeaderConverter.binary_write_recursive_action_response_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: body, type: FibonacciResult (struct), offset: 20, size: 8
    {allocator, heap_data} =
      HakoPdu.SampleActionMsgs.FibonacciResultConverter.binary_write_recursive_fibonacci_result(
        parent_off + 20,
        heap_data,
        allocator,
        obj.body
