require_relative 'pdu_type_ActionFeedbackHeader'


module HakoPdu::HakoActionMsgs::ActionFeedbackHeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_action_feedback_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoActionMsgs::ActionFeedbackHeader.new
    binary_read_recursive_action_feedback_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_action_feedback_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_action_feedback_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_action_feedback_header(meta, binary_data, obj, base_off)
    ...
    obj.reserved = read_primitive_array('uint8', binary_data.byteslice(base_off + 1, 3), 3, 3 / 3)
    ...
    obj.sequence_no = read_primitive('uint32', binary_data.byteslice(base_off + 20, 4))

    obj
  end

  def self.binary_write_recursive_action_feedback_header(parent_off, heap_data, allocator, obj)
    # member: version, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.version, 1))

    # member: reserved, type: uint8 (primitive), offset: 1, size: 3
    add_binary(allocator, parent_off + 1, primitive_array_to_bin('uint8', obj.reserved, 3 / 3))

    # member: goal_id, type: uint8 (primitive), offset: 4, size: 16
    add_binary(allocator, parent_off + 4, primitive_array_to_bin('uint8', obj.goal_id, 16 / 16))

    # member: sequence_no, type: uint32 (primitive), offset: 20, size: 4
    add_binary(allocator, parent_off + 20, primitive_to_bin('uint32', obj.sequence_no, 4))
