defmodule HakoPdu.HakoMavlinkMsgs.HakoAHRS2Converter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_a_h_r_s2(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMavlinkMsgs.HakoAHRS2.new()
    binary_read_recursive_hako_a_h_r_s2(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_a_h_r_s2(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_a_h_r_s2(0, "", "", obj)
  ...
  def binary_read_recursive_hako_a_h_r_s2(meta, binary_data, obj, base_off) do
    # member: roll, type: float32 (primitive), offset: 0, size: 4
    roll = read_primitive("float32", binary_part(binary_data, base_off + 0, 4))

    # member: pitch, type: float32 (primitive), offset: 4, size: 4
    pitch = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))

    # member: yaw, type: float32 (primitive), offset: 8, size: 4
    yaw = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: altitude, type: float32 (primitive), offset: 12, size: 4
    altitude = read_primitive("float32", binary_part(binary_data, base_off + 12, 4))

    # member: lat, type: int32 (primitive), offset: 16, size: 4
    lat = read_primitive("int32", binary_part(binary_data, base_off + 16, 4))

    # member: lng, type: int32 (primitive), offset: 20, size: 4
    lng = read_primitive("int32", binary_part(binary_data, base_off + 20, 4))
  ...
  def binary_write_recursive_hako_a_h_r_s2(parent_off, heap_data, allocator, obj) do
    # member: roll, type: float32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float32", obj.roll, 4))

    # member: pitch, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.pitch, 4))

    # member: yaw, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.yaw, 4))

    # member: altitude, type: float32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("float32", obj.altitude, 4))

    # member: lat, type: int32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("int32", obj.lat, 4))

    # member: lng, type: int32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("int32", obj.lng, 4))
