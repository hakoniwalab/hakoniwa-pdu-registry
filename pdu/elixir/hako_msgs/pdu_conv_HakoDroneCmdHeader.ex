defmodule HakoPdu.HakoMsgs.HakoDroneCmdHeaderConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_drone_cmd_header(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.HakoDroneCmdHeader.new()
    binary_read_recursive_hako_drone_cmd_header(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_drone_cmd_header(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_drone_cmd_header(0, "", "", obj)
  ...
  def binary_read_recursive_hako_drone_cmd_header(meta, binary_data, obj, base_off) do
    # member: request, type: bool (primitive), offset: 0, size: 4
    request = read_primitive("bool", binary_part(binary_data, base_off + 0, 4))

    # member: result, type: bool (primitive), offset: 4, size: 4
    result = read_primitive("bool", binary_part(binary_data, base_off + 4, 4))

    # member: result_code, type: int32 (primitive), offset: 8, size: 4
    result_code = read_primitive("int32", binary_part(binary_data, base_off + 8, 4))
  ...
  def binary_write_recursive_hako_drone_cmd_header(parent_off, heap_data, allocator, obj) do
    # member: request, type: bool (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("bool", obj.request, 4))

    # member: result, type: bool (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("bool", obj.result, 4))

    # member: result_code, type: int32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("int32", obj.result_code, 4))
