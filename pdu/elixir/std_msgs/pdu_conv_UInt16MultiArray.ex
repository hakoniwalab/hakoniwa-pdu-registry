defmodule HakoPdu.StdMsgs.UInt16MultiArrayConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_u_int16_multi_array(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.UInt16MultiArray.new()
    binary_read_recursive_u_int16_multi_array(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_u_int16_multi_array(obj) do
    {base_data, heap_data} = binary_write_recursive_u_int16_multi_array(0, "", "", obj)
  ...
  def binary_read_recursive_u_int16_multi_array(meta, binary_data, obj, base_off) do
    # member: layout, type: MultiArrayLayout (struct), offset: 0, size: 12
    layout =
      HakoPdu.StdMsgs.MultiArrayLayoutConverter.binary_read_recursive_multi_array_layout(
        meta,
        binary_data,
        HakoPdu.StdMsgs.MultiArrayLayout.new(),
        base_off + 0
      )

    # member: data, type: uint16 (primitive), offset: 12, size: 2
    array_size = read_primitive("int32", binary_part(binary_data, base_off + 12, 4))
    offset_from_heap = read_primitive("int32", binary_part(binary_data, base_off + 12 + 4, 4))
    data = read_primitive_array("uint16", binary_part(binary_data, meta.heap_off + offset_from_heap, 2 * array_size), array_size, 2)
  ...
  def binary_write_recursive_u_int16_multi_array(parent_off, heap_data, allocator, obj) do
    # member: layout, type: MultiArrayLayout (struct), offset: 0, size: 12
    {allocator, heap_data} =
      HakoPdu.StdMsgs.MultiArrayLayoutConverter.binary_write_recursive_multi_array_layout(
        parent_off + 0,
        heap_data,
        allocator,
        obj.layout
    ...
    values = obj.data || []
    allocator = add_binary(allocator, parent_off + 12, <<length(values)::little-signed-integer-size(32), offset_from_heap::little-signed-integer-size(32)>>)
    heap_data = add_binary(heap_data, offset_from_heap, primitive_array_to_bin("uint16", values, 2))
