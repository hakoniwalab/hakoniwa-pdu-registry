...
module HakoPdu::HakoSrvMsgs::JoinResponseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_join_response(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::JoinResponse.new
    binary_read_recursive_join_response(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_join_response(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_join_response(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_join_response(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_join_response(parent_off, heap_data, allocator, obj)
    # member: status_code, type: uint32 (primitive), offset: 0, size: 4
