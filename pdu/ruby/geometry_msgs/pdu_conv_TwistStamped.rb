require_relative 'pdu_type_TwistStamped'
...
module HakoPdu::GeometryMsgs::TwistStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_twist_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::TwistStamped.new
    binary_read_recursive_twist_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_twist_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_twist_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_twist_stamped(meta, binary_data, obj, base_off)
    ...
    # member: twist, type: Twist (struct), offset: 136, size: 48
    obj.twist = HakoPdu::GeometryMsgs::Twist.new
    HakoPdu::GeometryMsgs::TwistConverter.binary_read_recursive_twist(meta, binary_data, obj.twist, base_off + 136)

    obj
  end

  def self.binary_write_recursive_twist_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: twist, type: Twist (struct), offset: 136, size: 48
    HakoPdu::GeometryMsgs::TwistConverter.binary_write_recursive_twist(parent_off + 136, heap_data, allocator, obj.twist)
