defmodule HakoPdu.HakoMsgs.DroneVisualStateConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_drone_visual_state(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.DroneVisualState.new()
    binary_read_recursive_drone_visual_state(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_drone_visual_state(obj) do
    {base_data, heap_data} = binary_write_recursive_drone_visual_state(0, "", "", obj)
  ...
  def binary_read_recursive_drone_visual_state(meta, binary_data, obj, base_off) do
    # member: x, type: float32 (primitive), offset: 0, size: 4
    x = read_primitive("float32", binary_part(binary_data, base_off + 0, 4))

    # member: y, type: float32 (primitive), offset: 4, size: 4
    y = read_primitive("float32", binary_part(binary_data, base_off + 4, 4))

    # member: z, type: float32 (primitive), offset: 8, size: 4
    z = read_primitive("float32", binary_part(binary_data, base_off + 8, 4))

    # member: roll, type: float32 (primitive), offset: 12, size: 4
    roll = read_primitive("float32", binary_part(binary_data, base_off + 12, 4))

    # member: pitch, type: float32 (primitive), offset: 16, size: 4
    pitch = read_primitive("float32", binary_part(binary_data, base_off + 16, 4))

    # member: yaw, type: float32 (primitive), offset: 20, size: 4
    yaw = read_primitive("float32", binary_part(binary_data, base_off + 20, 4))

    # member: pwm_duty, type: float32 (primitive), offset: 24, size: 4
    ...
    pwm_duty = read_primitive_array("float32", binary_part(binary_data, meta.heap_off + offset_from_heap, 4 * array_size), array_size, 4)
  ...
  def binary_write_recursive_drone_visual_state(parent_off, heap_data, allocator, obj) do
    # member: x, type: float32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("float32", obj.x, 4))

    # member: y, type: float32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("float32", obj.y, 4))

    # member: z, type: float32 (primitive), offset: 8, size: 4
    allocator = add_binary(allocator, parent_off + 8, primitive_to_bin("float32", obj.z, 4))

    # member: roll, type: float32 (primitive), offset: 12, size: 4
    allocator = add_binary(allocator, parent_off + 12, primitive_to_bin("float32", obj.roll, 4))

    # member: pitch, type: float32 (primitive), offset: 16, size: 4
    allocator = add_binary(allocator, parent_off + 16, primitive_to_bin("float32", obj.pitch, 4))

    # member: yaw, type: float32 (primitive), offset: 20, size: 4
    allocator = add_binary(allocator, parent_off + 20, primitive_to_bin("float32", obj.yaw, 4))
