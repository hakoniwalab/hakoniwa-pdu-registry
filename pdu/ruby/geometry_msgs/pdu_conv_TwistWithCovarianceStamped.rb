require_relative 'pdu_type_TwistWithCovarianceStamped'
...
require_relative 'pdu_conv_TwistWithCovariance'


module HakoPdu::GeometryMsgs::TwistWithCovarianceStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_twist_with_covariance_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::TwistWithCovarianceStamped.new
    binary_read_recursive_twist_with_covariance_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_twist_with_covariance_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_twist_with_covariance_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_twist_with_covariance_stamped(meta, binary_data, obj, base_off)
    ...
    # member: twist, type: TwistWithCovariance (struct), offset: 136, size: 336
    obj.twist = HakoPdu::GeometryMsgs::TwistWithCovariance.new
    HakoPdu::GeometryMsgs::TwistWithCovarianceConverter.binary_read_recursive_twist_with_covariance(meta, binary_data, obj.twist, base_off + 136)

    obj
  end

  def self.binary_write_recursive_twist_with_covariance_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: twist, type: TwistWithCovariance (struct), offset: 136, size: 336
    HakoPdu::GeometryMsgs::TwistWithCovarianceConverter.binary_write_recursive_twist_with_covariance(parent_off + 136, heap_data, allocator, obj.twist)
