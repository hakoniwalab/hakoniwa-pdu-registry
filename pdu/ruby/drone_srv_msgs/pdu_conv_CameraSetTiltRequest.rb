...
module HakoPdu::DroneSrvMsgs::CameraSetTiltRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_camera_set_tilt_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::DroneSrvMsgs::CameraSetTiltRequest.new
    binary_read_recursive_camera_set_tilt_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_camera_set_tilt_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_camera_set_tilt_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_camera_set_tilt_request(meta, binary_data, obj, base_off)
    ...
    obj.tilt_angle_deg = read_primitive('float32', binary_data.byteslice(base_off + 128, 4))

    obj
  end

  def self.binary_write_recursive_camera_set_tilt_request(parent_off, heap_data, allocator, obj)
    # member: drone_name, type: string (primitive), offset: 0, size: 128
    add_binary(allocator, parent_off + 0, primitive_to_bin('string', obj.drone_name, 128))

    # member: tilt_angle_deg, type: float32 (primitive), offset: 128, size: 4
    add_binary(allocator, parent_off + 128, primitive_to_bin('float32', obj.tilt_angle_deg, 4))
