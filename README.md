[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/hakoniwalab/hakoniwa-pdu-registry)
...
The Dockerfile defaults to the ROS distro recorded in `ROS_VERSION.txt`.
The current registry toolchain expects ROS 2 Jazzy:
...
### ROS 2 type hash generation

ROS 2 type hash metadata is generated by a separate tool because it depends on
ROS 2 Jazzy `rosidl` type description APIs. The supported distro is managed by
`ROS_VERSION.txt`, and the tool fails if `ROS_DISTRO` is not `jazzy`.

Build and run the Jazzy image:
...
```bash
cd /root/workspace/hakoniwa-pdu-registry
python3 tools/generate_ros2_type_hash.py config/ros_msgs.txt
```

The default output is:

```text
pdu/type_hash/<pkg>/<Msg>.json
```
...
To also write hashes for resolved dependency messages that are not listed
directly in `config/ros_msgs.txt`:

```bash
python3 tools/generate_ros2_type_hash.py config/ros_msgs.txt --all-resolved
```

Messages that cannot be converted by ROS 2 `rosidl_adapter` are skipped with a
warning by default. Use `--strict` to fail instead. This matters for message names
that are accepted by Hakoniwa's generator but do not satisfy ROS 2 interface
naming rules.
