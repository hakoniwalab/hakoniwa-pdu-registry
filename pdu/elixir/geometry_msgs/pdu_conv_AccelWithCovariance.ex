defmodule HakoPdu.GeometryMsgs.AccelWithCovarianceConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_accel_with_covariance(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.GeometryMsgs.AccelWithCovariance.new()
    binary_read_recursive_accel_with_covariance(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_accel_with_covariance(obj) do
    {base_data, heap_data} = binary_write_recursive_accel_with_covariance(0, "", "", obj)
  ...
  def binary_read_recursive_accel_with_covariance(meta, binary_data, obj, base_off) do
    # member: accel, type: Accel (struct), offset: 0, size: 48
    accel =
      HakoPdu.GeometryMsgs.AccelConverter.binary_read_recursive_accel(
        meta,
        binary_data,
        HakoPdu.GeometryMsgs.Accel.new(),
        base_off + 0
      )

    # member: covariance, type: float64 (primitive), offset: 48, size: 288
    covariance = read_primitive_array("float64", binary_part(binary_data, base_off + 48, 288), 36, div(288, 36))
  ...
  def binary_write_recursive_accel_with_covariance(parent_off, heap_data, allocator, obj) do
    # member: accel, type: Accel (struct), offset: 0, size: 48
    {allocator, heap_data} =
      HakoPdu.GeometryMsgs.AccelConverter.binary_write_recursive_accel(
        parent_off + 0,
        heap_data,
        allocator,
        obj.accel
    ...
    # member: covariance, type: float64 (primitive), offset: 48, size: 288
    allocator = add_binary(allocator, parent_off + 48, primitive_array_to_bin("float64", obj.covariance, div(288, 36)))
