defmodule HakoPdu.HakoMsgs.GameControllerOperationConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_game_controller_operation(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.GameControllerOperation.new()
    binary_read_recursive_game_controller_operation(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_game_controller_operation(obj) do
    {base_data, heap_data} = binary_write_recursive_game_controller_operation(0, "", "", obj)
  ...
  def binary_read_recursive_game_controller_operation(meta, binary_data, obj, base_off) do
    # member: axis, type: float64 (primitive), offset: 0, size: 48
    axis = read_primitive_array("float64", binary_part(binary_data, base_off + 0, 48), 6, div(48, 6))

    # member: button, type: bool (primitive), offset: 48, size: 60
    button = read_primitive_array("bool", binary_part(binary_data, base_off + 48, 60), 15, div(60, 15))
  ...
  def binary_write_recursive_game_controller_operation(parent_off, heap_data, allocator, obj) do
    # member: axis, type: float64 (primitive), offset: 0, size: 48
    allocator = add_binary(allocator, parent_off + 0, primitive_array_to_bin("float64", obj.axis, div(48, 6)))

    # member: button, type: bool (primitive), offset: 48, size: 60
    allocator = add_binary(allocator, parent_off + 48, primitive_array_to_bin("bool", obj.button, div(60, 15)))
