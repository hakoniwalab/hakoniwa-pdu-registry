defmodule HakoPdu.Tf2Msgs.TF2ErrorConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_t_f2_error(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.Tf2Msgs.TF2Error.new()
    binary_read_recursive_t_f2_error(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_t_f2_error(obj) do
    {base_data, heap_data} = binary_write_recursive_t_f2_error(0, "", "", obj)
  ...
  def binary_read_recursive_t_f2_error(meta, binary_data, obj, base_off) do
    # member: error, type: uint8 (primitive), offset: 0, size: 1
    error = read_primitive("uint8", binary_part(binary_data, base_off + 0, 1))

    # member: error_string, type: string (primitive), offset: 1, size: 128
    error_string = read_primitive("string", binary_part(binary_data, base_off + 1, 128))
  ...
  def binary_write_recursive_t_f2_error(parent_off, heap_data, allocator, obj) do
    # member: error, type: uint8 (primitive), offset: 0, size: 1
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint8", obj.error, 1))

    # member: error_string, type: string (primitive), offset: 1, size: 128
    allocator = add_binary(allocator, parent_off + 1, primitive_to_bin("string", obj.error_string, 128))
