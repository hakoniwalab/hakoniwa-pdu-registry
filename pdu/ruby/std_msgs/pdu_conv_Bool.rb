require_relative 'pdu_type_Bool'


module HakoPdu::StdMsgs::BoolConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_bool(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::StdMsgs::Bool.new
    binary_read_recursive_bool(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_bool(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_bool(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_bool(meta, binary_data, obj, base_off)
    # member: data, type: bool (primitive), offset: 0, size: 4
    obj.data = read_primitive('bool', binary_data.byteslice(base_off + 0, 4))

    obj
  end

  def self.binary_write_recursive_bool(parent_off, heap_data, allocator, obj)
    # member: data, type: bool (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('bool', obj.data, 4))
