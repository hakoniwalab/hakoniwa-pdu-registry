...
module HakoPdu::SampleActionMsgs::FibonacciFeedbackConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_fibonacci_feedback(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SampleActionMsgs::FibonacciFeedback.new
    binary_read_recursive_fibonacci_feedback(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_fibonacci_feedback(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_fibonacci_feedback(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_fibonacci_feedback(meta, binary_data, obj, base_off)
    # member: partial_sequence, type: int32 (primitive), offset: 0, size: 4
    ...
    obj.partial_sequence = read_primitive_array('int32', binary_data.byteslice(meta[:heap_off] + offset_from_heap, 4 * array_size), array_size, 4)

    obj
  end

  def self.binary_write_recursive_fibonacci_feedback(parent_off, heap_data, allocator, obj)
    # member: partial_sequence, type: int32 (primitive), offset: 0, size: 4
    offset_from_heap = heap_data.bytesize
    values = obj.partial_sequence || []
    add_binary(allocator, parent_off + 0, [values.length, offset_from_heap].pack('l<l<'))
    add_binary(heap_data, offset_from_heap, primitive_array_to_bin('int32', values, 4))
