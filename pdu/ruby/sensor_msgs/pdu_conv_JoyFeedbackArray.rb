require_relative 'pdu_type_JoyFeedbackArray'
require_relative 'pdu_type_JoyFeedback'
require_relative 'pdu_conv_JoyFeedback'


module HakoPdu::SensorMsgs::JoyFeedbackArrayConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_joy_feedback_array(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::JoyFeedbackArray.new
    binary_read_recursive_joy_feedback_array(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_joy_feedback_array(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_joy_feedback_array(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_joy_feedback_array(meta, binary_data, obj, base_off)
    # member: array, type: JoyFeedback (struct), offset: 0, size: 8
    ...
    array_size.times do |i|
      child = HakoPdu::SensorMsgs::JoyFeedback.new
      HakoPdu::SensorMsgs::JoyFeedbackConverter.binary_read_recursive_joy_feedback(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 8)
      obj.array << child
    end
  ...
  def self.binary_write_recursive_joy_feedback_array(parent_off, heap_data, allocator, obj)
    # member: array, type: JoyFeedback (struct), offset: 0, size: 8
    ...
    values.each_with_index do |child, i|
      HakoPdu::SensorMsgs::JoyFeedbackConverter.binary_write_recursive_joy_feedback(offset_from_heap + i * 8, heap_data, heap_data, child)
    end
