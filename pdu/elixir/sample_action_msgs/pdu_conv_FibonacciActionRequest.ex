defmodule HakoPdu.SampleActionMsgs.FibonacciActionRequestConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_fibonacci_action_request(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SampleActionMsgs.FibonacciActionRequest.new()
    binary_read_recursive_fibonacci_action_request(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_fibonacci_action_request(obj) do
    {base_data, heap_data} = binary_write_recursive_fibonacci_action_request(0, "", "", obj)
  ...
  def binary_read_recursive_fibonacci_action_request(meta, binary_data, obj, base_off) do
    # member: header, type: hako_action_msgs/ActionRequestHeader (struct), offset: 0, size: 20
    header =
      HakoPdu.HakoActionMsgs.ActionRequestHeaderConverter.binary_read_recursive_action_request_header(
        meta,
        binary_data,
        HakoPdu.HakoActionMsgs.ActionRequestHeader.new(),
        base_off + 0
      )

    # member: body, type: FibonacciGoal (struct), offset: 20, size: 4
    body =
      HakoPdu.SampleActionMsgs.FibonacciGoalConverter.binary_read_recursive_fibonacci_goal(
        meta,
        binary_data,
        HakoPdu.SampleActionMsgs.FibonacciGoal.new(),
        base_off + 20
      )
  ...
  def binary_write_recursive_fibonacci_action_request(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_action_msgs/ActionRequestHeader (struct), offset: 0, size: 20
    {allocator, heap_data} =
      HakoPdu.HakoActionMsgs.ActionRequestHeaderConverter.binary_write_recursive_action_request_header(
        parent_off + 0,
        heap_data,
        allocator,
        obj.header
    ...
    # member: body, type: FibonacciGoal (struct), offset: 20, size: 4
    {allocator, heap_data} =
      HakoPdu.SampleActionMsgs.FibonacciGoalConverter.binary_write_recursive_fibonacci_goal(
        parent_off + 20,
        heap_data,
        allocator,
        obj.body
