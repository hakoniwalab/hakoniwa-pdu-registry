require_relative 'pdu_type_HakoStatusMagnetHolder'


module HakoPdu::HakoMsgs::HakoStatusMagnetHolderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_status_magnet_holder(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::HakoStatusMagnetHolder.new
    binary_read_recursive_hako_status_magnet_holder(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_status_magnet_holder(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_status_magnet_holder(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_status_magnet_holder(meta, binary_data, obj, base_off)
    # member: magnet_on, type: bool (primitive), offset: 0, size: 4
    obj.magnet_on = read_primitive('bool', binary_data.byteslice(base_off + 0, 4))

    # member: contact_on, type: bool (primitive), offset: 4, size: 4
    obj.contact_on = read_primitive('bool', binary_data.byteslice(base_off + 4, 4))

    obj
  end

  def self.binary_write_recursive_hako_status_magnet_holder(parent_off, heap_data, allocator, obj)
    # member: magnet_on, type: bool (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('bool', obj.magnet_on, 4))

    # member: contact_on, type: bool (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('bool', obj.contact_on, 4))
