require_relative 'pdu_type_AckermannDriveStamped'
...
require_relative 'pdu_conv_AckermannDrive'


module HakoPdu::AckermannMsgs::AckermannDriveStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ackermann_drive_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::AckermannMsgs::AckermannDriveStamped.new
    binary_read_recursive_ackermann_drive_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ackermann_drive_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ackermann_drive_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ackermann_drive_stamped(meta, binary_data, obj, base_off)
    ...
    # member: drive, type: AckermannDrive (struct), offset: 136, size: 20
    obj.drive = HakoPdu::AckermannMsgs::AckermannDrive.new
    HakoPdu::AckermannMsgs::AckermannDriveConverter.binary_read_recursive_ackermann_drive(meta, binary_data, obj.drive, base_off + 136)

    obj
  end

  def self.binary_write_recursive_ackermann_drive_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: drive, type: AckermannDrive (struct), offset: 136, size: 20
    HakoPdu::AckermannMsgs::AckermannDriveConverter.binary_write_recursive_ackermann_drive(parent_off + 136, heap_data, allocator, obj.drive)
