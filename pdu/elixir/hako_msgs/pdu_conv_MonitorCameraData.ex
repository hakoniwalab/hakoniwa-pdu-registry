defmodule HakoPdu.HakoMsgs.MonitorCameraDataConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_monitor_camera_data(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoMsgs.MonitorCameraData.new()
    binary_read_recursive_monitor_camera_data(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_monitor_camera_data(obj) do
    {base_data, heap_data} = binary_write_recursive_monitor_camera_data(0, "", "", obj)
  ...
  def binary_read_recursive_monitor_camera_data(meta, binary_data, obj, base_off) do
    # member: request_id, type: int32 (primitive), offset: 0, size: 4
    request_id = read_primitive("int32", binary_part(binary_data, base_off + 0, 4))

    # member: image_data_length, type: int32 (primitive), offset: 4, size: 4
    image_data_length = read_primitive("int32", binary_part(binary_data, base_off + 4, 4))

    # member: image, type: sensor_msgs/CompressedImage (struct), offset: 8, size: 272
    image =
      HakoPdu.SensorMsgs.CompressedImageConverter.binary_read_recursive_compressed_image(
        meta,
        binary_data,
        HakoPdu.SensorMsgs.CompressedImage.new(),
        base_off + 8
      )
  ...
  def binary_write_recursive_monitor_camera_data(parent_off, heap_data, allocator, obj) do
    # member: request_id, type: int32 (primitive), offset: 0, size: 4
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("int32", obj.request_id, 4))

    # member: image_data_length, type: int32 (primitive), offset: 4, size: 4
    allocator = add_binary(allocator, parent_off + 4, primitive_to_bin("int32", obj.image_data_length, 4))

    # member: image, type: sensor_msgs/CompressedImage (struct), offset: 8, size: 272
    {allocator, heap_data} =
      HakoPdu.SensorMsgs.CompressedImageConverter.binary_write_recursive_compressed_image(
        parent_off + 8,
        heap_data,
        allocator,
        obj.image
