defmodule HakoPdu.CanMsgs.HakoCanBodyConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_hako_can_body(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.CanMsgs.HakoCanBody.new()
    binary_read_recursive_hako_can_body(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_hako_can_body(obj) do
    {base_data, heap_data} = binary_write_recursive_hako_can_body(0, "", "", obj)
  ...
  def binary_read_recursive_hako_can_body(meta, binary_data, obj, base_off) do
    # member: data, type: uint8 (primitive), offset: 0, size: 8
    data = read_primitive_array("uint8", binary_part(binary_data, base_off + 0, 8), 8, div(8, 8))
  ...
  def binary_write_recursive_hako_can_body(parent_off, heap_data, allocator, obj) do
    # member: data, type: uint8 (primitive), offset: 0, size: 8
    allocator = add_binary(allocator, parent_off + 0, primitive_array_to_bin("uint8", obj.data, div(8, 8)))
