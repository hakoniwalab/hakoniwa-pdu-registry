defmodule HakoPdu.SensorMsgs.PointCloudConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_point_cloud(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.SensorMsgs.PointCloud.new()
    binary_read_recursive_point_cloud(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_point_cloud(obj) do
    {base_data, heap_data} = binary_write_recursive_point_cloud(0, "", "", obj)
  ...
  def binary_read_recursive_point_cloud(meta, binary_data, obj, base_off) do
    ...
    points =
      ...
      |> Enum.map(fn i ->
        HakoPdu.GeometryMsgs.Point32Converter.binary_read_recursive_point32(
          meta,
          binary_data,
          HakoPdu.GeometryMsgs.Point32.new(),
          meta.heap_off + offset_from_heap + i * 12
        )
      end)
    ...
    channels =
      range_indices(array_size)
      |> Enum.map(fn i ->
        HakoPdu.SensorMsgs.ChannelFloat32Converter.binary_read_recursive_channel_float32(
          meta,
          binary_data,
          HakoPdu.SensorMsgs.ChannelFloat32.new(),
  ...
  def binary_write_recursive_point_cloud(parent_off, heap_data, allocator, obj) do
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    {allocator, heap_data} =
    ...
    heap_data = add_binary(heap_data, offset_from_heap, :binary.copy(<<0>>, length(values) * 12))
    heap_data =
      values
      |> Enum.with_index()
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.GeometryMsgs.Point32Converter.binary_write_recursive_point32(
            offset_from_heap + i * 12,
            acc_heap,
            acc_heap,
            child
          )
    ...
    heap_data =
      ...
      |> Enum.reduce(heap_data, fn {child, i}, acc_heap ->
        {new_heap_allocator, new_heap_data} =
          HakoPdu.SensorMsgs.ChannelFloat32Converter.binary_write_recursive_channel_float32(
