...
module HakoPdu::CanMsgs::HakoCanBodyConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_hako_can_body(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::CanMsgs::HakoCanBody.new
    binary_read_recursive_hako_can_body(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_hako_can_body(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_hako_can_body(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_hako_can_body(meta, binary_data, obj, base_off)
    # member: data, type: uint8 (primitive), offset: 0, size: 8
    obj.data = read_primitive_array('uint8', binary_data.byteslice(base_off + 0, 8), 8, 8 / 8)

    obj
  end

  def self.binary_write_recursive_hako_can_body(parent_off, heap_data, allocator, obj)
    # member: data, type: uint8 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_array_to_bin('uint8', obj.data, 8 / 8))
