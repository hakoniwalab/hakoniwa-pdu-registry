require_relative 'pdu_type_TwistWithCovariance'
require_relative 'pdu_type_Twist'
require_relative 'pdu_conv_Twist'


module HakoPdu::GeometryMsgs::TwistWithCovarianceConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_twist_with_covariance(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::TwistWithCovariance.new
    binary_read_recursive_twist_with_covariance(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_twist_with_covariance(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_twist_with_covariance(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_twist_with_covariance(meta, binary_data, obj, base_off)
    # member: twist, type: Twist (struct), offset: 0, size: 48
    obj.twist = HakoPdu::GeometryMsgs::Twist.new
    HakoPdu::GeometryMsgs::TwistConverter.binary_read_recursive_twist(meta, binary_data, obj.twist, base_off + 0)
  ...
  def self.binary_write_recursive_twist_with_covariance(parent_off, heap_data, allocator, obj)
    # member: twist, type: Twist (struct), offset: 0, size: 48
    HakoPdu::GeometryMsgs::TwistConverter.binary_write_recursive_twist(parent_off + 0, heap_data, allocator, obj.twist)
