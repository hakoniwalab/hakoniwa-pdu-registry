...
{
    public static class MultiDOFJointTrajectoryConverter
    {
        public static MultiDOFJointTrajectory PduToMsg(byte[] binaryData)
        {
            var obj = new MultiDOFJointTrajectory();
            var meta = PduMetaData.Parse(binaryData);
            BinaryReadRecursive(meta, binaryData, obj, PduMetaData.PduMetaDataSize);
            return obj;
        }

        public static byte[] MsgToPdu(MultiDOFJointTrajectory obj)
        {
        ...
        public static void BinaryReadRecursive(PduMetaData meta, byte[] binaryData, MultiDOFJointTrajectory obj, int baseOff)
        {
            ...
            obj.points = new List<MultiDOFJointTrajectoryPoint>();
            {
                ...
                for (var i = 0; i < arraySize; i++) {
                    var tmp = new MultiDOFJointTrajectoryPoint();
                    MultiDOFJointTrajectoryPointConverter.BinaryReadRecursive(meta, binaryData, tmp, meta.HeapOff + offsetFromHeap + (i * 32));
                    obj.points.Add(tmp);
                }
            }
        }

        public static void BinaryWriteRecursive(int parentOff, BinaryWriterContainer writer, DynamicAllocator allocator, MultiDOFJointTrajectory obj)
        {
            ...
            }
            {
                ...
                for (var i = 0; i < arraySize; i++) {
                    MultiDOFJointTrajectoryPointConverter.BinaryWriteRecursive(offsetFromHeap + (i * 32), writer, writer.HeapAllocator, obj.points[i]);
                }
            }
        }
    }
