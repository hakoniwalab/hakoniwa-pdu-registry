defmodule HakoPdu.DroneSrvMsgs.MagnetGrabRequestPacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_magnet_grab_request_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.MagnetGrabRequestPacket.new()
    binary_read_recursive_magnet_grab_request_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_magnet_grab_request_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_magnet_grab_request_packet(0, "", "", obj)
  ...
  def binary_read_recursive_magnet_grab_request_packet(meta, binary_data, obj, base_off) do
    ...
    body =
      HakoPdu.DroneSrvMsgs.MagnetGrabRequestConverter.binary_read_recursive_magnet_grab_request(
        meta,
        binary_data,
        HakoPdu.DroneSrvMsgs.MagnetGrabRequest.new(),
        base_off + 268
      )
  ...
  def binary_write_recursive_magnet_grab_request_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_srv_msgs/ServiceRequestHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
    ...
    # member: body, type: MagnetGrabRequest (struct), offset: 268, size: 136
    {allocator, heap_data} =
      HakoPdu.DroneSrvMsgs.MagnetGrabRequestConverter.binary_write_recursive_magnet_grab_request(
        parent_off + 268,
        heap_data,
        allocator,
        obj.body
