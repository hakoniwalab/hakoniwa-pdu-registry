require_relative 'pdu_type_TF2Error'


module HakoPdu::Tf2Msgs::TF2ErrorConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_t_f2_error(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Tf2Msgs::TF2Error.new
    binary_read_recursive_t_f2_error(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_t_f2_error(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_t_f2_error(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_t_f2_error(meta, binary_data, obj, base_off)
    # member: error, type: uint8 (primitive), offset: 0, size: 1
    obj.error = read_primitive('uint8', binary_data.byteslice(base_off + 0, 1))

    # member: error_string, type: string (primitive), offset: 1, size: 128
    obj.error_string = read_primitive('string', binary_data.byteslice(base_off + 1, 128))

    obj
  end

  def self.binary_write_recursive_t_f2_error(parent_off, heap_data, allocator, obj)
    # member: error, type: uint8 (primitive), offset: 0, size: 1
    add_binary(allocator, parent_off + 0, primitive_to_bin('uint8', obj.error, 1))

    # member: error_string, type: string (primitive), offset: 1, size: 128
    add_binary(allocator, parent_off + 1, primitive_to_bin('string', obj.error_string, 128))
