...
module HakoPdu::HakoMsgs::DisturbanceWindConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_disturbance_wind(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::DisturbanceWind.new
    binary_read_recursive_disturbance_wind(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_disturbance_wind(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_disturbance_wind(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_disturbance_wind(meta, binary_data, obj, base_off)
    # member: value, type: geometry_msgs/Vector3 (struct), offset: 0, size: 24
    obj.value = HakoPdu::GeometryMsgs::Vector3.new
    HakoPdu::GeometryMsgs::Vector3Converter.binary_read_recursive_vector3(meta, binary_data, obj.value, base_off + 0)

    obj
  end

  def self.binary_write_recursive_disturbance_wind(parent_off, heap_data, allocator, obj)
    # member: value, type: geometry_msgs/Vector3 (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::Vector3Converter.binary_write_recursive_vector3(parent_off + 0, heap_data, allocator, obj.value)
