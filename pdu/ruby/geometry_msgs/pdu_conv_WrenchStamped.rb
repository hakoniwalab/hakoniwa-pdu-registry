require_relative 'pdu_type_WrenchStamped'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_Wrench'
require_relative 'pdu_conv_Wrench'


module HakoPdu::GeometryMsgs::WrenchStampedConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_wrench_stamped(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::WrenchStamped.new
    binary_read_recursive_wrench_stamped(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_wrench_stamped(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_wrench_stamped(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_wrench_stamped(meta, binary_data, obj, base_off)
    ...
    # member: wrench, type: Wrench (struct), offset: 136, size: 48
    obj.wrench = HakoPdu::GeometryMsgs::Wrench.new
    HakoPdu::GeometryMsgs::WrenchConverter.binary_read_recursive_wrench(meta, binary_data, obj.wrench, base_off + 136)

    obj
  end

  def self.binary_write_recursive_wrench_stamped(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: wrench, type: Wrench (struct), offset: 136, size: 48
    HakoPdu::GeometryMsgs::WrenchConverter.binary_write_recursive_wrench(parent_off + 136, heap_data, allocator, obj.wrench)
