...
module HakoPdu::Ev3Msgs::Ev3PduMotorConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ev3_pdu_motor(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Ev3Msgs::Ev3PduMotor.new
    binary_read_recursive_ev3_pdu_motor(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ev3_pdu_motor(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ev3_pdu_motor(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ev3_pdu_motor(meta, binary_data, obj, base_off)
    # member: power, type: int32 (primitive), offset: 0, size: 4
    obj.power = read_primitive('int32', binary_data.byteslice(base_off + 0, 4))

    # member: stop, type: uint32 (primitive), offset: 4, size: 4
    obj.stop = read_primitive('uint32', binary_data.byteslice(base_off + 4, 4))

    # member: reset_angle, type: uint32 (primitive), offset: 8, size: 4
    obj.reset_angle = read_primitive('uint32', binary_data.byteslice(base_off + 8, 4))

    obj
  end

  def self.binary_write_recursive_ev3_pdu_motor(parent_off, heap_data, allocator, obj)
    # member: power, type: int32 (primitive), offset: 0, size: 4
    add_binary(allocator, parent_off + 0, primitive_to_bin('int32', obj.power, 4))

    # member: stop, type: uint32 (primitive), offset: 4, size: 4
    add_binary(allocator, parent_off + 4, primitive_to_bin('uint32', obj.stop, 4))

    # member: reset_angle, type: uint32 (primitive), offset: 8, size: 4
    add_binary(allocator, parent_off + 8, primitive_to_bin('uint32', obj.reset_angle, 4))
