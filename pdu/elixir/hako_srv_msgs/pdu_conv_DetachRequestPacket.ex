defmodule HakoPdu.HakoSrvMsgs.DetachRequestPacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_detach_request_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.HakoSrvMsgs.DetachRequestPacket.new()
    binary_read_recursive_detach_request_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_detach_request_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_detach_request_packet(0, "", "", obj)
  ...
  def binary_read_recursive_detach_request_packet(meta, binary_data, obj, base_off) do
    ...
    # member: body, type: DetachRequest (struct), offset: 272, size: 8
    body =
      HakoPdu.HakoSrvMsgs.DetachRequestConverter.binary_read_recursive_detach_request(
        meta,
        binary_data,
        HakoPdu.HakoSrvMsgs.DetachRequest.new(),
        base_off + 272
      )
  ...
  def binary_write_recursive_detach_request_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: ServiceRequestHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
    ...
    # member: body, type: DetachRequest (struct), offset: 272, size: 8
    {allocator, heap_data} =
      HakoPdu.HakoSrvMsgs.DetachRequestConverter.binary_write_recursive_detach_request(
        parent_off + 272,
        heap_data,
        allocator,
        obj.body
