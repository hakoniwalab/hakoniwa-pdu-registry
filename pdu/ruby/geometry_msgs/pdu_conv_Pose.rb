...
require_relative 'pdu_type_Point'
require_relative 'pdu_conv_Point'
require_relative 'pdu_type_Quaternion'
require_relative 'pdu_conv_Quaternion'


module HakoPdu::GeometryMsgs::PoseConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_pose(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::GeometryMsgs::Pose.new
    binary_read_recursive_pose(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_pose(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_pose(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_pose(meta, binary_data, obj, base_off)
    # member: position, type: Point (struct), offset: 0, size: 24
    obj.position = HakoPdu::GeometryMsgs::Point.new
    HakoPdu::GeometryMsgs::PointConverter.binary_read_recursive_point(meta, binary_data, obj.position, base_off + 0)

    # member: orientation, type: Quaternion (struct), offset: 24, size: 32
    obj.orientation = HakoPdu::GeometryMsgs::Quaternion.new
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_read_recursive_quaternion(meta, binary_data, obj.orientation, base_off + 24)

    obj
  end

  def self.binary_write_recursive_pose(parent_off, heap_data, allocator, obj)
    # member: position, type: Point (struct), offset: 0, size: 24
    HakoPdu::GeometryMsgs::PointConverter.binary_write_recursive_point(parent_off + 0, heap_data, allocator, obj.position)

    # member: orientation, type: Quaternion (struct), offset: 24, size: 32
    HakoPdu::GeometryMsgs::QuaternionConverter.binary_write_recursive_quaternion(parent_off + 24, heap_data, allocator, obj.orientation)
