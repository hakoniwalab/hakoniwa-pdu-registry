...
2. Python / JavaScript / Godot が C++ バイナリを正しく読めること
3. Python / JavaScript / Godot が生成したバイナリを読んだとき、C++ と同じメンバ値になること
...
- Python / JavaScript / Godot はその C++ 生成バイナリを正しく decode できることを求める
- Python / JavaScript / Godot の encode 結果は、byte 完全一致ではなく decode 後のメンバ値一致を基準にする
...
- `hako_msgs/DroneVisualStateArray`
...
### Godot binary interop

`C++ oracle -> Godot decode` と `Godot encode -> Python decode`:
...
`Godot encode -> Python decode` の `size=0 / 1 / 2`:
...
- `C++ oracle -> Godot decode`
- `Godot encode -> Python decode` の semantic match
- `size=0 / 1 / 2`
- `string[]` / packed float array / struct `varray` / nested `varray`
...
- `tests/test_godot_generator.py`

確認内容:

- generated GDScript type の出力
- generated Godot C++ runtime の出力
- generated Godot C++ converter の出力
- `float64[]` / `string[]` / fixed array / struct `varray` / nested `varray`
...
- headless Godot で generated `GDScript` を実行できること
- `from_dict()` / `to_dict()` の roundtrip が成立すること
- nested struct / `string[]` / packed float array / struct array が壊れないこと
...
- 既定では macOS の `"/Applications/Godot_mono.app/Contents/MacOS/Godot"` を使う
- 別環境では `HAKO_GODOT_BIN` で Godot 実行ファイルを上書きできる

### Godot C++ bridge smoke

- `tests/test_godot_cpp_bridge.py`

確認内容:

- generated Godot C++ converter を GDExtension 経由で実行できること
- `PackedByteArray <-> Dictionary` の roundtrip が成立すること
...
- `HAKO_GODOT_CPP_ROOT` に `godot-cpp` の root を設定する
- 既定の Godot 実行ファイルは `HAKO_GODOT_BIN` で上書きできる
...
- C++ が生成した PDU を Godot が読む

確認内容:

- 値が期待値と一致すること

### 5. Python encode semantic match
...
### 6. JavaScript encode semantic match
...
### 7. Godot encode semantic match

対象:

- 同一 canonical input から Godot が生成した PDU

確認内容:

- Godot が生成した PDU を decode した結果が期待メンバ値と一致すること
- 必要に応じて C++ decode でも同じメンバ値になること
...
- Python / JavaScript / Godot が生成した PDU を C++ が読む
...
- `valid_count` が保持されること
- `DroneVisualState[] drones` の `varray` が正しく decode / encode されること
- 各 `DroneVisualState` が持つ `float32[] pwm_duty` の nested `varray` が正しく配置されること

## 追加ケース: `hako_msgs/DroneVisualStateArray`

用途:

- 箱庭ドローンの可視化用状態配列
- `position(x, y, z)`、`attitude(roll, pitch, yaw)`、`pwm_duty[]`

メッセージ構成:

- `hako_msgs/DroneVisualState`
- `hako_msgs/DroneVisualStateArray`
...
- `valid_count = 100`
- `drones` は 100 要素
- 各要素の `pwm_duty` は 4 要素
- すべて `float32`

確認項目:

- 分割送信ヘッダ 5 項目が保持されること
- `C++ -> Python / JavaScript` decode で 100 台分の値が一致すること
- `Python / JavaScript` encode 後も `valid_count` と各機体の値が一致すること
- `struct varray` の固定部 100 要素が先に連続配置されること
- 各 `pwm_duty` payload がその後ろに連続配置されること
- `PDU total_size` を確認できること

現在の確認値:

- `100 drones x 4 pwm` の `DroneVisualStateArray` は `4856 bytes`
- `size=0` のときに長さと offset が破綻しないこと
