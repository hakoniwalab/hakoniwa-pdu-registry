require_relative 'pdu_type_SimTime'


module HakoPdu::HakoMsgs::SimTimeConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_sim_time(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoMsgs::SimTime.new
    binary_read_recursive_sim_time(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_sim_time(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_sim_time(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_sim_time(meta, binary_data, obj, base_off)
  ...
  def self.binary_write_recursive_sim_time(parent_off, heap_data, allocator, obj)
    # member: time_usec, type: uint64 (primitive), offset: 0, size: 8
