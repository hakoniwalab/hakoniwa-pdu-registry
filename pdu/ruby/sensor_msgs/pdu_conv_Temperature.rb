require_relative 'pdu_type_Temperature'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'


module HakoPdu::SensorMsgs::TemperatureConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_temperature(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::Temperature.new
    binary_read_recursive_temperature(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_temperature(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_temperature(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_temperature(meta, binary_data, obj, base_off)
    ...
    obj.temperature = read_primitive('float64', binary_data.byteslice(base_off + 136, 8))
  ...
  def self.binary_write_recursive_temperature(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    HakoPdu::StdMsgs::HeaderConverter.binary_write_recursive_header(parent_off + 0, heap_data, allocator, obj.header)

    # member: temperature, type: float64 (primitive), offset: 136, size: 8
    add_binary(allocator, parent_off + 136, primitive_to_bin('float64', obj.temperature, 8))
