require_relative 'pdu_type_PointCloud2'
require_relative '../std_msgs/pdu_type_Header'
require_relative '../std_msgs/pdu_conv_Header'
require_relative 'pdu_type_PointField'
require_relative 'pdu_conv_PointField'


module HakoPdu::SensorMsgs::PointCloud2Converter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_point_cloud2(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::SensorMsgs::PointCloud2.new
    binary_read_recursive_point_cloud2(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_point_cloud2(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_point_cloud2(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_point_cloud2(meta, binary_data, obj, base_off)
    ...
    array_size.times do |i|
      child = HakoPdu::SensorMsgs::PointField.new
      HakoPdu::SensorMsgs::PointFieldConverter.binary_read_recursive_point_field(meta, binary_data, child, meta[:heap_off] + offset_from_heap + i * 140)
      obj.fields << child
    end

    # member: is_bigendian, type: bool (primitive), offset: 152, size: 4
    obj.is_bigendian = read_primitive('bool', binary_data.byteslice(base_off + 152, 4))

    # member: point_step, type: uint32 (primitive), offset: 156, size: 4
    obj.point_step = read_primitive('uint32', binary_data.byteslice(base_off + 156, 4))

    # member: row_step, type: uint32 (primitive), offset: 160, size: 4
    obj.row_step = read_primitive('uint32', binary_data.byteslice(base_off + 160, 4))
    ...
    obj.is_dense = read_primitive('bool', binary_data.byteslice(base_off + 172, 4))

    obj
  end

  def self.binary_write_recursive_point_cloud2(parent_off, heap_data, allocator, obj)
    # member: header, type: std_msgs/Header (struct), offset: 0, size: 136
    ...
    add_binary(heap_data, offset_from_heap, "\x00".b * (values.length * 140))
    values.each_with_index do |child, i|
      HakoPdu::SensorMsgs::PointFieldConverter.binary_write_recursive_point_field(offset_from_heap + i * 140, heap_data, heap_data, child)
    end

    # member: is_bigendian, type: bool (primitive), offset: 152, size: 4
    add_binary(allocator, parent_off + 152, primitive_to_bin('bool', obj.is_bigendian, 4))

    # member: point_step, type: uint32 (primitive), offset: 156, size: 4
    add_binary(allocator, parent_off + 156, primitive_to_bin('uint32', obj.point_step, 4))

    # member: row_step, type: uint32 (primitive), offset: 160, size: 4
    add_binary(allocator, parent_off + 160, primitive_to_bin('uint32', obj.row_step, 4))
    ...
    # member: is_dense, type: bool (primitive), offset: 172, size: 4
    add_binary(allocator, parent_off + 172, primitive_to_bin('bool', obj.is_dense, 4))
