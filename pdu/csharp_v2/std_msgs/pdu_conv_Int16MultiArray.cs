...
{
    public static class Int16MultiArrayConverter
    {
        public static Int16MultiArray PduToMsg(byte[] binaryData)
        {
            var obj = new Int16MultiArray();
            var meta = PduMetaData.Parse(binaryData);
            BinaryReadRecursive(meta, binaryData, obj, PduMetaData.PduMetaDataSize);
            return obj;
        }

        public static byte[] MsgToPdu(Int16MultiArray obj)
        {
        ...
        public static void BinaryReadRecursive(PduMetaData meta, byte[] binaryData, Int16MultiArray obj, int baseOff)
        {
            obj.layout = new MultiArrayLayout();
            MultiArrayLayoutConverter.BinaryReadRecursive(meta, binaryData, obj.layout, baseOff + 0);
            obj.data = new List<short>();
            {
                ...
                for (var i = 0; i < arraySize; i++) {
                    obj.data.Add(PduRuntime.ReadInt16(binaryData, meta.HeapOff + offsetFromHeap + (i * 2)));
                }
            }
        }

        public static void BinaryWriteRecursive(int parentOff, BinaryWriterContainer writer, DynamicAllocator allocator, Int16MultiArray obj)
        {
            MultiArrayLayoutConverter.BinaryWriteRecursive(parentOff + 0, writer, allocator, obj.layout);
            {
                ...
                for (var i = 0; i < obj.data.Count; i++) {
                    writer.HeapAllocator.Add(PduRuntime.GetBinaryForInt16(obj.data[i]));
                }
            }
        }
    }
