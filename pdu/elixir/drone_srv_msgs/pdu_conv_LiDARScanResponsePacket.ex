defmodule HakoPdu.DroneSrvMsgs.LiDARScanResponsePacketConverter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_li_d_a_r_scan_response_packet(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.DroneSrvMsgs.LiDARScanResponsePacket.new()
    binary_read_recursive_li_d_a_r_scan_response_packet(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_li_d_a_r_scan_response_packet(obj) do
    {base_data, heap_data} = binary_write_recursive_li_d_a_r_scan_response_packet(0, "", "", obj)
  ...
  def binary_read_recursive_li_d_a_r_scan_response_packet(meta, binary_data, obj, base_off) do
    ...
    body =
      HakoPdu.DroneSrvMsgs.LiDARScanResponseConverter.binary_read_recursive_li_d_a_r_scan_response(
        meta,
        binary_data,
        HakoPdu.DroneSrvMsgs.LiDARScanResponse.new(),
        base_off + 272
      )
  ...
  def binary_write_recursive_li_d_a_r_scan_response_packet(parent_off, heap_data, allocator, obj) do
    # member: header, type: hako_srv_msgs/ServiceResponseHeader (struct), offset: 0, size: 268
    {allocator, heap_data} =
    ...
    # member: body, type: LiDARScanResponse (struct), offset: 272, size: 368
    {allocator, heap_data} =
      HakoPdu.DroneSrvMsgs.LiDARScanResponseConverter.binary_write_recursive_li_d_a_r_scan_response(
        parent_off + 272,
        heap_data,
        allocator,
        obj.body
