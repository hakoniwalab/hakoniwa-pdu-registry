defmodule HakoPdu.StdMsgs.UInt16Converter do
  @pdu_meta_data_size 24
  @pdu_meta_data_magicno 0x12345678
  @pdu_meta_data_version 1

  def pdu_to_elixir_u_int16(binary_data) when is_binary(binary_data) do
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu.StdMsgs.UInt16.new()
    binary_read_recursive_u_int16(meta, binary_data, obj, meta.base_off)
  end

  def elixir_to_pdu_u_int16(obj) do
    {base_data, heap_data} = binary_write_recursive_u_int16(0, "", "", obj)
  ...
  def binary_read_recursive_u_int16(meta, binary_data, obj, base_off) do
    # member: data, type: uint16 (primitive), offset: 0, size: 2
    data = read_primitive("uint16", binary_part(binary_data, base_off + 0, 2))
  ...
  def binary_write_recursive_u_int16(parent_off, heap_data, allocator, obj) do
    # member: data, type: uint16 (primitive), offset: 0, size: 2
    allocator = add_binary(allocator, parent_off + 0, primitive_to_bin("uint16", obj.data, 2))
