...
module HakoPdu::Ev3Msgs::Ev3PduSensorHeaderConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_ev3_pdu_sensor_header(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::Ev3Msgs::Ev3PduSensorHeader.new
    binary_read_recursive_ev3_pdu_sensor_header(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_ev3_pdu_sensor_header(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_ev3_pdu_sensor_header(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_ev3_pdu_sensor_header(meta, binary_data, obj, base_off)
    ...
    obj.hakoniwa_time = read_primitive('int64', binary_data.byteslice(base_off + 136, 8))
  ...
  def self.binary_write_recursive_ev3_pdu_sensor_header(parent_off, heap_data, allocator, obj)
    # member: name, type: string (primitive), offset: 0, size: 128
    ...
    # member: hakoniwa_time, type: int64 (primitive), offset: 136, size: 8
    add_binary(allocator, parent_off + 136, primitive_to_bin('int64', obj.hakoniwa_time, 8))
