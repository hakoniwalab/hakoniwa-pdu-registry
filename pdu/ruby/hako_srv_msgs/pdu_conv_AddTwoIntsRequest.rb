...
module HakoPdu::HakoSrvMsgs::AddTwoIntsRequestConverter
  PDU_META_DATA_SIZE = 24
  PDU_META_DATA_MAGICNO = 0x12345678
  PDU_META_DATA_VERSION = 1

  def self.pdu_to_ruby_add_two_ints_request(binary_data)
    meta = load_pdu_meta(binary_data)
    obj = HakoPdu::HakoSrvMsgs::AddTwoIntsRequest.new
    binary_read_recursive_add_two_ints_request(meta, binary_data.b, obj, meta[:base_off])
    obj
  end

  def self.ruby_to_pdu_add_two_ints_request(obj)
    base_data = ''.b
    heap_data = ''.b
    binary_write_recursive_add_two_ints_request(0, heap_data, base_data, obj)
  ...
  def self.binary_read_recursive_add_two_ints_request(meta, binary_data, obj, base_off)
    # member: a, type: int64 (primitive), offset: 0, size: 8
    obj.a = read_primitive('int64', binary_data.byteslice(base_off + 0, 8))

    # member: b, type: int64 (primitive), offset: 8, size: 8
    obj.b = read_primitive('int64', binary_data.byteslice(base_off + 8, 8))

    obj
  end

  def self.binary_write_recursive_add_two_ints_request(parent_off, heap_data, allocator, obj)
    # member: a, type: int64 (primitive), offset: 0, size: 8
    add_binary(allocator, parent_off + 0, primitive_to_bin('int64', obj.a, 8))

    # member: b, type: int64 (primitive), offset: 8, size: 8
    add_binary(allocator, parent_off + 8, primitive_to_bin('int64', obj.b, 8))
